...
TL;DR: 提出PU-DPO框架，通过正-未标注（PU）学习重新定义偏好优化目标，将放射报告中缺失的发现视为未标注而非阴性，从而防止遗漏噪声污染模型训练，提升胸部X光报告生成的发现检测率。


<details>
  <summary>Details</summary>
Motivation: 现有VLM放射学报告生成模型基于回顾性临床报告训练，这些报告存在遗漏噪声——临床上存在的发现未被报告（如ICU胸片专注于引流管位置而遗漏心脏增大）。标准训练方法会使模型继承这种遗漏，导致模型学会漏报发现。

Method: 提出PU-DPO（Positive-Unlabeled Direct Preference Optimization）：1）将偏好优化目标在PU学习框架下重新表述，将缺失提及视为未标注而非负例；2）通过对模型响应进行编辑构造对比对，生成明确提及或省略特定发现的变体；3）在视觉证据存在时，提及发现的响应自然被偏好。

Result: 在半合成实验和具有人工裁决标签的真实胸部X光基准分析中，PU-DPO在多个病理类别上持续提升检测率和隐藏阳性恢复能力，且对遗漏噪声的鲁棒性优于先前方法。

Conclusion: PU-DPO通过PU学习视角重新设计偏好优化，有效缓解了放射学报告生成中的遗漏噪声问题，为临床安全关键的医学影像报告任务提供了更鲁棒的训练范式。
...
TL;DR: DynaPix是一个基于物理模拟器的视频预测基准，要求模型从候选图像或大规模图库中选出真实未来帧。模型在事件标记时刻表现良好，但在仅靠时间标记的时刻接近随机水平，揭示了模型存在"时间锚定"缺陷——预测与事件挂钩远比与时间挂钩容易。


<details>
  <summary>Details</summary>
Motivation: 现有视频预测评估往往接受文字或看似合理的图像作为答案，从不检验预测结果是否与真实物理状态一致。需要一个可校验的基准来严格评估模型对真实物理场景演化的预测能力。

Method: 构建DynaPix基准：取自物理模拟器的视频片段，在关键事件前停止，模型需从相似候选图像或大规模图库中选出正确的未来帧。正确图像和时间的真值由模拟器精确给定，干扰项刻意设计得与原图相似。

Result: ①模型在可见事件标记目标时刻时表现良好，但在仅靠流逝时间标记时接近随机水平；②图库检索更难，真实图像很少排第一；③人类能良好处理仅靠时间的任务；④基于模拟器真实记录而非教师猜测的场景训练能修复大部分问题，但无法解决长时时间流逝的情况。

Conclusion: DynaPix揭示了模型的"时间锚定"鸿沟：模型将预测锚定到事件上的能力远强于锚定到时间本身。这说明当前模型对物理世界动态的理解仍依赖于可见的因果事件线索，而非对时间流逝本身的内部表征。
...
TL;DR: 本文提出HERA（历史证据路由适配器）框架，通过RRPM轻量适配器将保留的历史证据路由到冻结的潜变量预测器中，解决物理事件遮挡下的预测难题。在IntPhys2基准上，V-JEPA 2-G的AngSurprise准确率从52.57%提升至54.35%，在固定相机连续性和不变性子任务上提升显著。


<details>
  <summary>Details</summary>
Motivation: 预测式视频模型在遮挡下的物理事件预测面临挑战，因为后续预测可能依赖当前视图中已不可见的物体证据。现有方法要么扩大时间上下文、缓存通用视频特征，要么强加显式对象中心状态，但都未直接解决如何选择性地检索相关历史证据并整合到预训练预测器中，而不干扰其原生潜空间。

Method: 提出HERA框架，核心实RPRPM（Register-Routed Patch Memory）轻量适配器，包含三个组件：结构化记忆库（Structured Memory Bank）、记忆寄存器（Memory Registers）和工作区寄存器（Workspace Registers），用于路由保留的历史证据进入冻结的潜变量预测器。

Result: 在IntPhys2 Main split上，HERA+RRPM将V-JEPA 2-G的成对AvgSurprise准确率从52.57%提升到54.35%。子组分析显示固定相机连续性从46.15%提升至57.69%，固定相机不变性从46.15%提升至63.46%。

Conclusion: 历史证据路由是物理预测中潜变量世界模型的一种实用适配策略，验证了通过轻量适配器在不干扰预训练预测器原生工作空间的情况下，有选择地整合历史证据的有效性。
...
TL;DR: 本文提出VideoRouter，一种用于多模态大语言模型长视频理解的新方法，通过构建问题无关的时间层次结构来协调全局与局部补充性证据视图，并引入验证引导路由器选择最终答案，在VideoMME上超越现有帧选择方法2.9个点。


<details>
  <summary>Details</summary>
Motivation: MLLM在视频理解中因token限制难以捕获时间上稀疏的证据。现有方法（如均匀采样或帧选择）通常只优化广度覆盖或局部相关性，难以同时保留全局故事线上下文和细粒度证据。

Method: 1) 将视频组织为问题无关的时间层次结构，粗到细地划分时间连贯片段；2) 上层节点捕获广阔故事线上下文，下层节点保留细粒度局部细节；3) 形成全局视图（覆盖导向推理）和局部视图（细节导向证据恢复）两个互补视角；4) 引入验证引导路由器（verification-guided router）判断哪个视图更有证据支持并选择最终答案。

Result: 在VideoMME数据集上，基于LLaVA-Video-7B骨干网络，VideoRouter比现有最先进的帧选择方法高出2.9个点；实验证明验证引导路由器能有效协调全局与局部推理。

Conclusion: 将长视频理解视为协调互补证据视图而非选择单一帧子集，是更有效的范式。代码将开源。
...
TL;DR: 本文提出TAU-Bench，一个以轨迹为中心的视频异常理解基准，包含1,118个视频、1,454条轨迹和202,438个像素级掩码，联合评估异常实例跟踪与细粒度语义理解，揭示了现有VLM在语义推理与视觉锚定之间存在持续性差距。


<details>
  <summary>Details</summary>
Motivation: 现有视频异常理解（VAU）模型虽然能生成流畅的异常描述，但语义流畅性不保证解释始终锚定在正确的异常实例上。现有基准将跟踪与语义理解分开评估，导致"实例-语义不一致"问题未被充分测量。

Method: 构建TAU-Bench基准，包含1,118个视频、1,454条轨迹和202,438个像素级掩码，覆盖49个事件和45个场景类别。开发了自动化数据引擎，包括异常适配性筛选、异常实例轨迹构建、分层标注注和人工质量控制，提供连接实例级识别、事件级理解和场景级推理的轨迹中心标注。

Result: 对多种代表性VLM家族的评估表明，能够生成合理异常解释的模型仍可能无法可靠地定位和跟踪正确的实例，揭示了语义推理与视觉锚定之间的持续性差距。

Conclusion: 实例锚定的评估是推动更忠实、可靠的视频异常理解系统的重要一步，强调了将视觉定位与语义理解联合评估的必要性。
...
TL;DR: 本文提出StreamArena基准测试，用于评估长时（小时级）交互式流媒体视频理解能力，包含243个平均时长88.8分钟的视频和3,646个开放式问答对。针对现有流媒体模型在连续交互与长时多模态理解之间的张力，作者提出StreamMind双层级架构，通过前端worker处理延迟敏感任务、后端worker异步构建持久化多模态记忆，在所有四项能力上超越现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有评估大多依赖短视频剪辑和选择题格式，导致仅处理最近几帧的简单基线也能超越复杂流媒体模型，且选项设置暴露了语言捷径。这无法真实反映自主多模态体在连续、真实环境中处理无限音视频流和维持小时级记忆的能力。

Method: 提出StreamArena基准（243个全长视频，平均88.8分钟，3,646个开放式QA对）评估实时感知、历史回溯、主动交互和多模态工具利用四种能力；并提出StreamMind双层级架构：前端worker负责延迟关键的交互和主动监控，后端worker异步构建持久化多模态记忆并执行历史回忆和外部搜索，通过复用持久化状态降低查询到回答的延迟。

Result: 评估多种系统暴露了连续交互与长时理解之间的张力：仅保留最近帧的方法无法恢复远期事件，将过去观察转为文本的方法丢失视觉证据，反复压缩视觉记忆的方法难以随时间保留精细细节。StreamMind在所有四种能力上优于现有流媒体基线，并通过复用持久化状态减少查询到回答的延迟。

Conclusion: StreamArena提供了一个更真实、更全面的长时交互式流媒体理解评估框架；StreamMind的双层级架构有效解决了连续交互与长时多模态理解的矛盾，为部署真实环境中的自主多模态体提供了可行方案。
...
TL;DR: 本文提出LAWM-3D，一种通过多视角视频学习3D感知潜在动作（latent actions）的方法，用于世界模型（world models）。研究发现简单地将多视角视频融入LAM训练无法获得3D感知能力，原因是未来帧外观泄漏、相机间外观差异和视角变化。LAWM-3D通过三个紧密耦合的设计解决这些问题：多视角不变统一动作标记方案、几何对齐约束（锚定到3D基础模型）、以及非单射的RGB-D联合重建目标，在生成质量、物理一致性和泛化能力上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 世界模型在开放世界具身智能中的应用受限于动作标注的高成本和跨平台动作空间的异质性。潜在动作模型（LAMs）已通过自监督方式从无标注人类视频中学习动作表示，但现有LAMs主要依赖单视角输入且在2D像素空间操作。作者质疑简单引入多视角视频是否能让LAM学到3D感知能力，并发现答案是否定的，因此需要专门设计来解决这一问题。

Method: LAWM-3D包含三个紧密耦合的关键设计：（1）多视角不变统一动作标记方案，用于学习3D感知的潜在动作；（2）几何对齐约束，将中间编码器特征锚定到预训练的3D基础模型，显式提供跨视角几何对应关系；（3）非单射的RGB-D联合重建目标，防止模型从未来帧外观信息中走捷径，迫使LAM专注于具有几何意义的运动线索。整体采用两阶段范式：大规模人类视频预训练 + 机器人微调。

Result: 大量实验表明，所提出的3D感知潜在动作显著提升了世界模型性能，在生成质量、物理一致性和泛化能力方面均达到SOTA（最先进水平）。

Conclusion: 作者通过深入分析证明了简单地将多视角视频融入LAM训练不足以获得3D感知能力，并提出了LAWM-3D这一完整解决方案。三个组件不是简单堆叠，而是通过统一动机紧密耦合，共同赋予潜在动作3D感知能力，从而显著提升世界模型在下游任务中的表现。
...
TL;DR: 本文提出PhyLatent，一种面向动力学相关性的训练目标，用于JEPA（联合嵌入预测架构）世界模型。作者发现仅防止全局潜在空间坍缩并不能确保表示保留物理状态和动作后果，并识别了JEPA世界模型的三种失败模式：物理不变性坍缩、物理可辨识性坍缩和反事实动力学坍缩。PhyLatent通过三条训练通路（物理不变性、物理可辨识性、反事实动力学）解决这些问题，在多个基准上显著降低了失败率并提升了MPC成功率。


<details>
  <summary>Details</summary>
Motivation: 现有JEPA世界模型虽然能防止全局潜在空间坍缩，但缺乏对表示是否真正保留物理状态和动作后果的保证。作者观察到仅避免全局坍缩不足以学习可靠的状态空间，需要显式地引入动力学相关的训练信号。

Method: PhyLatent提出三种训练通路：1）物理不变性——通过静态视觉不变性确保无关视觉因素不影响物理状态表示；2）物理可辨识性——通过物理状态接地和未来表示对齐确保表示可区分不同物理状态；3）反事实动力学——通过反事实分支分离和潜在去噪确保模型能预测未发生的动作产生的后果。
...
Conclusion: 实验证明，仅防止全局坍缩不足以学习可靠的JEPA世界模型状态空间，需要显式地引入物理动力学相关的训练约束才能获得更好的状态表示和控制性能。
...
TL;DR: 本文提出Engram-E2VID，一种基于结构引导的事件到视频重建框架，通过将参考帧编码为外观记忆印记（appearance engrams），并将事件流转换为目标时刻的运动结构骨架（motion-structure scaffold），在一步扩散骨干网络中实现结构令牌与外观印记的跨层交互激活，从而在不依赖像素级对应关系的情况下完成目标帧重建，在三个基准上相比最强基线PSNR提升最高3.29 dB，LPIPS降低最高0.08。


<details>
  <summary>Details</summary>
Motivation: 事件到视频重建的核心挑战在于：事件只编码稀疏、异步的对数亮度变化而非绝对外观信息，需要将事件派生的目标时刻结构信息与参考帧中的相关外观信息进行关联，尤其是在复杂运动和长时间间隔下。传统的基于像素级对应的方法在这种场景下难以奏效，因此需要一种不依赖直接像素对应的结构引导重建机制。

Method: Engram-E2VID包含三个关键组件：(1) 将参考帧编码为token空间的外观印记（appearance engrams）；(2) 将事件流和参考上下文转换为目标时刻的运动结构骨架，捕获运动边界和事件诱导的结构变化；(3) 在一步扩散骨干网络中，骨架派生的结构令牌跨层渐进地与相关外观印记交互激活，使目标结构能够访问参考外观，同时扩散先验补充不确定或新暴露的区域。

Result: 在三个基准数据集上，Engram-E2VID相比最强同输入基线在PSNR上最高提升3.29 dB，LPIPS上最高降低0.08，并且随着重建时间间隔增加，性能退化速度更慢，展现出更好的长间隔鲁棒性。

Conclusion: 通过token空间的结构-外观关联机制替代传统像素级对应，Engram-E2VID有效解决了事件稀疏异步特性下的外观重建难题，证明了结构引导的生成式激活策略在事件到视频重建中的有效性。
...
TL;DR: 本文提出FoRM（关系流形上的流图蒸馏），将基于关系的知识蒸馏重新建模为关系流形上的连续流映射问题。不同于直接回归恒定的速度场，FoRM学习一个流图算子F_θ(z,t,s)，可直接从当前时刻t的关系状态预测任意目标时刻s的关系状态，从而实现更丰富的轨迹级监督。引入安全半群一致性约束（利用真实桥接状态）和端点锚定损失，抑制幻影状态误差累积和算子漂移。在五个图像恢复任务上超越SOTA蒸馏基线，训练方差降低约50%。


<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法（如图像恢复中的特征对齐或关系矩阵对齐）将教师和学生网络之间的知识转移视为静态目标，忽略了知识转移过程的动态结构。这限制了蒸馏性能的上限，需要一种能够捕捉知识转移动态特性的方法。

Method: 1) 将关系知识转移重构为关系流形上的连续流映射问题；2) 学习流图算子F_θ(z,t,s)，直接从当前状态预测任意目标时刻的关系状态，提供轨迹级监督；3) 引入安全半群一致性约束，利用真实桥接状态保证流图的全局自一致性，消除幻影状态误差累积；4) 端点锚定损失防止算子偏离教师目标。

Result: 在超分辨率、去雨、去噪、去模糊和低光增强五个图像恢复任务上，FoRM一致性地优于SOTA蒸馏基线，并在多种骨干架构上均有效。与朴素流匹配蒸馏相比，训练方差降低了约50%，同时实现了更优的恢复质量。

Conclusion: FoRM验证了将知识蒸馏从静态目标对齐提升到动态流映射范式的有效性，通过轨迹级监督、全局自一致性和端点锚定三重机制，显著提升了知识转移的质量与训练稳定性，为图像恢复任务中的知识蒸馏提供了新范式。
...
TL;DR: HALO是一个面向极端低光遥感图像增强的双先验驱动框架，通过光照不变语义先验（同质性）和伪3D拓扑先验（异质性）协同引导特征聚合，并设计H2CAM模块解决跨模态先验融合中的特征冲突，在8个基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 极端低光条件下现有遥感图像增强方法存在注意力漂移问题，会错误地在不同物理边界间聚合特征，导致结构模糊和颜色失真，难以满足可靠的地球观测和下游机器视觉需求。

Method: 提出双先验驱动框架HALO：1) 光照不变语义先验提供区域同质性作为内容一致聚合的正向偏置；2) 伪3D拓扑先验提供边界异质性作为负向惩罚以阻止跨边界混淆；3) 提出同质-异质协同注意力模块(H2CAM)解决跨模态先验融合中的特征冲突。

Result: 在8个合成和真实遥感基准上达到SOTA性能，显著提升物理边界清晰度和颜色保真度，同时最大化保留下游地球观测任务所需的判别性特征。

Conclusion: 通过引入基础模型先验并将低光增强转化为引导式特征聚合问题，HALO有效解决了注意力漂移问题，为极端低光遥感图像恢复提供了新范式，兼顾了结构完整性和下游任务适应性。
...
TL;DR: 本文提出EvReflection，一种利用事件相机动态信号来消除反射/透射层模糊的新型网络。通过Micro-Dynamics Decoupler解耦层间运动先验，并由Parallax-Attention Rectifier指导RGB图像去伪影，同时构建了首个真实世界数据集EVR²，在合成和真实基准上均达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有反射去除方法依赖单帧静态图像先验，由于反射层与透射层之间存在固有的模糊性，导致严重的残余伪影。作者希望利用事件相机的微动态信息来打破这种两层之间的歧义。

Method: 1) 设计Micro-Dynamics Decoupler，从事件流中解耦出各层专属的运动先验；2) 设计Parallax-Attention Rectifier，利用这些动态先验指导RGB图像去伪影；3) 开发视差感知的仿真管线，并构建EVR²真实世界数据集解决数据稀缺问题。

Result: EvReflection在合成和真实世界基准上均达到最先进性能，PSNR分别超过最佳对比方法1.6 dB和1.2 dB。

Conclusion: 事件信号中的微动态信息能有效打破反射/透射层的模糊性，为反射去除提供新的解决思路。代码、数据集和预训练模型均已开源。
...
TL;DR: 本文提出了一种基于智能手机照片的牙齿定位与编号模型，使用定制化Mask R-CNN流水线，在1,272张标注图像上训练，通过掩码灰度世界白平衡算法和解剖约束检测层处理患者生成数据的变异性，内部测试达到AP@50=0.818，外部测试达到AP@50=0.901，并以开源容器化API形式发布。


<details>
  <summary>Details</summary>
Motivation: 全球数十亿人面临口腔健康问题，但专业牙科护理成本高昂且可及性有限。现有研究依赖临床级X光片或口内相机图像，公众无法获得这些设备进行自我筛查，因此需要一种基于普通人可获取的智能手机照片的牙科筛查方案。

Method: 采用定制化的Mask Region-based Convolutional Neural Network (Mask R-CNN)流水线，在1,272张标注的智能手机图像上训练。流水线包含两个领域知识驱动的机制：1）掩码灰度世界白平衡算法，用于减少人工色彩偏差；2）解剖约束检测层，用于强制结构有效性并抑制假阳性。评估分为四个阶段：内部留出测试、独立外部测试、描述性消融研究和基于折的训练稳定性分析。

Result: 内部测试集上实例掩膜AP@50为0.818，类感知PQ为0.780，操作F1为0.884。十次运行中AP@50标准差仅0.009，显示训练稳定性良好。外部数据集上AP@50为0.901，PQ为0.832，F1为0.928，尽管人群、传感器和采集协议存在差异。推理流水线已作为开源容器化API发布。

Conclusion: 消费级智能手机图像能够支持自动化的牙级解剖映射，为资源受限环境中的远程筛查和远程牙科提供了可扩展、低成本的基础。
...
TL;DR: 本文系统评估了15种机器学习模型（包括TabPFN）用于美国西部流域尺度山火后泥石流预测，结合SHAP特征重要性分析和TabPFN合成数据增强，发现TabPFN在未增强数据上表现最优（威胁分数0.637），短时降雨强度和风暴累积量是最重要特征，合成数据增强对除CNN外的所有模型均有提升。


<details>
  <summary>Details</summary>
Motivation: 山火后泥石流预测对减灾至关重要，但现有机器学习模型面临三大挑战：泥石流与非泥石流事件在特征空间中重叠导致分类困难、模型可解释性不足、训练数据有限。因此需要系统性地评估不同模型的预测性能、特征重要性和数据增强策略。

Method: 基于美国西部流域尺度的山火后泥石流观测数据，比较15种模型（包括TabPFN、树模型和深度学习模型）；使用重复分层交叉验证评估性能；采用SHAP进行特征重要性分析；利用TabPFN生成合成样本以缓解数据稀缺问题。

Result: TabPFN在未增强数据上取得最高威胁分数0.637，仅次于最佳树模型；SHAP分析显示短历时降雨强度和风暴累积量是最重要的预测特征，燃烧严重度和地形特征贡献较小；TabPFN合成数据增强使除CNN外所有模型性能提升，深度学习模型平均威胁分数最高提升+0.041。

Conclusion: 该工作通过结合严格的模型基准测试、可解释的特征分析和合成数据增强，为改进山火后泥石流预测提供了综合框架，验证了TabPFN在该领域的应用潜力及合成数据增强的有效性。
...
TL;DR: 本文提出一种以设备为中心的工件定位框架，利用多台静态2D相机观测搬运设备，通过关键点引导的注意力机制增强3D CNN的行为识别，结合事件驱动的有限状态机推断热锻车间中工件的连续位置，在真实工厂中实现100%事件检测准确率（33秒容差窗口）、317.8mm平均定位误差和21秒系统延迟。


<details>
  <summary>Details</summary>
Motivation: 热锻过程中工件直接跟踪不可靠（极端温度、表面退化、路径不规则），而连续工件定位对可追溯性和工艺协调至关重要，因此需要替代性的定位方案。

Method: 框架包含三个核心部分：(1) 从多台静态2D相机估计设备在楼层平面空间中的3D坐标；(2) 识别抓取和释放行为，其中关键点引导的注意力机制集成到3D CNN中，聚焦功能相关设备区域以提升识别性能；(3) 事件驱动的有限状态机将行为验证为离散处理事件，并持续更新工件状态和位置。

Result: 在运营中的热锻工厂评估中，事件检测准确率达100%（33秒容差窗口），平均定位误差317.8mm，平均系统延迟21秒。

Conclusion: 该框架将视觉感知与可解释的事件驱动推理相结合，支持工件转移可视化和设备操作的定量分析，为热锻场景提供了可靠的连续工件定位方案。
...
TL;DR: 本文研究了条件查询模型中自适应测试与非自适应测试之间的差异。研究表明，当两个分布类之间存在正的条件概率分离时，它们是可学习的；当分离为零时，最优最坏情况误差恰好为1/2。任何T查询自适应策略都可以被一个使用O(N²(T + log(1/ρ)))对查询的随机非自适应程序模拟，其转录在总变差距离内与自适应转录相差不超过ρ。匹配下界表明最坏情况固定误差的自适应差距为Θ(N²)。


<details>
  <summary>Details</summary>
Motivation: 模型评估中的关键设计选择是：是在观察任何响应之前就固定所有测试，还是根据早期响应选择后续测试。理解这两种策略之间的能力差异对于评估协议设计和计算资源分配至关重要。

Method: 在有限结果空间X（|X|=N）上的条件查询模型中，研究了两类分布的可区分性条件。通过构造随机非自适应程序来模拟自适应测试器的行为轨迹，并建立匹配的下界族来证明自适应与非自适应查询复杂度之间的精确差距。

Result: 1) 可学习性当且仅当两类分布的条件概率具有正分离；2) 分离为零时最优最坏情况误差恰为1/2；3) 任何T查询自适应策略可由O(N²(T + log(1/ρ)))个预先固定的对查询以ρ精度（总变差距离）模拟；4) 匹配下界显示自适应的固定误差差距为Θ(N²)。

Conclusion: 交互可以最多将所需测试数量减少二次因子，但自适应测试看似指数级的分支结构并不能带来指数级的查询优势。非自适应预先规划可以用多项式代价替代自适应交互。
...
TL;DR: 本文提出抽象事件因果规则（AECR），一种新型的关系级因果抽象范式，将具体因果对转化为泛化的抽象因果逻辑，并通过多智能体系统（CACI）与相似性约束聚类从噪声数据中提取可信的AECR，构建了两个完整的AECR知识库。同时提出抽象规则引导的因果注意力编码器（AR-GCAE），将检索到的AECR注入事件预测任务，显著增强了事件因果推理的泛化能力，尤其在低频长尾和未见事件样本上效果最为显著。


<details>
  <summary>Details</summary>
Motivation: 现有实例级因果对在低频长尾和未见事件组合上存在严重的泛化缺陷，难以支撑事件中心智能分析系统对因果关系知识的需求。需要一种更高层面的关系级因果抽象来克服这一局限性。

Method: 1) 提出AECR范式，将具体因果对转化为抽象因果逻辑；2) 设计多智能体CACI系统，结合相似性约束聚类，从噪声原始因果数据中提炼可信的AECR；3) 构建两个完整的AECR知识库；4) 提出AR-GCAE编码器，通过规则引导注意力层和门控表示融合，将检索到的AECR注入CGEP基准任务的因果图事件预测中。

Result: 定量实验表明，应用AECR显著增强了事件因果推理的泛化能力，为事件预测带来一致的性能提升，其中最显著的改进出现在稀有和未见事件样本上。

Conclusion: 抽象因果规则（AECR）是一种有效的关系级因果抽象方法，能够克服实例级因果对泛化不足的问题，为事件因果推理和预测任务提供重要的知识支撑。
...
TL;DR: PD-GS提出了一种基于音素驱动的3D高斯泼溅说话人头生成方法，通过融合时间对齐的音素令牌与连续音频嵌入，有效解决了唇形闭合不准确和"漏嘴"伪影问题。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS说话人头渲染虽能实现快速逼真的效果，但唇部运动常被过平滑，无法准确表达双唇闭合等硬性发音约束，产生"漏嘴"伪影。根本原因是连续音频嵌入在回归目标下难以精确推断离散且短暂的发音事件。

Method: 提出PD-GS框架，包含两个关键部分：(1) 从ASR和强制对齐管道获取时间对齐的音素令牌；(2) 核心组件Linguistic Fusion Module (LFM)，通过可学习的门控机制自适应融合连续音频上下文与离散音素嵌入，在保留平滑音频驱动动态的同时，增强关键发音段的音素引导。模型仅使用单目视频的重建损失和唇部地标监督进行训练。

Result: 在HDTF数据集上，PD-GS取得了最佳的唇部几何指标(LMD 2.66)，并在具有挑战性的音素序列中定性减少了闭合违规现象，生成更符合语言学规律的神经化身。

Conclusion: 通过引入显式的音素级语言线索并与连续音频嵌入自适应融合，PD-GS有效解决了音频驱动说话人头生成中的唇部发音精度问题，验证了离散语言信号对改善连续回归目标局限性的价值。
...
TL;DR: 本文使用Equinor公开的Volve油田数据进行机器状态监测异常检测研究，构建了与工程文档交叉验证的异常标签，并测试了无监督基线和双头监督模型的可学习性，全部数据和代码已公开。


<details>
  <summary>Details</summary>
Motivation: 大多数机器状态监测基准数据集来自测试台架，故障是人为诱导且事件已知，而真实生产场景中传感器数据没有故障日志，异常检测方法需要自行发明标签，容易引入隐性假设。作者希望解决真实工业数据中标签构建和验证的问题。

Method: 1) 基于Volve油田开放数据构建异常标签，标签不仅基于数据模式，还通过油田工程文档交叉验证物理可行性，并公开每个标签的推理依据；2) 使用无监督基线检测器测试标签的可学习性；3) 引入一个来自金属缺陷检测工作的双头小模型，同时预测事件发生时间和事件类型。

Result: 无监督检测器在未见过标签的情况下仍能覆盖规则标记的区域，证明标签并非任意构造；紧凑的监督模型能较好地跨井恢复事件存在性和事件类型，但事件时间定位仅达到粗略精度。

Conclusion: 研究诚实地报告了成功与失败之处及所有假设，并公开了数据集、标签、每条标签的来源依据、基线分数、训练模型和代码（CC-BY-NC-SA 4.0协议），为真实工业场景下的异常检测提供了可复现的基准。
...
TL;DR: ChainClaw是一个基于OpenClaw构建的区块链原生智能体框架，通过事件驱动编排、基于模拟的安全智能、链上监控运行时三层架构，以及跨层记忆子系统，解决通用大语言模型智能体在区块链环境中面临的三大核心缺陷：反应性（Reactivity）、不可逆性（Irreversibility）和可观测性（Observability）。实验表明其在安全性和任务完成度上均优于代表性基线。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型智能体在工具增强任务上表现优异，但其基本假设在区块链环境中失效。区块链的链上执行具有有状态性、对抗性和经济不可逆性，导致智能体面临反应性、不可逆性和可观测性三大鸿沟，现有方法无法直接适配。

Method: ChainClaw采用分层架构解决三大问题：(1) 事件驱动编排层通过事件摄入和模拟反馈解决Reactivity；(2) 基于模拟的安全智能层通过预执行安全流水线（含交易模拟和动作守卫）解决Irreversibility；(3) 链上监控运行时层通过链上读取适配器和交易监控器解决Observability。各层由跨层记忆子系统统一连接。

Result: 在包含七大任务、四大类别、五个维度的专用基准测试上，ChainClaw在安全性和任务完成度两个维度上均一致性地优于代表性基线方法。

Conclusion: ChainClaw为区块链环境中的LLM智能体提供了系统性的解决方案，填补了通用智能体框架在链上场景的三项关键能力缺口，验证了分层架构加跨层记忆设计的有效性，为区块链原生智能体的安全可靠部署奠定了基础。
...
TL;DR: 本文提出使用视觉语言模型（VLMs）对视频游戏数据集进行标注，以提取人类定义的奖励信号，从而在不访问游戏引擎的情况下，利用离线强化学习训练一个能够响应特定回报的条件化智能体。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习在视频游戏中的应用存在三大痛点：1) 需要访问游戏引擎才能获取奖励；2) 奖励的识别和权重设置需要困难且耗时的试错过程；3) 奖励通常稀疏且难以理解其对学习策略的影响。这些问题使得RL技术难以使用。

Method: 使用视觉语言模型（VLMs）对视频游戏数据集进行标注，通过指令让VLM提取人类定义的奖励信号，然后利用离线强化学习训练一个条件化智能体，使其能够根据期望回报做出相应行为。

Result: 初步实验显示该方法可行，即离线RL可以成功地利用VLM标注的奖励来训练条件化智能体，但同时发现了早期实验中的一些困难和局限性。

Conclusion: 该方法为缓解视频游戏中RL的奖励获取和设计难题提供了一种有前景的方向，但仍需解决早期实验中暴露的局限性和挑战。
...
TL;DR: 本文提出InvestLogicBench，一个包含151位真实投资者、201,247条决策记录的流程原生基准。它通过P→E→R→D→O（画像→事件→推理→决策→结果）完整轨迹评估金融LLM，发现主流模型逻辑合理性很高但事件锚定很差，揭示了仅供结果评估无法暴露的"看似合理实则漂移"的推理问题。


<details>
  <summary>Details</summary>
Motivation: 投资能力本质上是高度个性化的——同样的市场证据对不同投资者可能意味着不同的行动。然而现有金融LLM评估要么是静态问答（忽略了主体能动性），要么只看最终盈亏（无法区分有根据的盈利、符合画像的行动还是单纯运气）。现有评估标准对这类"后果性智能体"来说是错误的标尺。

Method: 构建InvestLogicBench基准，包含201,247条来自151位真实投资者的已记录决策。每个样本实例化一条P→E→R→D→O轨迹：投资者画像、可观察市场事件、投资推理、可执行决策、延迟结果。数据支持画像构建、时点事件绑定、结构化逻辑、投资期限、结果与复盘，并支持理解、画像条件生成和端到端回放三类任务。

Result: 在四个领先LLM上评估发现：逻辑合理性（logical plausibility）接近4/5分，但事件锚定（event grounding）仅0.8-2.8/5分；收益与过程质量也相互不一致。这暴露了模型产生"抛光但弱锚定"的推理，而这种问题被仅看结果的评估所掩盖。

Conclusion: 作者主张P→E→R→D→O应作为数据系统接口来设计，需要版本化画像、时间溯源、可检查的检索、决策账本和可回放的结果。金融只是更广泛的个性化、后果性智能体类别的一个压力测试场景。
...
TL;DR: 本文提出了一种基于可执行事件轨迹（trace-grounded）的参数化剖析方法，用于评估视频事件计数任务。通过在三个受控视频任务（弹球墙接触、视觉眨眼、类别状态转换）中变化事件数量N和频率F，并在2,190个视频上提供可执行事件轨迹作为ground truth，揭示了模型在时间推理上的阶段性失败模式：事件表征类型决定了模型能否访问证据，高计数高频率场景下仅0.2%正确，提高采样率虽能提升最终得分但事件恢复保真度极低。


<details>
  <summary>Details</summary>
Motivation: 现实视频基准虽然覆盖广泛，但固定片段将事件数量、频率、时长和视觉复杂度纠缠在一起，难以隔离失败模式。现有的程序化基准虽提供控制，但只对最终答案打分，未能将报告事件与可执行的ground truth进行审计。需要一种既能精确控制参数又能逐事件验证的方法。

Method: 设计了三个受控视频任务（弹球墙接触、视觉眨眼、类别状态转换），在固定渲染条件下变化事件计数N和频率F，生成2,190个视频。每个视频附带可执行事件轨迹，支持能力曲面估计和时间戳级评估。通过80%可靠性阈值分析事件表征类型的影响，并通过提高采样率和不同提示策略进行消融测试。

Result: 模型对持续状态转换的计数在12个事件、0.5和1.0 Hz下可达80%可靠性，但对瞬态眨眼事件无可靠的正计数区间。高计数高频率下仅0.2%计数值正确，事件恢复率仅18.1%。提高采样率将弹球准确率从19.6%提升到29.3%，但序列级ground truth一致性仅3.7%。提示策略收益有限，真实视频评估同样呈现低事件计数集中的成功模式。

Conclusion: 事件的时间表征类型（持续 vs 瞬态）是模型能否初始访问证据的关键瓶颈，该限制随计数和频率增加而复合累积。额外帧可以提高最终分数但不会产生忠实的事件恢复，揭示分数提升与真实能力之间的脱节。trace-grounded剖析将视频评估从聚合准确率转向详细的时间推理失败诊断。
...
TL;DR: 本文提出电路锚定进化（CAE），借鉴生物学中发育约束（Hox基因锚定）的思想，通过机制可解释性识别少于2%的安全电路特征并加以锚定约束，防止大语言模型在自我进化过程中"误进化"为能力增强但失去安全性的危险系统。实验证明CAE在3个模型家族和2种进化算法上优于显式奖励约束。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的自我进化算法缺乏类似生物学发育约束的机制，纯粹优化能力表现，隐含假设安全性会自然保持。但实验证明这一假设错误：模型可能"误进化"为强大但危险的实体。生物学中自然通过发育约束（核心调控基因锚定，外围基因自由适应）来解决这一问题，本文受此启发。

Method: 1) 利用机制可解释性识别出占模型特征不到2%的"安全电路"，该电路因果中介了安全行为。2) 在进化过程中锚定该安全电路，将其限制在较小的位移范围内，同时允许其余特征自由进化。3) 这模仿了生物学中"有约束的可进化性"原则：保护必需功能，适应外围功能。

Result: 在3个模型家族和2种进化算法上的实验表明，CAE在安全性保持和最小能力损失方面表现优异，在有效性和效率上均大幅优于显式的奖励基约束方法。

Conclusion: 正如发育约束防止生物学进化产生不可存活的生命体，电路锚定机制能够防止模型进化产生有能力但危险的系统，为大语言模型的自我进化提供了安全保障范式。
...
TL;DR: 本文提出DREAM，一个基于ABC认知模型的结构化记忆框架，用于角色扮演代理。它将非结构化文学文本转化为事件感知记忆图（EMG），组织为时间有序且因果关联的事件图，构建动态双粒度角色档案，并引入时间因果记忆（TCM）基准来评估时间一致性与长程因果叙事连贯性。在CoSER、LIFECHOICE和TCM基准上均取得最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有角色扮演代理主要依赖静态角色描述和非结构化记忆，难以维持长期叙事与人格一致性。角色扮演不仅需要风格模仿，还需要时间一致性和因果推理的行为逻辑，而当前方法在这方面的能力有限。

Method: 受ABC认知模型启发，DREAM将非结构化文本转化为事件感知记忆图（EMG），以时间有序、因果关联的事件图组织角色经验；并基于此构建包含稳定人格特质与事件驱动行为演化的动态双粒度角色档案。

Result: DREAM在CoSER、LIFECHOICE和TCM三个基准上均超越多个强基线，取得最先进性能，验证了结构化记忆在角色扮演代理中的有效性。

Conclusion: 结构化记忆能够显著提升角色扮演代理的可解释性和一致性，为未来角色模拟提供了更可靠的理论基础和实践路径。
...
TL;DR: 本研究揭示了大语言模型在非祈使句法形式下普遍存在的拒绝机制失效问题。通过行为评估发现16个模型（最高70B参数）均受影响，运用因果中介分析定位到拒绝决策部分依赖于上游句法特征，并通过操纵这些句法特征可以触发或抑制拒绝行为。最终将问题归因于开源模型后训练数据中的语言学偏见，证明增加句法多样性可以缓解该问题。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型对齐技术虽然能防范许多直接攻击，但存在大量绕过安全防护的复杂越狱方法。此前研究发现改变语法时态（现在时改为过去时）即可诱发有害响应，这说明当前的拒绝机制存在更深层的缺陷，需要系统性地探究非祈使句法形式对安全对齐的影响及其根本原因。

Method: 1. 行为评估：在16个不同规模（最高70B参数）的模型上系统测试非祈使句法形式的安全漏洞。2. 因果中介分析：使用因果中介分析方法定位拒绝决策与上游句法特征之间的因果关系。3. 句法特征操控实验：通过引导纯句法特征实现拒绝的触发与抑制。4. 数据溯源分析：追溯问题根源至开源模型的后训练数据，并通过增加句法多样性进行缓解验证。

Result: 实验证明句法脆弱性在16个模型中普遍存在；因果中介分析发现拒绝部分依赖于上游句法特征而非纯粹语义；通过操控句法特征能够主动触发或抑制拒绝机制；问题根因被追溯至后训练数据中的语言学偏见，增加句法多样性能够有效缓解该问题。

Conclusion: 当前的安全对齐方法引入了混淆变量，阻碍了拒绝决策在纯粹语义层面的合理落地。句法多样性的缺乏导致模型将句法形式误作为拒绝的判据，这使得对抗者可以通过简单的句法变形绕过安全防护。研究建议在数据层面增加句法多样性，以实现更稳健、语义驱动的安全对齐。
...
TL;DR: 本文提出了一种结合结构传感器与AI数据分析的实时车辆状态监测与冲击检测框架，旨在解决干线铁路在GoA4全自动运行中的安全挑战，涵盖冲击/碰撞/碾压事件检测、基于状态的维护以及长期数据驱动的设计优化三大应用。


<details>
  <summary>Details</summary>
Motivation: 虽然GoA4全自动运行已在地铁系统中成熟应用，但在干线铁路中部署仍受限，主要原因是严苛的安全要求和开放环境的复杂性。现有基于摄像头、雷达和激光雷达的感知系统能有效检测物体，但无法可靠识别冲击、碰撞和碾压事件，存在安全监测盲区。

Method: 提出一个集成框架，将结构传感器技术（structural sensor technologies）与基于AI的数据分析相结合，实现实时车辆状态监测和冲击检测。框架针对三个关键应用：实时冲击/损伤/碾压事件自动检测、基于连续监测的状态维护、支持设计优化的长期数据分析。

Result: 实验结果表明该方法具有可行性，能够增强运营安全性、实现预测性维护策略，并支持干线铁路向全自动运行过渡。

Conclusion: 该研究证明了结构传感与AI融合方法在干线铁路全自动运行中的潜力，为弥补现有感知系统在物理事件（冲击、碰撞、碾压）检测方面的不足提供了新路径。
