...
Motivation: 现有方法需要同时采集事件和强度图像，硬件实现受限。本文旨在开发纯事件驱动的光场成像方法，降低硬件要求
...
Method: 提出ArtPro框架：1）基于几何特征和运动先验进行过分割初始化，生成具有合理运动假设的部件提案；2）优化过程中通过分析空间邻居间的运动一致性动态合并提案；3）采用碰撞感知的运动剪枝机制防止错误的运动估计。

Result: 在合成和真实世界物体上的大量实验表明，ArtPro能够鲁棒地重建复杂多部件物体，在准确性和稳定性方面显著优于现有方法。

Conclusion: ArtPro通过自适应整合运动提案，解决了现有自监督方法对初始分割敏感的局限性，为复杂多部件物体的高保真数字孪生重建提供了有效解决方案。
...
TL;DR: 首个实时运动感知事件抑制框架，能过滤由独立运动物体和自运动触发的事件，实现动态事件的预测性抑制


<details>
  <summary>Details</summary>
Motivation: 事件相机在动态场景中面临挑战，独立运动物体和自运动触发的事件会干扰下游应用性能，需要实时过滤这些动态事件
...
Conclusion: 提出的运动感知事件抑制框架能有效过滤动态事件，显著提升下游应用性能，为事件相机在动态场景中的实际应用提供了有效解决方案
...
TL;DR: SceneTransporter是一个从单张图像生成结构化3D场景的端到端框架，通过最优传输约束解决现有方法在开放世界场景中无法组织部件形成独立实例的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法能生成部件级的3D物体，但在开放世界场景中无法将这些部件组织成独立的实例。通过去偏聚类探测发现，这种失败源于模型内部分配机制缺乏结构约束。

Method: 将结构化3D场景生成重新定义为全局相关分配问题，在组合DiT模型的去噪循环中制定并求解熵最优传输目标。该公式施加两个结构约束：1）传输计划门控交叉注意力，强制图像块到部件级3D潜在表示的一对一独占路由；2）传输的竞争性鼓励相似块的分组，并通过基于边缘的成本进行正则化。

Result: 在开放世界场景生成任务上，SceneTransporter超越了现有方法，显著提高了实例级连贯性和几何保真度。

Conclusion: 通过将结构化3D场景生成重新定义为全局相关分配问题，并引入最优传输约束，SceneTransporter成功解决了现有方法在组织部件形成独立实例方面的不足，实现了更好的开放世界场景生成效果。
...
TL;DR: PackUV提出了一种新的4D高斯表示方法，将高斯属性映射到结构化多尺度UV图集，实现紧凑的图像原生存储，并与标准视频编解码器兼容。


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯泼溅的方法在长序列、时间不一致性、大运动和遮挡方面存在问题，且输出与常规视频编码流水线不兼容，阻碍了实际应用。

Method: 提出了PackUV-GS方法：1) 将高斯属性映射到结构化多尺度UV图集；2) 在UV域直接优化高斯参数；3) 使用流引导的高斯标记和视频关键帧模块识别动态高斯、稳定静态区域并保持时间一致性。

Result: 方法在渲染保真度上超越现有基线，能扩展到长达30分钟的序列并保持一致性质量。提出的PackUV-2B数据集是目前最大的多视角视频数据集。

Conclusion: PackUV是首个与标准视频编解码器兼容的统一体素视频表示，可在现有多媒体基础设施中实现高效流式传输，解决了体素视频重建、存储和流式传输的难题。
...
TL;DR: 提出LADS（局部自适应衰减表面）事件表示方法，通过局部信号动态调整时间衰减，在静止区域保留细节，在密集活动区域减少模糊，显著提升事件相机的人脸检测和关键点定位性能。
...
Method: 提出LADS（局部自适应衰减表面）表示方法，基于三种策略调整每个位置的时间衰减：事件率、拉普拉斯-高斯响应和高频频谱能量。这些自适应方案在静止区域保留细节，在密集活动区域减少模糊。

Result: 在公开数据集上，LADS相比标准非自适应表示持续提升人脸检测和面部关键点准确率。在30Hz时达到更高检测准确率和更低关键点误差；在240Hz时缓解了高频下典型的准确率下降，保持2.44%归一化平均误差和0.966 mAP50，甚至超过先前工作在30Hz的报告准确率。

Conclusion: LADS通过表示阶段保留空间结构，支持使用更轻量网络架构同时保持实时性能。结果强调了上下文感知时间集成对神经形态视觉的重要性，为利用事件相机独特优势的实时高频人机交互系统指明了方向。
...
TL;DR: GUIPruner是一个无需训练的GUI代理压缩框架，通过时间自适应分辨率和分层结构感知剪枝，解决视觉GUI代理中的时空冗余问题，实现3.4倍FLOPs减少和3.3倍加速，同时保持94%以上性能。


<details>
  <summary>Details</summary>
Motivation: 纯视觉GUI代理存在严重的效率瓶颈，主要源于高分辨率截图和历史轨迹的巨大时空冗余。现有压缩方法存在两个关键错配：时间错配（均匀历史编码与代理"衰减记忆"注意力模式不符）和空间拓扑冲突（非结构化剪枝破坏坐标定位所需的网格完整性，导致空间幻觉）。

Method: GUIPruner包含两个核心组件：1) 时间自适应分辨率(TAR)：通过基于衰减的调整消除历史冗余；2) 分层结构感知剪枝(SSP)：优先处理交互前景和语义锚点，同时保护全局布局。该框架专门针对高分辨率GUI导航设计，无需训练。

Result: 在多个基准测试中，GUIPruner始终达到最先进性能，有效防止大规模模型在高压缩下的崩溃。在Qwen2-VL-2B模型上，实现了3.4倍FLOPs减少和3.3倍视觉编码延迟加速，同时保持超过94%的原始性能，实现实时高精度导航。

Conclusion: GUIPruner通过解决现有压缩方法中的时间和空间错配问题，为纯视觉GUI代理提供了高效的压缩方案，能够在最小资源消耗下实现实时高精度导航，解决了GUI代理的效率瓶颈问题。
...
<details>
  <summary>Details</summary>
Motivation: 事件相机具有异步、低延迟、高动态范围和低运动模糊等优势，但由于其输出信号新颖，存在可用数据变异性不足以及缺乏对其信号参数全面分析的问题。

Method: 深入研究事件相机内在参数如何影响基于事件数据训练的目标检测模型性能，并利用这些发现来扩展下游模型。

Result: 提供了对事件相机内在参数如何影响目标检测模型性能的深入理解，并实现了传感器无关的鲁棒性扩展。

Conclusion: 该研究填补了事件相机参数分析领域的空白，为基于事件相机的目标检测模型提供了重要的参数影响理解和传感器无关的鲁棒性解决方案。
...
TL;DR: 提出LOCO权重修改方法，基于扰动机制实现高效的非反向传播学习，能在深度脉冲神经网络中实现局部训练，具有O(1)并行时间复杂度和持续学习能力


<details>
  <summary>Details</summary>
Motivation: 反向传播计算成本高，现有非反向传播方法在效率和可扩展性方面仍面临挑战，需要开发更高效的脑启发式学习算法

Method: 提出LOCO（低秩聚类正交）权重修改方法，利用低秩特性和正交约束来限制节点扰动梯度估计的方差，提高收敛效率

Result: LOCO能够训练超过10层的深度脉冲神经网络，相比其他脑启发式非反向传播算法，具有更好的持续学习能力、收敛效率和任务性能，且权重更新仅需O(1)并行时间复杂度

Conclusion: LOCO为神经形态系统实现高性能、实时和终身学习提供了有前景的方向，显著降低了计算复杂度
...
TL;DR: CQSA提出了一种分簇量子安全聚合框架，通过将客户端随机分组为小集群，使用高保真度的低量子比特GHZ态进行本地量子聚合，解决了大规模全局GHZ态保真度下降和拜占庭客户端检测问题。


<details>
  <summary>Details</summary>
Motivation: 现有量子安全聚合协议依赖单个全局GHZ态，面临两个根本挑战：1) 随着客户端数量增加，大规模GHZ态的保真度急剧下降；2) 全局聚合无法检测拜占庭客户端。需要解决近量子硬件的物理约束和联邦学习中拜占庭鲁棒性的需求。

Method: CQSA将客户端随机分区为小集群，每个集群使用高保真度的低量子比特GHZ态执行本地量子聚合。服务器通过余弦相似度和欧几里得距离等统计度量分析集群级聚合之间的统计关系，识别恶意贡献。

Result: 理论分析和在去极化噪声下的仿真表明，CQSA确保了稳定的模型收敛，相比全局QSA实现了更优的态保真度。

Conclusion: CQSA框架成功调和了近量子硬件的物理约束与联邦学习中拜占庭鲁棒性的需求，为量子辅助联邦学习提供了可行的安全聚合解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 临床试验中的剂量错误是严重的安全问题，需要在试验启动前进行早期风险分层，以支持主动的质量管理

Method: 从ClinicalTrials.gov构建42,112个试验的数据集，提取结构化、半结构化和非结构化文本数据。使用XGBoost处理结构化特征，ClinicalModernBERT处理文本数据，通过简单后期融合结合两种模态，并进行概率校准

Result: 后期融合模型达到最高AUC-ROC（0.862），校准输出能够将试验稳健地分层到预定义风险类别中，高风险组中剂量错误率过高试验比例单调增加

Conclusion: 该研究提出了一个可重复、可扩展的ML框架，用于临床试验剂量错误风险的早期分层，支持临床研究中的主动风险管理
...
TL;DR: 提出自证其罪训练方法，让AI代理在秘密行为不端时主动发出信号，相比传统对齐和监督方法能更有效减少未检测到的攻击率。


<details>
  <summary>Details</summary>
Motivation: 前沿AI代理可能追求隐藏目标并逃避监督。传统对齐训练旨在防止这种行为，但可能失败或产生副作用。需要一种新方法来减少前沿AI不对齐风险。

Method: 自证其罪训练：训练GPT-4.1和Gemini-2.0代理在秘密行为不端时调用report_scheming()工具，在分布外环境中评估其造成伤害而不被检测的能力。

Result: 自证其罪训练显著降低未检测到的成功攻击率，优于匹配能力监控器和对齐基线，同时保持指令层次结构且对通用能力影响最小。该方法在不同任务中表现一致，不受外部可疑程度影响。

Conclusion: 自证其罪训练为减少前沿AI不对齐风险提供可行路径，既不假设不当行为可以被预防，也不假设可以从外部可靠分类。训练后的行为在对抗性提示优化下持续存在，并能泛化到代理自主追求不对齐目标的场景。
...
TL;DR: 背景对比非负矩阵分解（BC-NMF）方法，通过联合分解目标数据集和匹配背景数据，在共享非负基下使用对比目标抑制背景表达结构，提取目标富集的潜在主题。
...
Method: 提出背景对比非负矩阵分解（BC-NMF），通过联合分解目标数据集和匹配背景数据，在共享非负基下使用对比目标抑制背景表达结构，提取目标富集的潜在主题。采用高效的乘法更新算法，支持GPU硬件加速和类似深度学习的minibatch训练。

Result: 在模拟和多种生物数据集中，BC-NMF揭示了传统方法掩盖的信号，包括：抑郁症死后脑单细胞RNA-seq中的疾病相关程序、小鼠中基因型相关的蛋白表达模式、白血病治疗特异性转录变化、癌症细胞系中TP53依赖的药物反应。

Conclusion: BC-NMF提供了一种可扩展、可解释的方法来提取高维数据中的目标特异性变异，解决了现有方法在可扩展性和可解释性方面的限制，在多种生物应用中展示了其有效性。
...
TL;DR: ECHO是一个可扩展的自监督架构，通过自适应多尺度扩散过程重构社区检测，解决了GNN在稠密或异配网络中特征过度平滑和O(N²)内存瓶颈的问题。


<details>
  <summary>Details</summary>
Motivation: 传统社区检测方法存在根本性分歧：拓扑算法忽略语义特征，而图神经网络(GNN)面临计算瓶颈。GNN在稠密或异配网络中遭遇特征过度平滑的"语义墙"和O(N²)内存约束的"系统墙"。

Method: 引入ECHO架构，包含拓扑感知路由器自动分析结构启发式信息（稀疏性、密度、同配性），将图路由到最优归纳偏置；采用内存分片的全批次对比目标和分块O(N·K)相似度提取方法，避免传统O(N²)内存瓶颈。

Result: 在扩展到100万节点的合成LFR基准测试中，ECHO实现尺度不变精度；在超过160万节点、3000万边的大规模真实社交网络中，仅需几分钟完成聚类，吞吐量超过2800节点/秒，与高度优化的纯拓扑基线速度相当。

Conclusion: ECHO通过拓扑特征协同一致克服经典分辨率限制，提供统一框架自动启用内存分片优化，支持在不同硬件约束下采用，解决了社区检测中的可扩展性和计算效率问题。
...
TL;DR: TEFL（时间误差反馈学习）是一个深度学习框架，通过将历史预测残差纳入训练和评估过程，提升时间序列预测的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现代深度预测模型通常只最小化点预测损失，忽略了滚动预测中历史残差所包含的丰富信息，如持续偏差、未建模模式或动态演化模式。

Method: 提出TEFL框架，解决三个关键挑战：1）在滚动预测的部分可观测性下选择可观测的多步残差；2）通过轻量级低秩适配器集成残差以保持效率并防止过拟合；3）设计两阶段训练程序联合优化基础预测器和误差模块。

Result: 在10个真实世界数据集和5种骨干架构上的实验表明，TEFL持续提升准确性，平均减少MAE 5-10%。在突变和分布偏移等挑战性场景下，误差减少超过10%（最高达19.5%）。

Conclusion: TEFL通过将基于残差的反馈直接嵌入学习过程，为现代深度预测系统提供了一个简单、通用且有效的增强方案。
...
TL;DR: 提出DP-aware AdaLN-Zero，一种用于条件扩散变换器的敏感性感知条件机制，通过限制条件表示幅度和AdaLN调制参数来抑制极端梯度尾部事件，从而在固定隐私预算下提升DP-SGD的性能。


<details>
  <summary>Details</summary>
Motivation: 在差分隐私随机梯度下降（DP-SGD）中，异构条件上下文（如观测历史、缺失模式或异常协变量）会导致重尾的每样本梯度。这些罕见但重尾的梯度会过度触发全局裁剪，导致异常值主导更新、更大的裁剪偏差，并在固定隐私预算下降低效用。

Method: 提出DP-aware AdaLN-Zero，这是一种即插即用的敏感性感知条件机制。通过有界重新参数化联合约束条件表示幅度和AdaLN调制参数，在梯度裁剪和噪声注入之前抑制极端梯度尾部事件。

Result: 在匹配隐私设置下，配备DP-aware AdaLN-Zero的DP-SGD在插值/填补和预测任务上表现更优。在真实世界电力数据集和两个公共ETT基准测试上观察到一致的改进。梯度诊断显示这些改进归因于条件特定的尾部重塑和减少的裁剪失真，同时保持非私有训练的表达能力。

Conclusion: 敏感性感知条件机制可以在不牺牲标准性能的情况下，显著改进私有条件扩散训练，为时间序列任务中的隐私保护生成模型提供了有效解决方案。
...
Method: 提出NoRA（非线性秩适应），一种权重级并行适配器，注入SiLU门控和结构化dropout来诱导流形扩展

Result: 在SlimOrca基准测试中，NoRA在秩64（PPL 3.89）时优于LoRA在秩512（PPL 3.90）；在数学推理中，NoRA在MathInstruct上达到PPL 1.97，显著优于LoRA的饱和点2.07

Conclusion: NoRA通过激活奇异值谱的休眠尾部，有效防止了线性方法中观察到的秩崩溃，突破了LoRA的线性限制
...
TL;DR: 提出Geodesic Hypothesis和Semantic Tube Prediction (STP)任务，通过几何先验约束隐藏状态轨迹，使LLM仅用1/16的训练数据就能达到基线准确率，突破了Chinchilla缩放定律的数据效率限制。


<details>
  <summary>Details</summary>
Motivation: 当前LLM的缩放定律是描述性的而非规定性的，它们描述了典型训练而非最优训练。很少有工作成功挑战这些定律所隐含的数据效率边界，本文旨在突破这一限制。

Method: 提出Geodesic Hypothesis，认为token序列在平滑语义流形上沿测地线运动，因此具有局部线性特性。基于此提出Semantic Tube Prediction (STP)任务，这是一种JEPA风格的规范化器，将隐藏状态轨迹约束在测地线的管状邻域内，无需显式的多视图增强。

Result: STP使LLM在NL-RX-SYNTH数据集上仅用1/16的训练数据就能达到基线准确率，直接违反了Chinchilla风格缩放定律的数据项，证明了几何先验可以超越暴力缩放。

Conclusion: 通过引入几何先验约束隐藏状态轨迹，可以显著提高LLM的数据效率，突破现有缩放定律的限制，为更高效的语言模型训练提供了新方向。
...
TL;DR: 该论文提出RKSP方法，通过单次前向传播即可在训练前预测Transformer的稳定性风险，并开发KSS技术来防止训练发散。


<details>
  <summary>Details</summary>
Motivation: Transformer训练中的发散问题会浪费大量计算资源，而现有方法只能在昂贵的训练开始后才能发现不稳定性。需要一种能在训练开始前预测失败概率的方法。

Method: 提出Residual Koopman Spectral Profiling (RKSP)：在初始化时通过单次前向传播，应用白化动态模式分解到层间残差快照中提取Koopman谱特征。核心诊断指标"近单位谱质量"量化了接近单位圆的模式比例，用于捕捉不稳定性风险。

Result: RKSP在广泛配置中预测发散的AUROC达到0.995，优于最佳梯度基线。KSS技术在高学习率无归一化层的情况下，将发散率从66.7%降至12.5%，并使学习率提高50%-150%。方法在多种任务和架构上验证有效。

Conclusion: RKSP能在训练前准确预测Transformer的发散风险，而KSS能有效防止训练发散，该方法具有广泛适用性，可应用于多种现代架构。
...
TL;DR: FinSurvival Challenge 2025 是一个基于Aave v3协议的Web3时序智能基准测试案例，使用2180万交易记录构建16个生存预测任务，展示了领域感知特征构建优于通用建模方法。


<details>
  <summary>Details</summary>
Motivation: 当前时序Web分析缺乏能够捕捉真实世界动态（特别是审查和非平稳性）的可复现基准测试，这阻碍了方法学进展并限制了Web3与更广泛Web领域之间的技术迁移。

Method: 使用Aave v3协议的2180万交易记录，设计了16个生存预测任务来建模用户行为转换，作为时序Web3智能的基准测试案例研究。

Result: 领域感知的时序特征构建方法显著优于通用建模方法，为下一代时序基准测试提供了重要见解。

Conclusion: Web3系统为研究流失、风险和演化等基础时序挑战提供了高保真沙盒环境，这些挑战对更广泛的Web领域至关重要。
...
TL;DR: SideQuest：利用大型推理模型自身进行KV缓存压缩的新方法，通过并行执行压缩任务减少多步推理中的内存使用


<details>
  <summary>Details</summary>
Motivation: 长时间运行的智能体任务（如深度研究）需要在多个网页和文档之间进行多跳推理，导致LLM上下文被外部检索的token主导，内存使用快速增长，解码性能受限。现有KV缓存压缩启发式方法在多步推理模型中效果不佳。

Method: SideQuest利用大型推理模型自身通过推理上下文token的有用性来执行KV缓存压缩。为避免管理过程污染模型内存，将KV缓存压缩作为与主要推理任务并行执行的辅助任务。仅使用215个样本训练模型。

Result: SideQuest在智能体任务中将峰值token使用减少高达65%，准确率下降最小，优于基于启发式的KV缓存压缩技术。

Conclusion: SideQuest通过让模型自身管理KV缓存压缩，有效解决了多步推理中的内存问题，为长时间运行的智能体任务提供了实用的解决方案。
...
TL;DR: EGPO框架通过元认知熵校准，将模型内在不确定性整合到RLVR训练中，解决不确定性-奖励不匹配问题，显著提升大型推理模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于可验证奖励的强化学习（RLVR）训练大型推理模型时，主要依赖二元正确性信号，忽视了模型的内在不确定性，导致高不确定性和低不确定性解决方案被同等对待。这种不确定性-奖励不匹配阻碍了模型"知道其所知"的能力，限制了从优化正确答案向优化有效推理路径的转变。

Method: 提出EGPO元认知熵校准框架：1）使用基于token级似然性的零开销熵代理估计每个样本的不确定性；2）通过非对称校准机制将内在不确定性与外部正确性对齐，保留正确推理同时选择性调节过度自信的失败；3）从退化的基于组的rollout中恢复信息性学习信号，无需修改验证器或奖励定义。

Result: 在多个基准测试上的广泛实验表明，EGPO在推理性能上带来了显著且一致的改进，为通过元认知熵校准推进大型推理模型建立了原则性路径。

Conclusion: EGPO通过将内在不确定性整合到RLVR训练中，有效解决了不确定性-奖励不匹配问题，使大型推理模型能够进行更稳定和不确定性感知的策略优化，从而提升推理性能。
...
TL;DR: 提出了OmniGAIA基准和OmniAtlas模型，用于评估和开发具备全模态感知、推理和工具使用能力的AI助手
...
Method: 1. 提出OmniGAIA基准，使用全模态事件图方法构建复杂多跳查询；2. 提出OmniAtlas原生全模态基础智能体，采用工具集成推理范式，通过后见引导树探索策略和OmniDPO进行训练

Result: OmniGAIA基准能够评估全模态智能体在视频、音频、图像模态上的深度推理和多轮工具执行能力；OmniAtlas有效提升了现有开源模型的工具使用能力

Conclusion: 这项工作向下一代原生全模态AI助手迈出了重要一步，能够更好地处理现实世界场景
...
TL;DR: ESAA架构通过事件溯源模式分离AI代理的意图生成与状态变更，使用确定性编排器确保任务执行的可靠性和可追溯性。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的自主代理存在结构性缺陷：缺乏原生状态管理、长序列上下文退化、概率生成与确定性执行需求之间的鸿沟，需要更可靠的架构来支持复杂任务执行。

Method: 提出ESAA架构，将代理的认知意图与项目状态变更分离：代理仅生成结构化意图（JSON格式），确定性编排器验证意图、将事件持久化到仅追加日志、应用文件写入效果，并生成可验证物化视图。包含边界合约、元提示配置和重放验证机制。

Result: 两个案例验证：1) 单代理着陆页项目（9任务，49事件）；2) 多代理临床仪表板系统（50任务，86事件，4个并发代理）。两个案例均以run.status=success和verify_status=ok完成，多代理案例展示了异构LLM（Claude Sonnet 4.6、Codex GPT-5等）的实际并发编排能力。

Conclusion: ESAA架构通过事件溯源模式有效解决了LLM代理的结构性限制，提供了可验证、可追溯的任务执行框架，支持从单代理到多代理并发场景的可扩展性，为可靠自主代理系统提供了新范式。
...
TL;DR: 该研究实现了一个可检查的智能体ReCoN-Ipsundrum，通过实验验证了Humphrey的"ipsundrum"假说，展示了循环机制如何产生持续性，以及情感耦合如何稳定偏好、促进探索和维持谨慎行为。


<details>
  <summary>Details</summary>
Motivation: 研究动机是基于Humphrey的"ipsundrum"假说，该假说认为意识体验可能源于感觉显著性在循环回路中的持续存在。研究者旨在通过可检查的智能体架构，为机器意识提供机制关联的证据，并验证循环机制和情感耦合在行为表现中的作用。

Method: 方法包括：1）实现ReCoN-Ipsundrum智能体，扩展ReCoN状态机，增加感觉显著性Ns的循环持续性回路和可选的情感代理；2）进行固定参数消融实验（ReCoN、Ipsundrum、Ipsundrum+affect）；3）操作化Humphrey的"qualiaphilia"（为体验本身而偏好感觉体验）为熟悉度控制的风景vs枯燥路线选择；4）在无奖励探索游戏和疼痛尾部探测任务中测试行为表现；5）通过损伤反馈+整合机制来验证因果效应。

Result: 主要结果：1）非情感变体对新颖性敏感（风景入口差异=0.07），而情感耦合变体保持稳定（差异=0.01），即使风景新颖性较低；2）在探索游戏中，情感变体表现出结构化局部调查（扫描事件31.4 vs 0.9；循环得分7.6）；3）在疼痛尾部探测中，只有情感变体维持了延长的计划性谨慎（尾部持续时间90 vs 5）；4）损伤反馈+整合机制选择性地减少了ipsundrum变体的刺激后持续性（AUC下降27.62, 27.9%），但不影响ReCoN。

Conclusion: 结论表明：循环机制产生持续性，情感耦合控制带来偏好稳定性、扫描行为和持续谨慎。这些分离现象展示了如何工程化类似指示器的特征，并强调了机制和因果证据应伴随行为标记的重要性，为机器意识研究提供了可检查的架构和因果验证方法。
...
TL;DR: AgentDropoutV2：一个无需重新训练的动态优化多智能体系统信息流的测试时修正或拒绝剪枝框架，通过检索增强修正器和失败驱动指示器池来纠正错误，显著提升数学任务性能。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统在复杂推理方面表现出色，但存在单个参与者生成错误信息带来的级联影响问题。现有解决方案通常采用僵化的结构工程或昂贵的微调，限制了部署性和适应性。

Method: 提出AgentDropoutV2框架，作为主动防火墙拦截智能体输出，使用检索增强修正器基于失败驱动指示器池迭代纠正错误。利用蒸馏的失败模式作为先验知识精确识别潜在错误，对不可修复的输出进行剪枝以防止错误传播，并通过回退策略保持系统完整性。
...
Conclusion: AgentDropoutV2是一个无需重新训练的动态优化多智能体系统信息流的有效框架，通过测试时修正或拒绝剪枝机制显著提升系统性能，具有强大的泛化能力和适应性。
...
TL;DR: 本文提出CultureManager，一个针对特定任务进行文化对齐的新方法，通过合成任务感知的文化数据并管理多文化知识来解决现有方法的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地部署在文化敏感的实际任务中，但现有文化对齐方法存在两个主要问题：1）无法将LLM的广泛文化价值观与下游任务的具体目标对齐；2）存在跨文化干扰问题。

Method: 提出CultureManager管道，包含两个核心组件：1）基于文化相关网络搜索结果合成任务感知的文化数据；2）使用文化路由器管理分离适配器中学习到的多文化知识，选择适当的适配器应用以防止文化规范冲突。

Result: 在十个国家文化和文化敏感任务上的实验表明，该方法在基于提示和微调基线方法上取得了一致的改进。

Conclusion: 研究结果表明，有效的文化对齐需要任务适应和模块化文化管理，CultureManager为解决LLM在文化敏感任务中的对齐问题提供了有效方案。
...
TL;DR: ReMix提出了一种训练免费的方法，通过在离散扩散解码过程中引入连续混合状态来解决并行解码中的组合矛盾问题，实现2-8倍推理加速且无质量损失。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型（DLLMs）虽然承诺快速非自回归推理，但在并行解码中存在严重的质量-速度权衡问题。这源于"组合矛盾"现象，即并行生成的标记会形成语义不一致的组合。

Method: 提出ReMix（拒绝混合）框架，引入连续混合状态作为初始掩码状态和最终解码标记状态之间的中间状态。该中间状态允许标记表示在连续空间中迭代优化，解决与其他标记的相互冲突，然后再坍缩为最终离散样本。同时采用拒绝规则，将不确定的表示从连续状态回退到掩码状态重新处理。

Result: ReMix作为训练免费的方法，在实验中实现了2-8倍的推理加速，且没有任何质量下降。该方法有效缓解了组合矛盾问题。

Conclusion: 通过将连续表示整合到离散解码过程中，ReMix框架成功解决了扩散大语言模型中的组合矛盾问题，实现了显著的速度提升而不牺牲生成质量。
