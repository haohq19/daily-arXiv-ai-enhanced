...
Motivation: 在接触性运动中及早识别危险动作能够及时干预并提高运动员安全。美式足球中的危险擒抱动作检测对于预防伤害至关重要，但现有数据集较小且检测方法有待改进。

Method: 使用视觉Transformer模型，结合处理类别不平衡的训练策略，在扩展的数据集（733个单运动员-假人擒抱片段）上进行危险擒抱检测，每个片段都标注了标准擒抱技术评估的撞击区域组件。

Result: 在交叉验证中获得0.67的危险召回率和0.59的危险F1分数。相比先前在较小数据集上的基线（危险召回率0.58，危险F1 0.56），危险召回率提升了超过8个百分点。
...
TL;DR: GRAZE：无需训练、无需标注的橄榄球训练视频中首次接触点（FPOC）定位方法，通过目标检测、运动感知时序推理和像素级接触验证实现


<details>
  <summary>Details</summary>
Motivation: 橄榄球训练视频量大但关键交互时间短暂，需要可靠的时空定位来识别交互实体和接触起始点，用于生物力学分析。现有方法面临相机运动、杂乱场景、多运动员和快速姿态变化等挑战。

Method: GRAZE训练免费流水线：1) 使用Grounding DINO发现候选球员-假人交互；2) 运动感知时序推理进行精炼；3) 使用SAM2作为像素级接触验证器，不依赖检测置信度。候选发现与接触确认分离提高鲁棒性。

Result: 在738个擒抱训练视频上，GRAZE在97.4%的片段中产生有效输出，77.5%的片段中FPOC定位误差在±10帧内，82.7%的片段中误差在±20帧内。

Conclusion: 无需任务特定训练即可在真实训练视频中实现帧级准确的接触起始定位是可行的，GRAZE为生物力学分析提供了实用解决方案。
...
TL;DR: 该论文提出了一种结构化奖励机制来改善多模态大语言模型在视频理解中的视觉和时间基础问题，通过分解事实和时间单元来增强模型的忠实性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视频理解中虽然取得了显著进展，但经常存在视觉和时间基础不准确的问题，如虚构对象存在、分配错误属性或压缩重复事件等。标准句子级监督无法有效检测这些基础错误，而强化学习中通用的句子级奖励也难以准确定位具体的接地失败。

Method: 提出了一种结构化奖励机制，包含三个互补组件：(1) 实例感知的场景图奖励，用于事实对象、属性和关系；(2) 时间奖励，用于事件排序和重复；(3) 视频接地的视觉问答奖励，用于分层自验证。通过组合一致性审计分解字幕为事实和时间声明，验证高层预测是否得到有效底层证据支持。

Result: 在时间基准、通用视频理解和幻觉导向基准测试中，该结构化奖励方法在开源骨干模型上取得了一致的性能提升，表明结构化奖励塑造是实现更忠实视频理解的有效途径。

Conclusion: 结构化奖励机制能够有效改善多模态大语言模型在视频理解中的视觉和时间基础问题，通过细粒度的奖励组件提供更精确的监督信号，是实现更忠实视频理解的实用途径。
...
### [4] [DynaVid: Learning to Generate Highly Dynamic Videos using Synthetic Motion Data](https://arxiv.org/abs/2604.01666)
...
<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在处理高度动态运动或需要精细运动控制时表现不佳，主要原因是训练数据中缺乏此类示例

Method: 提出DynaVid框架：1）使用计算机图形学渲染的合成光流数据作为训练数据；2）采用两阶段生成框架：运动生成器先合成运动（光流），然后运动引导的视频生成器基于运动生成视频帧

Result: 在剧烈人体运动生成和极端相机运动控制两个挑战性场景中，DynaVid显著提升了动态运动生成的真实性和可控性

Conclusion: 通过合成光流数据训练，DynaVid能够从合成数据学习动态运动模式，同时从真实世界视频保持视觉真实感，解决了现有数据集不足的问题
...
### [5] [CompassAD: Intent-Driven 3D Affordance Grounding in Functionally Competing Objects](https://arxiv.org/abs/2604.02060)
...
<details>
  <summary>Details</summary>
Motivation: 现有3D affordance方法主要评估孤立单物体，且常依赖显式类别名称查询，无法处理真实场景中多个物体共享相同功能但只有特定物体适合给定任务上下文的情况（混淆对问题）
...
Result: 在CompassAD基准上取得最先进结果，在可见和未见查询上均表现优异，在机器人操作器上的部署验证了在真实世界混淆多物体场景中有效抓取的迁移能力

Conclusion: 形式化了Multi-Object Affordance Grounding新任务，构建了首个关注隐式意图的混淆多物体场景基准CompassAD，提出的CompassNet框架通过ICI和BCR模块有效解决了混淆物体选择问题，展示了向真实机器人应用的可行性
...
<details>
  <summary>Details</summary>
Motivation: 解决无人机在GNSS拒止环境中的跨视角地理定位挑战，特别是斜视无人机图像与正交卫星地图之间的严重几何差异问题。现有方法将透视畸变视为外观噪声而非显式几何变换，采用解耦的检索-姿态估计流程存在局限。

Method: 提出几何感知框架：1) 使用视觉几何基础Transformer从多视角无人机图像序列重建局部3D场景；2) 渲染虚拟鸟瞰图表示，将无人机视角正交校正以对齐卫星图像；3) 引入卫星注意力块隔离每个候选位置与重建场景的交互，避免候选间干扰；4) 发布重新校准的University-1652数据集，包含精确坐标标注和空间重叠分析。

Result: 在重新校准的University-1652基准和SUES-200数据集上的广泛实验表明，该方法显著优于现有最先进基线，实现鲁棒的米级定位精度，在复杂城市环境中具有更好的泛化能力。

Conclusion: 通过显式建模3D场景几何，将粗粒度位置识别和细粒度姿态估计统一到单个推理流程中，解决了跨视角地理定位中的几何差异问题，为GNSS拒止环境中的无人机定位提供了有效解决方案。
...
TL;DR: EventHub是一个无需地面真实标注的事件立体视觉训练框架，利用标准彩色图像生成代理标注和代理事件，将RGB立体模型迁移到事件数据，实现前所未有的泛化能力


<details>
  <summary>Details</summary>
Motivation: 传统事件立体视觉网络需要来自昂贵主动传感器的地面真实标注，成本高昂。本文旨在开发无需地面真实标注的深度事件立体网络训练方法

Method: 提出EventHub框架：1）从标准彩色图像通过最先进的新视角合成技术生成代理标注和代理事件；2）当图像已与事件数据配对时仅生成代理标注；3）利用生成的数据集重新利用RGB立体模型处理事件数据

Result: 在广泛使用的事件立体数据集上的实验支持EventHub的有效性，展示了前所未有的泛化能力。相同的数据蒸馏机制还能提高RGB立体基础模型在夜间场景等挑战性条件下的准确性

Conclusion: EventHub为事件立体视觉提供了一种无需地面真实标注的有效训练框架，成功将RGB立体模型迁移到事件数据，并展示了在挑战性条件下的性能提升
...
TL;DR: DySCo框架通过动态语义压缩解决长时间序列预测中的噪声和计算冗余问题，使用熵引导采样、层次频率分解和跨尺度交互混合器来提升主流模型的长期相关性捕捉能力。


<details>
  <summary>Details</summary>
Motivation: 长时间序列预测中，延长回看窗口理论上能提供更丰富的历史上下文，但实际上会引入无关噪声和计算冗余，阻碍模型有效捕捉复杂的长期依赖关系。

Method: 提出动态语义压缩(DySCo)框架：1) 熵引导动态采样(EGDS)机制自主识别并保留高熵片段，压缩冗余趋势；2) 层次频率增强分解(HFED)策略分离高频异常和低频模式；3) 跨尺度交互混合器(CSIM)动态融合全局上下文与局部表示。

Result: 实验结果表明DySCo作为通用即插即用模块，显著增强了主流模型捕捉长期相关性的能力，同时降低了计算成本。

Conclusion: DySCo框架有效解决了长时间序列预测中的噪声和冗余问题，通过动态语义压缩机制提升了模型的长期依赖捕捉能力，具有通用性和实用性。
...
TL;DR: 提出端到端框架训练LLM进行供应链中断概率预测，显著优于GPT-5等基线模型


<details>
  <summary>Details</summary>
Motivation: 供应链中断预测面临低频高影响事件、噪声和非结构化数据的挑战，通用模型需要任务特定适应

Method: 引入端到端框架，使用实际中断结果作为监督训练LLM产生校准的概率预测

Result: 模型在准确性、校准性和精确度上大幅优于GPT-5等强基线，训练诱导出更结构化可靠的概率推理
...
TL;DR: 提出了一种基于并行前缀扫描的线性指数Hawkes过程最大似然估计算法，将计算复杂度从O(N²)降低到约O(N/P)，实现大规模并行化


<details>
  <summary>Details</summary>
Motivation: 传统多元Hawkes过程的最大似然估计计算复杂度为O(N²)，线性指数Hawkes过程虽有O(N)递推但无法利用现代GPU并行化，需要开发能充分利用并行计算的高效算法

Method: 将Hawkes过程强度表示为稀疏转移矩阵的乘积，通过线性时间关联乘法实现并行前缀扫描计算，开发了简单且大规模并行化的最大似然估计算法

Result: 在模拟和真实数据集上实现了数量级的速度提升，可扩展到数千个节点和数千万个事件，远超先前工作的规模，并提供了开源的PyTorch库实现

Conclusion: 提出的并行前缀扫描算法为线性指数Hawkes过程提供了精确、高效且可扩展的最大似然估计方法，在保持模型简单性和可解释性的同时显著提升了计算性能
...
<details>
  <summary>Details</summary>
Motivation: 自适应逆强化学习需要从观察到的梯度中重建前向学习者的损失函数，但所需的反事实梯度条件概率为零，传统蒙特卡洛估计效率低下，核平滑收敛慢

Method: 使用Malliavin微积分将反事实条件化为无条件期望的比值，推导Langevin结构的Malliavin导数及其伴随Skorohod积分公式，提供具体算法实现反事实梯度估计

Result: 通过Malliavin方法克服了反事实梯度估计的困难，恢复了标准估计速率，比传统方法更高效

Conclusion: 提出的基于Malliavin微积分的被动Langevin算法为自适应逆强化学习提供了有效的反事实梯度估计方法，解决了传统方法的效率问题
...
TL;DR: 提出MATA-Former模型，利用医学语义动态参数化注意力权重，结合PSL方法将二元分类转化为连续多时间范围回归，在临床风险预测中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前临床风险预测方法存在两个主要问题：1) 依赖粗糙的二元监督信号，无法捕捉风险演变的连续性；2) 过度依赖物理时间戳而非病理依赖关系。需要开发能更好对齐临床逻辑的预测模型。

Method: 提出MATA-Former（医学语义感知时间ALiBi Transformer），利用事件语义动态参数化注意力权重，优先考虑因果有效性而非时间间隔。同时提出PSL（Plateau-Gaussian软标签）方法，将二元分类重新表述为连续多时间范围回归，实现全轨迹风险建模。

Result: 在SIICU（新构建的数据集，包含50.6万+事件，具有专家验证的细粒度标注）和MIMIC-IV数据集上评估，该框架在捕捉文本密集型、不规则临床时间序列风险方面表现出优越效果和强大泛化能力。

Conclusion: 通过将医学语义融入注意力机制和采用连续风险建模方法，MATA-Former能够更好地捕捉临床风险的演变规律，为临床决策提供更准确的预测支持。
...
TL;DR: DDCL提出首个完全可微的端到端无监督原型表示学习框架，用内部双竞争层替代外部k-means，实现从特征提取到聚类分配的完整可微训练。


<details>
  <summary>Details</summary>
Motivation: 深度聚类中存在特征学习与聚类分配的结构性脱节问题，大多数方法依赖外部聚类步骤（如k-means）生成伪标签，阻碍了网络直接优化聚类质量。

Method: 引入双竞争层（DCL）作为网络原生可微输出生成原型，通过单一统一损失实现从特征提取到软聚类分配的端到端反向传播训练，无需外部聚类步骤。

Result: 理论推导显示软量化损失可分解为单纯形约束重构误差和非负加权原型方差项，实验验证分解恒等式零违反，负反馈循环Pearson相关系数-0.98，聚类准确率比不可微消融提升65%，比DeepCluster提升122%。

Conclusion: DDCL通过架构创新解决了深度聚类中特征学习与聚类分配的脱节问题，实现了完全可微的端到端训练，理论分析揭示了损失几何中的自调节机制，实验验证了其优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型容易产生幻觉，主要源于弱编码的现有知识。需要一种方法让模型自主识别知识缺口，避免外部监督，同时防止相邻知识被污染。

Method: 1) 检测异常高损失段落；2) 生成问答链让模型自我验证知识缺口；3) 基于验证深度k调整AdamW的β₂参数：β₂ = 0.999·r^k；4) 整个学习强度由单个参数r控制。

Result: 标准微调产生死记硬背（扰动间隙11.6±0.2倍基线），而所有LSCP条件学习语义（2.7-3.0倍）。r=1.0条件证明训练数据格式（而非β₂门控）是防止记忆的主要机制；门控保护相邻知识免受污染（r=0.98时相邻问题准确率93±7% vs 90%基线）。

Conclusion: LSCP框架实现了自主知识获取，模拟生物记忆巩固过程，将上下文中的临时信息选择性地巩固到参数权重中。该框架自熄灭，随着模型学习逐渐收敛到标准AdamW。
...
TL;DR: Ouroboros 提出了一种带控制器的递归Transformer系统，通过超网络生成输入相关的调制向量，使递归块在不同深度执行不同操作，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统递归Transformer在多个深度步骤中重用相同的权重块，导致每个步骤应用相同的变换，无法在不同深度组合不同的操作，限制了模型的表达能力。

Method: 1. 将Controller超网络附加到递归Transformer块；2. Controller观察当前隐藏状态，生成每步对角调制向量；3. 将调制应用于冻结的SVD初始化LoRA基；4. 结合门控递归（偏置初始化为88%保留率）和每步LayerNorm实现稳定深度迭代。

Result: 在Qwen2.5-3B的17层架构上，Ouroboros相比未修改的17层基线减少43.4%训练损失，恢复了51.3%因层移除导致的性能差距。仅增加9.2M可训练参数，在所有测试深度和秩上都优于同等大小的静态每步LoRA。门控递归是必要的，没有它递归层应用会使模型变差。

Conclusion: Ouroboros通过输入相关的递归变换显著提升了递归Transformer的性能，但改进目前仅限于训练分布，在未见文本上尚未超越基线，这归因于冻结的下游层，是需要进一步研究的方向。
...
TL;DR: PGPO提出了一种细粒度的信用分配框架，通过量化视觉依赖并动态调整token级优势，解决了多模态推理中视觉信号被稀释的问题，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于可验证奖励的强化学习框架存在方法缺陷：将所有生成token分配相同优势值，稀释了多模态推理中关键视觉步骤的学习信号，导致视觉依赖的token得不到有效优化。

Method: 提出Token Visual Dependency概念，通过KL散度量化视觉输入的信息增益；设计Perception-Grounded Policy Optimization框架，使用阈值门控的质量守恒机制动态重塑token级优势，放大视觉依赖token的学习信号，抑制语言先验的梯度噪声。

Result: 在Qwen2.5-VL系列模型和7个挑战性多模态推理基准上的实验表明，PGPO平均提升模型性能18.7%，有效降低梯度方差，防止训练崩溃，并作为强大的正则化器。

Conclusion: PGPO通过细粒度的token级信用分配，解决了多模态强化学习中视觉信号稀释问题，实现了更稳健、感知基础的多模态推理，为视觉语言模型的强化学习训练提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 需要一种能够在特定上下文中进行推理的逻辑框架，能够表达复杂的知识关系（如"每个人都知道Alice说的一切"），同时支持上下文内的矛盾（paraconsistent逻辑），并与现有的一阶逻辑定理证明器兼容。

Method: 基于一阶逻辑构建Qiana框架，使其可有限公理化，支持对公式和上下文的量化，允许在上下文中使用paraconsistent逻辑处理矛盾，并设计不同的框架变体。

Result: 展示了Qiana能够表示时间性、事件演算和模态逻辑，证明了其表达能力，并讨论了不同的设计替代方案。

Conclusion: Qiana是一个强大而灵活的逻辑框架，能够在特定上下文中进行推理，支持复杂的量化表达和矛盾处理，并与现有的一阶逻辑工具兼容，具有广泛的应用潜力。
...
TL;DR: LLM智能体在多用户共享知识层时，会出现无意识的跨用户污染问题，导致一个用户的有效信息被错误地应用于其他用户，造成静默错误。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体在重复会话中运行并维护任务状态，单个智能体服务于团队或组织内的多个用户，共享知识层。这种共享持久性扩大了故障面：对某个用户本地有效的信息，当智能体不考虑范围而重新应用时，会静默地降低其他用户的结果质量。

Method: 通过受控评估协议形式化UCC问题，引入三种污染类型的分类法，并在两种共享状态机制中评估该问题。评估了原始共享状态和写入时净化两种机制的效果。

Result: 在原始共享状态下，仅良性交互就会产生57-71%的污染率。写入时净化在共享状态为对话式时有效，但当共享状态包含可执行工件时，仍存在大量残余风险，污染通常表现为静默错误答案。

Conclusion: 共享状态智能体需要超越文本级净化的工件级防御措施，以防止静默的跨用户故障。仅靠对话层面的净化不足以应对包含可执行工件的复杂场景。
...
TL;DR: 论文提出稳定性过滤方法，通过筛选行为稳定的边界来改进推理行为的转向向量构建，显著提升数学推理性能


<details>
  <summary>Details</summary>
Motivation: 现有方法通过关键词匹配检测推理行为边界，但大多数边界行为不稳定，无法在相同前缀下重现，这稀释了转向信号的有效性

Method: 提出稳定性过滤方法，只保留模型能一致重现目标行为的边界；结合内容子空间投影去除问题特定噪声；构建概率模型形式化推理行为

Result: 在MATH-500上达到0.784准确率（比最强基线提升5.0%）；转向向量可在同架构模型间迁移，提升Nemotron-Research-Reasoning-1.5B（+5.0）和DeepScaleR-1.5B-Preview（+6.0）性能

Conclusion: 稳定性过滤能有效识别真正的行为信号，构建更鲁棒的转向向量，显著提升推理性能并支持跨模型迁移
...
Method: 1) 提出分布式线性相对定位算法与可定位性充分条件；2) 将MAP问题重构为流形上的加权总体最小二乘优化；3) 结合神经密度估计器处理先验概率未知问题；4) 设计边缘化机制控制计算成本

Result: 开发了系统性的3D相对定位框架，能够以纯线性方式确定邻居的相对位置和方向，并通过改进的MAP估计器有效处理测量噪声

Conclusion: 该框架为噪声环境下的机器人相对定位提供了系统解决方案，通过线性理论、WTLS优化、NDE结合和边缘化机制解决了MAP估计中的三个主要挑战
...
<details>
  <summary>Details</summary>
Motivation: 虽然表达性生成模型能够捕捉复杂多模态的动作分布，但通过强化学习微调这些策略仍然面临不稳定和样本效率低的问题。

Method: POCO将策略改进形式化为时间动作块的后验推断问题，通过期望最大化过程将奖励加权的隐式后验蒸馏到策略中，无需似然估计。采用离线到在线范式，将在线探索锚定到预训练先验，模型无关设计可扩展微调大型VLA模型。

Result: 在7个模拟基准和4个接触丰富的真实世界任务中，POCO防止了灾难性策略崩溃，超越了最先进的基线方法，在真实世界任务中实现了96.7%的成功率。

Conclusion: POCO提供了一个原则性的强化学习框架，能够有效微调表达性生成模型策略，在模拟和真实世界任务中都表现出优越性能。
