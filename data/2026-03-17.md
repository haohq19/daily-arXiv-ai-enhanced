...
### [36] [Bi-CamoDiffusion: A Boundary-informed Diffusion Approach for Camouflaged Object Detection](https://arxiv.org/abs/2603.13357)
*Patricia L. Suarez,Leo Thomas Ramos,Angel D. Sappa*

Main category: cs.CV

TL;DR: Bi-CamoDiffusion 是一种改进的伪装目标检测方法，通过边缘先验注入和统一优化目标，在多个基准测试中超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的伪装目标检测方法在处理细长结构和边界过渡时存在困难，容易出现结构模糊和误检问题，需要更精确的边界恢复和对象-背景分离能力。

Method: 基于 CamoDiffusion 框架改进，通过无参数注入过程将边缘先验整合到早期嵌入中，采用统一优化目标平衡空间精度、结构约束和不确定性监督。

Result: 在 CAMO、COD10K 和 NC4K 基准测试中超越了基线方法，在 $S_m$、$F_β^{w}$、$E_m$ 和 $MAE$ 等所有评估指标上都优于现有最先进方法。

Conclusion: Bi-CamoDiffusion 通过边缘先验注入和统一优化目标，显著提升了伪装目标检测的边界锐度和结构准确性，实现了更精确的对象-背景分离。
...
### [37] [Comparative Analysis of Deep Learning Architectures for Multi-Disease Classification of Single-Label Chest X-rays](https://arxiv.org/abs/2603.13392)
*Ali M. Bahram,Saman Muhammad Omer,Hardi M. Mohammed*

Main category: cs.CV

TL;DR: 本研究系统比较了7种深度学习架构在胸部X光多疾病分类中的表现，发现所有模型均超过90%准确率，其中ConvNeXt-Tiny性能最佳，MobileNetV2参数效率最高。


<details>
  <summary>Details</summary>
Motivation: 胸部X光成像作为全球肺部和心脏疾病的主要诊断工具，其准确性受到放射科医生短缺和观察者间变异性的限制。本研究旨在评估不同深度学习架构在胸部疾病分类中的表现，为AI辅助诊断提供参考。

Method: 构建了包含18,080张胸部X光图像的平衡数据集，涵盖5种疾病类别。系统比较了7种深度学习架构（ConvNeXt-Tiny、DenseNet121、DenseNet201、ResNet50、ViT-B/16、EfficientNetV2-M、MobileNetV2），所有模型在相同条件下使用ImageNet预训练权重、标准化预处理和一致超参数进行训练。

Result: 所有7种架构测试准确率均超过90%。ConvNeXt-Tiny表现最佳（92.31%准确率，95.70% AUROC），MobileNetV2参数效率最高（350万参数，90.42%准确率，94.10% AUROC）。结核病和COVID-19分类在所有架构中接近完美（AUROC≥99.97%），而正常、心脏肥大和肺炎分类更具挑战性。Grad-CAM可视化确认了临床一致的注意力模式。

Conclusion: 研究表明，无需过多计算资源即可实现高精度的胸部X光多疾病分类，这对资源丰富和资源受限的医疗环境中AI辅助诊断具有重要意义。
...
TL;DR: EVD框架通过事件驱动的视频生成，减少文本到视频模型中交互幻觉问题，提升动态交互的准确性


<details>
  <summary>Details</summary>
Motivation: 当前文本到视频模型在帧级别看起来真实，但在简单交互上失败：运动在接触前开始、动作未实现、物体放置后漂移、支撑关系断裂。这源于帧优先去噪方法，缺乏明确的交互活动概念

Method: 引入事件驱动视频生成(EVD)框架：轻量级事件头预测token对齐的事件活动，事件接地损失在训练中将活动与状态变化耦合，事件门控采样（带滞后和早期步骤调度）抑制虚假更新，在交互期间集中更新

Result: 在EVD-Bench上，EVD持续改善人类偏好和VBench动态，显著减少状态持久性、空间准确性、支撑关系和接触稳定性方面的失败模式，同时不牺牲外观质量

Conclusion: 明确的事件接地是减少视频生成中交互幻觉的实用抽象方法，为改进文本到视频模型的交互准确性提供了有效框架
...
<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是导致可预防性失明的主要原因，自动眼底图像分级在大规模筛查中具有重要作用。本研究旨在探索基于CLIP的不同方法在五级DR严重程度分级中的性能。

Method: 研究了三种CLIP-based方法：(1) 使用提示工程的零样本基线；(2) 结合CBAM注意力的混合FCN-CLIP模型；(3) 编码DR进展顺序结构的排序感知提示模型。在APTOS 2019和Messidor-2组合数据集（n=5,406）上进行训练评估，通过重采样和类别特定最优阈值处理解决类别不平衡问题。

Result: 排序感知模型获得最高整体准确率（93.42%，AUROC 0.9845）和临床关键严重病例的高召回率；混合FCN-CLIP模型（92.49%，AUROC 0.99）在检测增殖性DR方面表现优异；两者均显著优于零样本基线（55.17%，AUROC 0.75）。

Conclusion: 排序感知提示模型在整体性能上最优，混合FCN-CLIP模型在特定严重病例检测上具有优势，两种方法均显示出在筛查实践中的互补价值，为大规模DR筛查提供了有效的自动化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 交互式长视频生成需要在提示切换时保持感知保真度和连贯运动，但现有流式方法存在渐进质量退化和运动动态减弱的问题，特别是在提示切换时无法同时保留语义上下文和潜在线索，且无边界时间索引导致位置分布偏移。

Method: 提出Anchor Forcing框架：1）锚点引导的重新缓存机制，将KV状态存储在锚点缓存中，在每次提示切换时从这些锚点热启动重新缓存；2）三区域RoPE设计，具有区域特定的参考原点，配合RoPE重新对齐蒸馏，协调无边界流式索引与预训练RoPE机制。

Result: 实验表明，在长视频生成中，该方法在交互设置下比先前的流式基线提高了感知质量和运动指标。

Conclusion: Anchor Forcing框架通过创新的缓存管理和RoPE对齐策略，有效解决了交互式长视频生成中的关键挑战，实现了更好的感知质量和运动连贯性。
...
TL;DR: PHARL：一种无需临床标签的物理感知对齐表示学习方法，通过轨迹级时间一致性和多类物理对齐约束，学习具有物理意义的跌倒表示，在四个公开数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: 基于视觉的跌倒分析面临关键瓶颈：视觉相似的动作可能对应完全不同的物理结果，因为接触力学和保护性反应的微小差异难以从外观推断。现有方法依赖监督式损伤预测，但损伤标签难以获取（视频证据模糊、真实损伤事件罕见且无法安全模拟），导致监督噪声。

Method: PHARL（物理感知对齐表示学习）通过两种互补约束正则化运动嵌入：1）轨迹级时间一致性用于稳定表示学习；2）多类物理对齐，通过仿真导出的接触结果塑造嵌入几何。通过将视频窗口与时间对齐的仿真描述符配对，PHARL捕获局部冲击相关动态，同时保持纯前向推理。

Result: 在四个公开数据集上的实验表明，PHARL持续提升风险对齐表示质量，优于纯视觉基线，同时保持强大的跌倒检测性能。PHARL还表现出零样本有序性：在没有显式有序监督的情况下，出现了可解释的严重性结构（头部>躯干>支撑）。

Conclusion: PHARL通过学习物理有意义的跌倒表示，无需临床结果标签，解决了视觉相似动作对应不同物理结果的问题。该方法通过物理对齐约束实现了更好的风险表示，并展现出零样本有序性，为跌倒分析提供了更可靠的解决方案。
...
### [42] [LADR: Locality-Aware Dynamic Rescue for Efficient Text-to-Image Generation with Diffusion Large Language Models](https://arxiv.org/abs/2603.13450)
*Chenglin Wang,Yucheng Zhou,Shawn Chen,Tao Wang,Kai Zhang*

Main category: cs.CV

TL;DR: LADR是一种无需训练的方法，通过利用图像的空间马尔可夫特性加速离散扩散语言模型的推理，实现约4倍加速，同时保持甚至提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 离散扩散语言模型在统一多模态生成方面表现出色，但其迭代解码导致推理延迟高。现有加速方法需要昂贵重训练或未能充分利用视觉数据的2D空间冗余性。

Method: 提出LADR方法：1) 形态学邻居识别定位候选token；2) 风险有界过滤机制防止错误传播；3) 流形一致逆调度使扩散轨迹与加速掩码密度对齐。该方法优先恢复"生成前沿"区域（与观察像素空间相邻的token）。

Result: 在四个文本到图像生成基准测试中，LADR相比标准基线实现约4倍加速，同时保持甚至增强生成保真度，特别是在空间推理任务中，在效率和质量之间达到最先进的平衡。

Conclusion: LADR为离散扩散语言模型提供了一种训练免费的推理加速方法，通过利用图像的空间马尔可夫特性，在显著提升效率的同时保持生成质量，为多模态生成的实际部署提供了可行方案。
...
### [43] [DiveUp: Learning Feature Upsampling from Diverse Vision Foundation Models](https://arxiv.org/abs/2603.13571)
*Xiaoqiong Liu,Heng Fan*

Main category: cs.CV

TL;DR: DiveUp提出多视觉基础模型关系引导的特征上采样框架，通过跨模型结构共识避免单模型依赖带来的空间结构误差传播


<details>
  <summary>Details</summary>
Motivation: 现有特征上采样方法依赖同一基础模型的高分辨率特征进行自重建，但单模型特征会导致上采样器过度拟合源模型固有的位置错位和高范数伪影

Method: 1) 引入多VFM关系引导，利用不同模型的结构共识正则化上采样器学习；2) 提出通用关系特征表示（局部质心场）解决跨模型特征空间不对齐；3) 设计尖峰感知选择策略评估各VFM空间可靠性，仅聚合最可靠专家的指导

Result: DiveUp在各种下游密集预测任务中达到最先进性能，验证了多专家关系引导的有效性。该框架是统一的、编码器无关的，无需针对每个模型重新训练

Conclusion: 通过打破单模型依赖，利用多视觉基础模型的结构共识进行关系引导，DiveUp有效解决了特征上采样中的空间结构误差传播问题，为像素级理解任务提供了更鲁棒的特征增强方案
...
### [44] [Sparse-Dense Mixture of Experts Adapter for Multi-Modal Tracking](https://arxiv.org/abs/2603.13719)
*Yabin Zhu,Jianqi Li,Chenglong Li,Jiaxiang Wang,Chengjie Gu,Jin Tang*

Main category: cs.CV

TL;DR: 提出SDMoEA框架，通过稀疏-稠密混合专家适配器和基于Gram矩阵的超图融合模块，解决多模态跟踪中参数高效微调的跨模态异构性问题。


<details>
  <summary>Details</summary>
Motivation: 现有PEFT方法在多模态跟踪中面临跨模态异构性挑战，难以在统一框架中有效表示多模态特征，需要解决模态特定信息和共享信息的建模问题。

Method: 1. SDMoE模块：稀疏MoE捕获模态特定信息，稠密共享MoE建模跨模态共享信息；2. GSAHF模块：使用Gram矩阵进行跨模态语义对齐，构建超图建模高阶相关性，实现多层次多模态深度融合。

Result: 在LasHeR、RGBT234、VTUAV、VisEvent、COESOT、DepthTrack、VOT-RGBD2022等多个多模态跟踪基准测试中，性能优于其他PEFT方法。

Conclusion: SDMoEA框架有效解决了多模态跟踪中的跨模态异构性问题，通过混合专家适配器和超图融合实现了参数高效的多模态特征表示和深度融合。
...
### [45] [QTrack: Query-Driven Reasoning for Multi-modal MOT](https://arxiv.org/abs/2603.13759)
*Tajamul Ashraf,Tavaheed Tariq,Sonia Yadav,Abrar Ul Riyaz,Wasif Tak,Moloud Abdar,Janibul Bashir*

Main category: cs.CV

TL;DR: 提出QTracK：基于自然语言查询的跟踪新范式，将跟踪视为时空推理问题，通过语言指令指定跟踪目标


<details>
  <summary>Details</summary>
Motivation: 传统多目标跟踪(MOT)跟踪视频中所有对象，无法根据语义指令选择性地推理用户指定的目标，需要更灵活的跟踪范式

Method: 1) 构建RMOT26大规模基准数据集，包含基于查询的标注和序列级划分；2) 提出QTracK端到端视觉语言模型，整合多模态推理与跟踪定位；3) 引入时间感知策略优化(TPPO)方法，使用结构化奖励鼓励运动感知推理

Result: 实验证明该方法在推理为中心、语言引导的跟踪任务中有效，代码和数据已开源

Conclusion: 提出查询驱动的跟踪新范式，将跟踪从无差别跟踪所有对象转变为基于自然语言查询的语义目标跟踪，为更智能的视觉推理系统奠定基础
...
### [46] [ALTIS: Automated Loss Triage and Impact Scoring from Sentinel-1 SAR for Property-Level Flood Damage Assessment](https://arxiv.org/abs/2603.13803)
*Amogh Vinaykumar,Prem Kamasani*

Main category: cs.CV

TL;DR: ALTIS是一个基于卫星SAR数据的洪水保险理赔智能分级系统，能在洪水峰值后24-48小时内生成房产级影响评分，旨在减少不必要的现场勘察。


<details>
  <summary>Details</summary>
Motivation: 洪水是全球成本最高的自然灾害之一，但保险业灾后响应仍依赖人工现场勘察，这种方式缓慢、昂贵且受地理限制。现有SAR洪水检测研究使用学术指标（如IoU、F1分数），未能捕捉保险工作流程的实际需求。

Method: ALTIS是一个五阶段流水线：1) 使用双极化VV/VH强度和InSAR相干性的多时相SAR变化检测；2) 融合洪水范围和高分辨率DEM的物理信息深度估计；3) 基于地块边界的房产级区域统计；4) 针对NFIP理赔数据的深度-损失校准；5) 置信度评分分级排序。系统还定义了保险级洪水分级(IGFT)概念，引入了勘察减少率(IRR)和分级效率评分(TES)。

Result: 在哈里斯县（德克萨斯州）2017年飓风哈维案例中，初步分析显示ALTIS设计目标是在高严重性理赔90%召回率下实现约0.52的IRR，可能消除超过一半的不必要现场派遣。
...
### [47] [OpenCOOD-Air: Prompting Heterogeneous Ground-Air Collaborative Perception with Spatial Conversion and Offset Prediction](https://arxiv.org/abs/2603.13919)
*Xianke Wu,Songlin Bai,Chengxiang Li,Zhiyao Luo,Yulin Tian,Fenghua Zhu,Yisheng Lv,Yonglin Tian*

Main category: cs.CV

TL;DR: OpenCOOD-Air 是一个将无人机集成到车车协同感知中的新框架，通过迁移学习和空间转换器解决地面-空中域差距，显著提升了感知性能。


<details>
  <summary>Details</summary>
Motivation: 车车协同感知虽然扩展了感知范围，但仍受地面遮挡和底盘传感器视角限制，存在关键感知盲区。需要引入无人机作为可扩展平台来克服这些限制。

Method: 采用迁移学习策略微调无人机权重；将地面-空中协同感知建模为异构集成任务，引入跨域空间转换器和空间偏移预测变换器；创建OPV2V-Air基准数据集。

Result: 相比现有方法，2D AP@0.7提升4%，3D AP@0.7提升7%，有效验证了从车车协同到车车-无人机协同的过渡。

Conclusion: OpenCOOD-Air成功将无人机集成到协同感知系统中，通过创新的迁移学习和空间建模方法，显著提升了感知性能并克服了地面视角限制。
...
### [48] [Interp3R: Continuous-time 3D Geometry Estimation with Frames and Events](https://arxiv.org/abs/2603.14528)
*Shuang Guo,Filbert Febryanto,Lei Sun,Guillermo Gallego*

Main category: cs.CV

TL;DR: Interp3R是首个利用异步事件数据在任意时间点插值点云图以恢复深度和相机姿态的方法，实现了时间连续的几何表示。


<details>
  <summary>Details</summary>
Motivation: 现有基于点云图的3D视觉基础模型（如DUSt3R）只能恢复图像捕获时刻的离散场景几何，无法探索连续帧之间的场景演化。需要解决时间连续性不足的问题。

Method: 利用异步事件数据插值帧基模型生成的点云图，通过将插值点云图与帧基模型预测的点云图对齐到一致空间框架中，联合恢复深度和相机姿态。

Result: 仅在合成数据集上训练，但在合成和真实世界基准测试中表现出强泛化能力，显著优于先进行2D视频帧插值再进行3D几何估计的两阶段基线方法。

Conclusion: Interp3R通过结合事件数据实现了时间连续的3D几何恢复，为点云图基础模型提供了时间插值能力，在连续场景理解方面具有重要价值。
...
### [49] [Efficient Event Camera Volume System](https://arxiv.org/abs/2603.14738)
*Juan Camilo Soto,Ian Noronha,Saru Bharti,Upinder Kaur*

Main category: cs.CV

TL;DR: EECVS是一个高效的事件相机压缩框架，通过将事件流建模为连续时间狄拉克脉冲序列，结合DCT、DTFT、DWT三种变换的自适应选择和变换特定系数剪枝，实现无伪影压缩和实时处理。


<details>
  <summary>Details</summary>
Motivation: 事件相机具有低延迟和高动态范围的优点，但其稀疏输出难以集成到标准机器人流程中。现有方法存在时间分箱伪影问题，需要一种既能保持计算效率又能实现良好泛化的自适应压缩框架。

Method: 将事件流建模为连续时间狄拉克脉冲序列，通过事件时间戳的直接变换评估实现无伪影压缩。采用密度驱动的自适应选择机制，在DCT、DTFT和DWT三种变换间动态切换，并结合每种变换域的稀疏特性设计专门的系数剪枝策略。

Result: 在EHPT-XC和MVSEC数据集上实现优越的重建保真度，DTFT获得最低的地球移动距离。在下游分割任务中表现出强大的泛化能力：使用EventSAM分割时，在MVSEC上达到平均IoU 0.87（体素网格为0.44），在EHPT-XC上保持竞争力。ROS2实现中DCT处理延迟仅1.5毫秒，吞吐量比其他变换高2.7倍。

Conclusion: EECVS是首个自适应事件压缩框架，通过消除时间分箱伪影、自适应选择压缩策略，在保持计算效率的同时实现了跨数据集和机器人场景的优越泛化性能，为事件相机在机器人系统中的实时部署提供了有效解决方案。
...
### [50] [Fine-tuning MLLMs Without Forgetting Is Easier Than You Think](https://arxiv.org/abs/2603.14493)
*He Li,Yuhui Zhang,Xiaohan Wang,Kaifeng Lyu,Serena Yeung-Levy*

Main category: cs.CV

TL;DR: 通过简单的微调策略调整即可缓解多模态大语言模型的灾难性遗忘，无需复杂机制


<details>
  <summary>Details</summary>
Motivation: 挑战现有假设，探索多模态大语言模型在微调过程中的灾难性遗忘问题，证明模型具有内在鲁棒性

Method: 设计2×2实验框架评估模型在分布内/外图像和文本输入上的表现；采用正则化方法（限制可训练参数、低学习率）；提出数据混合训练策略结合不同数据集和任务

Result: 适当正则化能有效防止分布外图像导致的遗忘；发现分布内图像+分布外文本场景下的任务特定过拟合；数据混合策略能解决此问题并自然扩展到持续学习，优于现有复杂方法

Conclusion: 多模态大语言模型具有内在鲁棒性，通过简单微调策略调整即可缓解灾难性遗忘，为模型适应提供实用指导
...
### [51] [VAD4Space: Visual Anomaly Detection for Planetary Surface Imagery](https://arxiv.org/abs/2603.13993)
*Fabrizio Genilotti,Arianna Stropeni,Francesco Borsatti,Manuel Barusco,Davide Dalle Pezze,Gian Antonio Susto*

Main category: cs.CV

TL;DR: 该论文首次在真实行星图像上评估了基于特征的视觉异常检测方法，为月球和火星任务建立了两个基准数据集，证明了这些方法在资源受限环境下有效检测罕见行星表面现象的能力。


<details>
  <summary>Details</summary>
Motivation: 太空任务产生大量高分辨率图像，远超人工检查能力。传统监督学习因标记样本稀缺和封闭世界假设而难以发现真正新颖的观测现象，需要自动化发现框架。

Method: 引入两个基准数据集：月球数据集（来自LROC窄角相机图像，包含新鲜和退化陨石坑作为异常）和火星表面数据集（模拟火星车获取图像特征）。评估多种基于特征的视觉异常检测方法，重点关注适合星载部署的计算高效、边缘导向解决方案。

Result: 基于特征的视觉异常检测方法能有效识别罕见行星表面现象，同时适用于资源受限环境。为行星科学中的异常检测建立了实用基准。

Conclusion: 这项工作展示了开放世界感知系统在支持任务关键应用方面的潜力，包括战术规划、着陆点选择、危险检测、带宽感知数据优先级排序以及未预料地质过程的发现。
...
### [52] [High-speed Imaging through Turbulence with Event-based Light Fields](https://arxiv.org/abs/2603.14023)
*Yu-Hsiang Huang,Levi Burner,Sachin Shah,Ziyuan Qu,Adithya Pediredla,Christopher A. Metzler*

Main category: cs.CV

TL;DR: 首个能够通过强大气湍流对快速移动的非刚性物体进行高速成像的系统，结合事件相机和光场技术，利用机器学习算法区分运动与湍流效应。


<details>
  <summary>Details</summary>
Motivation: 事件相机虽然能以数千帧/秒的速度捕捉高速图像，但无法区分场景运动与大气湍流效应。需要开发一种系统来解决这一限制，实现对快速移动非刚性物体在强湍流条件下的高质量成像。

Method: 采用事件光场相机同时捕捉场景的多个视角，结合机器学习重建算法。通过分析不同视角下事件的相关性来区分运动（强相关）和湍流（弱相关）效应。

Result: 实验证明该系统能够在强湍流条件下对高速运动物体（速度高达每秒16,000像素）进行成像，成功克服了湍流对成像质量的影响。

Conclusion: 事件光场相机与机器学习算法的结合为解决湍流环境下高速运动物体成像问题提供了有效方案，首次实现了在强湍流条件下对快速移动非刚性物体的高质量高速成像。
...
### [53] [A Multi-Agent Perception-Action Alliance for Efficient Long Video Reasoning](https://arxiv.org/abs/2603.14052)
*Yichang Xu,Gaowen Liu,Ramana Rao Kompella,Tiansheng Huang,Sihao Hu,Fatih Ilhan,Selim Furkan Tekin,Zachary Yahn,Ling Liu*
...
<details>
  <summary>Details</summary>
Motivation: 现有视频语言模型在处理长视频推理时面临效率和质量挑战，需要一种能够有效扩展至现实世界长视频同时保持高质量推理的方法。

Method: 采用多轮感知-动作探索循环：1) 感知探索阶段，智能体从采样帧中提取查询特定感知线索并进行线索对齐；2) 动作探索阶段，智能体生成初步答案和推理，通过交叉评审和相关性排名进行协作评分，根据共识决定是否开始新一轮探索或生成最终答案。

Result: 在五个流行的VideoQA基准测试中，A4VL超越了18个现有代表性VLM和10个针对长视频推理优化的最新方法，同时实现了显著更低的推理延迟。

Conclusion: 通过多智能体联盟的多轮感知-动作探索，结合事件驱动分区和线索引导的块对齐，A4VL能够有效扩展到现实世界长视频，同时保持高质量的视频推理能力。
...
### [54] [Enhancing Eye Feature Estimation from Event Data Streams through Adaptive Inference State Space Modeling](https://arxiv.org/abs/2603.14077)
*Viet Dung Nguyen,Mobina Ghorbaninejad,Chengyi Ma,Reynold Bailey,Gabriel J. Diaz,Alexander Fix,Ryan J. Suess,Alexander Ororbia*

Main category: cs.CV

TL;DR: 提出自适应推理状态空间模型(AISSM)，通过动态置信网络估计信噪比和事件密度，动态调整当前与近期信息的权重，提升事件驱动眼动特征提取性能


<details>
  <summary>Details</summary>
Motivation: 现有眼动特征提取器难以处理由不同注视行为运动学变化引起的事件密度突变，导致预测性能下降

Method: 提出自适应推理状态空间模型(AISSM)，结合动态置信网络估计信噪比和事件密度，动态调整信息权重，并开发新的训练技术提升训练效率

Result: 实验结果表明AISSM系统在事件驱动眼动特征提取任务上优于现有最先进模型

Conclusion: AISSM能够有效处理事件密度突变问题，通过自适应信息加权机制提升眼动特征提取性能，为实际眼动跟踪系统提供高效低能耗解决方案
...
### [55] [Diffusion Reinforcement Learning via Centered Reward Distillation](https://arxiv.org/abs/2603.14128)
*Yuanzhi Zhu,Xi Wang,Stéphane Lathuilière,Vicky Kalogeiton*

Main category: cs.CV

TL;DR: CRD是一种基于前向过程的扩散模型强化学习微调框架，通过中心化奖励蒸馏解决分布漂移和奖励攻击问题，在文本到图像生成中实现快速收敛和减少奖励攻击。


<details>
  <summary>Details</summary>
Motivation: 扩散和流模型虽然达到SOTA生成性能，但细粒度提示保真度、组合正确性和文本渲染等行为在预训练目标中规范较弱。RL微调是自然解决方案，但现有扩散RL方法存在脆弱性：轨迹方法内存成本高、梯度估计方差大；前向过程方法收敛快但易出现分布漂移和奖励攻击。

Method: 提出中心化奖励蒸馏(CRD)框架，基于KL正则化奖励最大化构建在前向过程微调上。核心洞察是在提示内中心化下难处理的归一化常数会抵消。引入三种技术控制分布漂移：1)解耦采样器和移动参考防止比率信号崩溃；2)KL锚定到CFG引导的预训练模型控制长期漂移；3)奖励自适应KL强度加速早期学习同时减少后期奖励模型漏洞利用。

Result: 在文本到图像后训练中使用GenEval和OCR奖励的实验显示，CRD实现了竞争性的SOTA奖励优化结果，具有快速收敛和减少奖励攻击的特点，在未见偏好指标上得到验证。

Conclusion: CRD为扩散模型RL微调提供了一个稳定框架，有效解决了分布漂移和奖励攻击问题，在文本到图像生成任务中表现出色。
...
### [56] [Deep Learning From Routine Histology Improves Risk Stratification for Biochemical Recurrence in Prostate Cancer](https://arxiv.org/abs/2603.14187)
*Clément Grisi,Khrystyna Faryna,Nefise Uysal,Vittorio Agosti,Enrico Munari,Solène-Florence Kammerer-Jacquet,Paulo Guilherme de Oliveira Salles,Yuri Tolkach,Reinhard Büttner,Sofiya Semko,Maksym Pikul,Axel Heidenreich,Jeroen van der Laak,Geert Litjens*

Main category: cs.CV

TL;DR: 深度学习模型从H&E染色前列腺切除标本直接预测生化复发风险，结合临床风险评分显著改善风险分层


<details>
  <summary>Details</summary>
Motivation: 现有临床病理风险模型将复杂形态简化为相对粗糙的描述符，导致常规组织病理学中大量预后信息未被充分利用，需要更精确的生化复发预测工具来指导辅助治疗和监测决策

Method: 开发基于深度学习的生物标志物，直接从H&E染色全切片前列腺切除标本端到端训练，预测患者特异性连续生化复发风险，在四个独立国际队列中评估，并与CAPRA-S临床风险评分整合

Result: 模型在跨机构和患者群体中表现出稳健的泛化能力，与CAPRA-S结合后，各队列的一致性指数从0.725-0.772提升至0.749-0.788，结果可解释性分析揭示了传统风险评分未捕捉的细微组织形态模式
...
### [57] [OAHuman: Occlusion-Aware 3D Human Reconstruction from Monocular Images](https://arxiv.org/abs/2603.14249)
*Yuanwang Yang,Hongliang Liu,Muxin Zhang,Nan Ma,Jingyu Yang,Yu-Kun Lai,Kun Li*

Main category: cs.CV

TL;DR: OAHuman：一种遮挡感知的单目3D人体重建框架，通过解耦几何重建和纹理合成，解决遮挡区域中几何与纹理交叉污染问题，提升遮挡条件下的重建完整性和真实感。


<details>
  <summary>Details</summary>
Motivation: 现实场景中的单目3D人体重建面临严重遮挡问题（来自周围物体、人物或图像截断），导致几何缺失和外观线索不可靠，严重影响重建模型的完整性和真实感。现有神经隐式方法在干净输入上表现良好，但在遮挡情况下因形状和纹理的纠缠建模而表现不佳。

Method: 提出OAHuman框架，采用解耦感知范式，明确分离几何重建和纹理合成。核心创新在于：1）几何重建在遮挡区域仍能获得感知强化，与纹理干扰隔离；2）纹理合成仅从可见区域学习，防止纹理错误传播到遮挡区域。这种解耦方法解决了遮挡区域中几何-纹理交叉污染的根本问题。

Result: 在遮挡丰富的基准测试中，OAHuman在结构完整性、表面细节和纹理真实感方面均表现出优越性能，显著改善了遮挡条件下的单目3D人体重建效果。

Conclusion: OAHuman通过解耦几何重建和纹理合成的创新方法，有效解决了遮挡条件下3D人体重建的长期挑战，实现了鲁棒且高保真的重建结果，为该领域提供了重要进展。
...
### [58] [RL-ScanIQA: Reinforcement-Learned Scanpaths for Blind 360°Image Quality Assessment](https://arxiv.org/abs/2603.14297)
*Yujia Wang,Yuyan Li,Jiuming Liu,Fang-Lue Zhang,Xinhu Zheng,Neil. A Dodgson*

Main category: cs.CV

TL;DR: RL-ScanIQA：基于强化学习的盲360°图像质量评估框架，通过端到端优化扫描路径策略和质量评估器，提升任务相关观看策略学习
...
Method: 提出强化学习框架RL-ScanIQA，使用PPO训练的扫描路径策略和质量评估器，通过质量驱动反馈学习任务相关观看策略；设计多层次奖励（扫描路径多样性和赤道偏向先验）提升训练稳定性；采用失真空间增强和排序一致性损失提升跨数据集鲁棒性

Result: 在三个基准数据集上的实验表明，RL-ScanIQA在数据集内性能和跨数据集泛化能力方面均优于现有方法

Conclusion: RL-ScanIQA通过端到端强化学习框架有效解决了360°图像质量评估中扫描路径生成与质量评估分离的问题，实现了任务对齐的观看策略学习和更好的性能表现
...
### [59] [AgroNVILA: Perception-Reasoning Decoupling for Multi-view Agricultural Multimodal Large Language Models](https://arxiv.org/abs/2603.14342)
*Jiarui Zhang,Junqi Hu,Zurong Mai,Yuhang Chen,Shuohong Lou,Henglian Huang,Lingyuan Zhao,Jianxi Huang,Yutong Lu,Haohuan Fu,Juepeng Zheng*

Main category: cs.CV

TL;DR: AgroNVILA：首个针对农业多模态推理的MLLM，通过感知-推理解耦架构解决尺度混淆问题，在农业空间规划任务上超越现有方法15.18%


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型存在"陆地中心"偏差，导致农业场景中不同尺度（地面特写、无人机、卫星图像）的空间理解混乱和逻辑漂移，无法满足精准农业的复杂规划需求

Method: 1) 构建大规模多视图训练数据集AgroOmni（288K样本）；2) 提出感知-推理解耦架构：感知侧使用视图条件元网络注入宏观空间上下文，推理侧采用农业感知相对策略优化对齐专家逻辑

Result: AgroNVILA在多种海拔农业推理任务上显著超越现有最先进MLLMs，性能提升达15.18%，展现出强大的整体农业空间规划能力

Conclusion: 该研究通过专门的数据集和架构设计成功解决了农业多模态推理中的尺度混淆问题，为精准农业的智能决策提供了有效解决方案
...
### [60] [Wi-Spike: A Low-power WiFi Human Multi-action Recognition Model with Spiking Neural Networks](https://arxiv.org/abs/2603.14475)
*Nengbo Zhang,Yao Ying,Lu Wang,Kaishun Wu,Jieming Ma,Fei Luo*

Main category: cs.CV

TL;DR: Wi-Spike：基于脉冲神经网络的WiFi动作识别框架，在保持高准确率的同时显著降低能耗


<details>
  <summary>Details</summary>
Motivation: 现有WiFi动作识别模型主要关注识别准确率，而对功耗和能效问题讨论不足。需要开发既能保持高准确率又能实现低功耗的解决方案，适用于实时边缘传感应用。

Method: 提出Wi-Spike框架，采用生物启发的脉冲神经网络（SNN）。使用脉冲卷积层进行时空特征提取，引入新颖的时间注意力机制增强判别性表示，通过脉冲全连接层和投票层进行编码和分类。

Result: 在三个基准数据集（NTU-Fi-HAR、NTU-Fi-HumanID、UT-HAR）上验证，单动作识别达到竞争性准确率，多动作识别任务表现优异。能耗至少降低一半，同时保持95.83%的动作识别准确率。

Conclusion: Wi-Spike在WiFi多动作识别任务中建立了新的最先进水平，为实时、高能效的边缘传感应用提供了有前景的解决方案，平衡了准确率和能耗。
...
### [61] [LatSearch: Latent Reward-Guided Search for Faster Inference-Time Scaling in Video Diffusion](https://arxiv.org/abs/2603.14526)
*Zengqun Zhao,Ziquan Liu,Yu Cao,Shaogang Gong,Zhensong Zhang,Jifei Song,Jiankang Deng,Ioannis Patras*

Main category: cs.CV

TL;DR: 提出LatSearch方法，通过潜在奖励模型在去噪过程中提供中间反馈，结合重采样和剪枝机制，提升视频扩散模型的生成质量和效率。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型的推理时优化方法存在局限性：要么依赖噪声采样开始时的先验，要么只在完全去噪解码的视频上评估奖励，导致误差累积、奖励信号延迟稀疏、计算成本过高，限制了更强搜索算法的应用。

Method: 提出潜在奖励模型，在任意时间步对部分去噪的潜在表示进行视觉质量、运动质量和文本对齐的评分；基于此提出LatSearch方法，包含奖励引导的重采样和剪枝两个阶段：重采样阶段根据奖励归一化概率采样候选以减少对奖励模型的过度依赖；剪枝阶段在最终调度步骤保留累积奖励最高的候选。

Result: 在VBench-2.0基准测试中，LatSearch相比基线Wan2.1模型，在多个评估维度上一致提升了视频生成质量。

Conclusion: 通过潜在奖励引导实现了高效的推理时缩放，为视频扩散模型解锁了更强的搜索算法，提升了可控性、样本效率和生成质量。
...
### [62] [E2EGS: Event-to-Edge Gaussian Splatting for Pose-Free 3D Reconstruction](https://arxiv.org/abs/2603.14684)
*Yunsoo Kim,Changki Sung,Dasol Hong,Hyun Myung*

Main category: cs.CV

TL;DR: E2EGS是一个仅使用事件流、无需姿态信息的框架，通过提取边缘信息实现轨迹估计和高质量新视角合成，解决了现有方法对高质量RGB输入和准确姿态的依赖问题。


<details>
  <summary>Details</summary>
Motivation: NeRF和3DGS等新视角合成方法需要高质量RGB输入和准确相机姿态，在快速相机运动或恶劣光照等真实场景中鲁棒性不足。事件相机具有高时间分辨率和宽动态范围，能精确感知动态场景，但现有事件方法要么假设已知姿态，要么依赖深度估计模型，无法泛化到未观察区域。

Method: 提出E2EGS框架，关键洞察是边缘信息为轨迹估计和高质量新视角合成提供丰富结构线索。通过分析边缘和非边缘区域的时空特性差异：相机运动在边缘产生一致事件，非边缘区域产生稀疏噪声。采用基于块的时间一致性分析测量局部方差来提取边缘并抑制噪声。提取的边缘指导结构感知的高斯初始化，并在初始化、跟踪和束调整过程中使用边缘加权损失。

Result: 在合成和真实数据集上的广泛实验表明，E2EGS实现了优越的重建质量和轨迹精度，建立了完全无需姿态的事件3D重建范式。

Conclusion: E2EGS通过有效提取事件流中的边缘信息，实现了仅基于事件流的姿态无关新视角合成和轨迹估计，为事件相机在动态场景中的3D重建提供了鲁棒解决方案。
...
### [63] [A Skill-augmented Agentic Framework and Benchmark for Multi-Video Understanding](https://arxiv.org/abs/2603.14733)
*Yue Zhang,Liqiang Jing,Jia Li,Yapeng Tian,Xinya Du,Yunhui Guo,Vibhav Gogate*

Main category: cs.CV

TL;DR: MVX-Bench：多视频跨维度基准测试，将11个CV任务统一为多视频问答框架；SAMA：技能增强的代理框架，通过视觉工具和冲突感知验证实现结构化推理。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在多视频理解方面能力有限，现有方法存在训练-推理不匹配、帧压缩信息丢失、缺乏显式跨视频协调等问题，且现有基准主要关注事件级比较，忽视了身份级匹配、细粒度区分和结构化多步推理。

Method: 提出MVX-Bench基准，将11个经典计算机视觉任务统一为多视频问答框架，包含1,442个问题和4,255个视频；提出SAMA框架，整合视觉工具、任务特定技能和冲突感知验证机制，实现迭代式结构化推理。

Result: SAMA在MVX-Bench上优于强大的开源基线和GPT，消融实验验证了技能设计和冲突解决机制的有效性。

Conclusion: MVX-Bench填补了多视频理解基准的空白，SAMA框架通过技能增强和冲突感知验证显著提升了多视频推理能力，为多视频理解提供了新的解决方案。
...
### [64] [AnyPhoto: Multi-Person Identity Preserving Image Generation with ID Adaptive Modulation on Location Canvas](https://arxiv.org/abs/2603.14770)
*Longhui Yuan*
...
<details>
  <summary>Details</summary>
Motivation: 多人物身份保持生成需要在文本提示下将多个参考人脸绑定到指定位置，但强烈的身份/布局条件容易触发复制粘贴的捷径行为，削弱了提示驱动的可控性。

Method: 1) RoPE对齐的位置画布加位置对齐的令牌剪枝用于空间定位；2) 基于人脸识别嵌入的AdaLN风格身份自适应调制用于持续身份注入；3) 身份隔离注意力防止跨身份干扰。训练结合条件流匹配和嵌入空间人脸相似度损失，以及参考人脸替换和位置画布退化来抑制捷径行为。

Result: 在MultiID-Bench上，AnyPhoto提高了身份相似度，同时减少了复制粘贴倾向，随着身份数量的增加，增益更加明显。AnyPhoto还支持具有准确放置的提示驱动风格化。

Conclusion: AnyPhoto通过创新的空间定位、身份自适应调制和身份隔离注意力机制，有效解决了多人物身份保持生成中的复制粘贴问题，在保持身份相似度的同时增强了提示驱动的可控性，具有很大的应用潜力。
...
### [65] [Video-CoE: Reinforcing Video Event Prediction via Chain of Events](https://arxiv.org/abs/2603.14935)
*Qile Su,Jing Tang,Rui Chen,Lei Sun,Xiangxiang Chu*

Main category: cs.CV

TL;DR: 本文提出CoE（事件链）范式，通过构建时间事件链来提升多模态大语言模型在视频事件预测任务上的性能，解决了现有模型在逻辑推理和视觉信息利用方面的不足。


<details>
  <summary>Details</summary>
Motivation: 尽管MLLMs在各种视频任务中取得进展，但视频事件预测（VEP）仍相对未被充分探索。VEP需要模型进行细粒度的时间建模并建立视频与未来事件之间的逻辑关系，而当前MLLMs在这方面仍有困难。

Method: 提出CoE（事件链）范式，通过构建时间事件链来隐式强制MLLM关注视觉内容和视频与未来事件之间的逻辑连接，使用多种训练协议激励模型的推理能力。

Result: 在公共基准测试上的实验结果表明，该方法优于领先的开源和商业MLLMs，在VEP任务上建立了新的最先进水平。

Conclusion: CoE范式通过构建时间事件链有效解决了MLLMs在视频事件预测中的逻辑推理和视觉信息利用问题，显著提升了预测性能。
...
### [66] [Relevance Feedback in Text-to-Image Diffusion: A Training-Free And Model-Agnostic Interactive Framework](https://arxiv.org/abs/2603.14936)
*Wenxi Wang,Hongbin Liu,Mingqian Li,Junyan Yuan,Junqi Zhang*

Main category: cs.CV

TL;DR: RFD是一个基于相关性反馈机制的交互式文本到图像生成框架，用户通过多选视觉反馈表达偏好，无需文本对话或模型微调，实现低认知负荷、可解释的偏好推断和通用即插即用。


<details>
  <summary>Details</summary>
Motivation: 用户在文本到图像生成中虽有明确视觉意图，但难以用语言精确表达，导致提示模糊和图像不对齐。现有方法依赖高负荷文本对话、不透明的黑盒推断或昂贵的微调，无法同时实现低认知负荷、可解释偏好推断且保持免训练和模型无关。

Method: RFD将信息检索中的相关性反馈机制应用于扩散模型：1）用户通过多选视觉反馈表达偏好，降低认知负荷；2）构建专家策划的特征库，采用信息论加权累积偏好分析，白盒计算当前轮次反馈并增量累积偏好；3）使用概率采样机制进行提示重建，平衡利用与探索；4）完全在外部文本空间操作，免训练且模型无关。

Result: 大量实验表明，RFD能有效捕捉用户的真实视觉意图，在偏好对齐方面显著优于基线方法。

Conclusion: RFD通过交互式视觉反馈机制成功解决了文本到图像生成中的意图表达难题，实现了低认知负荷、可解释偏好推断、免训练和模型无关的统一，为通用即插即用解决方案提供了新范式。
...
### [67] [Question-guided Visual Compression with Memory Feedback for Long-Term Video Understanding](https://arxiv.org/abs/2603.15167)
*Sosuke Yamao,Natsuki Miyahara,Yuankai Qi,Shun Takeuchi*
...
<details>
  <summary>Details</summary>
Motivation: 现有长视频理解方法通常独立压缩每帧，缺乏对完整事件的理解能力，在需要时序推理的任务上表现不佳。需要重新思考从感知到记忆的单向流程，建立反馈驱动的过程。

Method: 提出QViC-MF框架，核心是问题引导的多模态选择性注意力(QMSA)，从当前片段和记忆中的相关历史帧中保留与问题相关的视觉信息。压缩器和记忆反馈为每个视频片段迭代工作。

Result: 在多个长视频理解任务上取得显著提升：MLVU测试集提升6.1%，LVBench提升8.3%，VNBench Long提升18.3%，VideoMME Long提升3.7%。

Conclusion: QViC-MF通过简单有效的反馈驱动设计，在长视频理解任务上实现了大幅性能提升，证明了从感知到记忆的单向流程向双向反馈机制转变的重要性。
...
### [68] [IConE: Batch Independent Collapse Prevention for Self-Supervised Representation Learning](https://arxiv.org/abs/2603.15263)
*Konstantinos Almpanakis,Anna Kreshuk*

Main category: cs.CV

TL;DR: IConE是一种自监督学习框架，通过全局可学习的辅助实例嵌入和显式多样性目标来防止表示坍塌，解决了小批量训练时现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的联合嵌入架构（JEAs）依赖批量交互来防止表示坍塌，但在小批量（如高维科学数据）或类别不平衡的情况下，这种方法变得不可行。需要一种不依赖批量统计的防坍塌机制。

Method: IConE框架维护一组全局可学习的辅助实例嵌入，通过显式多样性目标进行正则化，将防坍塌机制从瞬时批量转移到数据集级别的嵌入空间，支持小到批量大小为1的训练。

Result: 在2D和3D生物医学模态中，IConE在小批量（B=1到B=64）下优于强对比和非对比基线，对严重类别不平衡表现出显著鲁棒性。几何分析显示IConE保持了高内在维度。

Conclusion: IConE成功地将防坍塌机制与批量大小解耦，为小批量或类别不平衡场景下的自监督学习提供了有效解决方案。
...
### [69] [AnyCrowd: Instance-Isolated Identity-Pose Binding for Arbitrary Multi-Character Animation](https://arxiv.org/abs/2603.15415)
*Zhenyu Xie,Ji Xia,Michael Kampffmeyer,Panwen Hu,Zehua Ma,Yujian Zheng,Jing Wang,Zheng Chong,Xujie Zhang,Xianhang Cheng,Xiaodan Liang,Hao Li*
...
<details>
  <summary>Details</summary>
Motivation: 随着角色数量增加，多角色参考编码容易受到潜在身份纠缠的影响，导致身份混淆和控制性降低。同时，学习参考身份与驱动姿态序列之间精确的时空对应关系变得困难，经常导致身份-姿态错误绑定和生成视频的不一致性。

Method: 1. 提出实例隔离潜在表示(IILR)，在DiT处理前独立编码角色实例以防止潜在身份纠缠；2. 基于解耦表示提出三级解耦注意力(TSDA)，将自注意力分解为：实例感知前景注意力、背景中心交互、全局前景-背景协调；3. 在TSDA中集成自适应门控融合(AGF)模块，预测身份感知权重，有效融合竞争令牌组为身份一致表示。

Result: AnyCrowd能够扩展到任意数量的角色，有效解决了多角色动画中的身份纠缠问题，提高了身份-姿态绑定的准确性和生成视频的时空一致性。

Conclusion: AnyCrowd通过创新的实例隔离编码和解耦注意力机制，为多角色可控动画生成提供了有效的解决方案，解决了现有方法在角色数量增加时面临的身份纠缠和绑定问题。
...
### [70] [FreeTalk: Emotional Topology-Free 3D Talking Heads](https://arxiv.org/abs/2603.15512)
*Federico Nocentini,Thomas Besnier,Claudio Ferrari,Stefano Berretti,Mohamed Daoudi*

Main category: cs.CV

TL;DR: FreeTalk是一个两阶段框架，用于情感条件化的3D说话头动画，能够泛化到任意拓扑结构的未注册人脸网格，无需模板拟合或对应监督。


<details>
  <summary>Details</summary>
Motivation: 现有语音驱动的3D面部动画方法大多依赖于注册模板网格，无法有效应用于具有任意拓扑结构的原始3D扫描。同时，超越嘴唇发音的可控情感动态建模仍然具有挑战性，且通常与基于模板的参数化绑定。

Method: 采用两阶段框架：1) Audio-To-Sparse (ATS) 从语音音频预测3D地标位移序列，考虑情感类别和强度；2) Sparse-To-Mesh (STM) 通过结合内在表面特征和地标到顶点的条件化，将预测的地标运动转移到目标网格，生成密集的逐顶点变形。

Result: 在域内训练时与专业基线方法表现相当，同时在未见过的身份和网格拓扑结构上提供显著改进的鲁棒性。

Conclusion: FreeTalk解决了现有方法在未注册网格上的泛化问题，实现了情感条件化的3D说话头动画，无需模板拟合或对应监督，具有更好的泛化能力。
...
### [71] [From Stochastic Answers to Verifiable Reasoning: Interpretable Decision-Making with LLM-Generated Code](https://arxiv.org/abs/2603.13287)
...
### [72] [A Robust Framework for Secure Cardiovascular Risk Prediction: An Architectural Case Study of Differentially Private Federated Learning](https://arxiv.org/abs/2603.13293)
...
### [73] [Preventing Curriculum Collapse in Self-Evolving Reasoning Systems](https://arxiv.org/abs/2603.13309)
*Vaibhav Mishra*
...
### [74] [Volumetric Radar Echo Motion Estimation Using Physics-Informed Deep Learning: A Case Study Over Slovakia](https://arxiv.org/abs/2603.13589)
...
### [75] [Collapse or Preserve: Data-Dependent Temporal Aggregation for Spiking Neural Network Acceleration](https://arxiv.org/abs/2603.13810)
*Jiahao Qin*
...
### [76] [Robust Self-Training with Closed-loop Label Correction for Learning from Noisy Labels](https://arxiv.org/abs/2603.13894)
...
### [77] [Distributed Acoustic Sensing for Urban Traffic Monitoring: Spatio-Temporal Attention in Recurrent Neural Networks](https://arxiv.org/abs/2603.13903)
...
### [78] [Gated Graph Attention Networks for Predicting Duration of Large Scale Power Outages Induced by Natural Disasters](https://arxiv.org/abs/2603.14069)
...
### [79] [Soft Mean Expected Calibration Error (SMECE): A Calibration Metric for Probabilistic Labels](https://arxiv.org/abs/2603.14092)
*Michael Leznik*
...
### [80] [SPARQ: Spiking Early-Exit Neural Networks for Energy-Efficient Edge AI](https://arxiv.org/abs/2603.14380)
...
### [81] [Multimodal Deep Learning for Early Prediction of Patient Deterioration in the ICU: Integrating Time-Series EHR Data with Clinical Notes](https://arxiv.org/abs/2603.14719)
*Binesh Sadanandan*
...
### [82] [LaPro-DTA: Latent Dual-View Drug Representations and Salient Protein Feature Extraction for Generalizable Drug--Target Affinity Prediction](https://arxiv.org/abs/2603.14792)
...
### [83] [Real-Time Driver Safety Scoring Through Inverse Crash Probability Modeling](https://arxiv.org/abs/2603.14841)
...
### [84] [Spiking Layer-Adaptive Magnitude-based Pruning](https://arxiv.org/abs/2603.14946)
...
### [85] [CrossADR: enhancing adverse drug reactions prediction for combination pharmacotherapy with cross-layer feature integration and cross-level associative learning](https://arxiv.org/abs/2603.15047)
*Y. Cheung*
...
### [86] [CATFormer: When Continual Learning Meets Spiking Transformers With Dynamic Thresholds](https://arxiv.org/abs/2603.15184)
...
### [87] [FuXiWeather2: Learning accurate atmospheric state estimation for operational global weather forecasting](https://arxiv.org/abs/2603.15358)
...
### [88] [Grokking as a Variance-Limited Phase Transition: Spectral Gating and the Epsilon-Stability Threshold](https://arxiv.org/abs/2603.15492)
...
### [89] [Bridging Local and Global Knowledge: Cascaded Mixture-of-Experts Learning for Near-Shortest Path Routing](https://arxiv.org/abs/2603.15541)
...
### [90] [From Stochastic Answers to Verifiable Reasoning: Interpretable Decision-Making with LLM-Generated Code](https://arxiv.org/abs/2603.13287)
*Anirudh Jaidev Mahesh,Ben Griffin,Fuat Alican,Joseph Ternasky,Zakari Salifu,Kelvin Amoaba,Yagiz Ihlamur,Aaron Ontoyin Yin,Aikins Laryea,Afriyie Samuel,Yigit Ihlamur*
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM决策方法难以平衡可扩展性、可解释性和可复现性。黑盒模型不透明，而基于LLM的规则系统需要逐样本评估，成本随数据集规模增长且输出存在随机性和幻觉问题

Method: 将LLM重构为代码生成器而非逐实例评估器。单次LLM调用生成可执行、人类可读的决策逻辑，在结构化数据上确定性地运行。结合代码生成与自动统计验证（精度提升、二项显著性检验、覆盖过滤），并应用基于聚类的差距分析迭代优化决策逻辑，无需人工标注

Result: 在VCBench基准测试中（4500名创始人，9%基础成功率），该方法达到37.5%精度和F0.5分数25.0%，优于GPT-4o（30.0%精度和25.7% F0.5分数），同时保持完全可解释性

Conclusion: 该方法通过生成可执行规则实现了可验证、可解释的LLM决策，为高风险决策任务提供了可扩展、透明且可复现的解决方案
...
### [91] [A Robust Framework for Secure Cardiovascular Risk Prediction: An Architectural Case Study of Differentially Private Federated Learning](https://arxiv.org/abs/2603.13293)
*Rodrigo Tertulino,Laércio Alencar*

Main category: cs.LG

TL;DR: 该论文验证了FedCVR联邦学习框架在心血管风险预测中的工程鲁棒性，通过服务器端自适应优化在差分隐私约束下实现了稳定的临床效用恢复


<details>
  <summary>Details</summary>
Motivation: 心血管风险预测对预防性医疗至关重要，但临床数据因隐私法规分散在不同机构，阻碍了AI模型的开发。需要验证隐私保护联邦学习框架在实际临床网络中的工程鲁棒性

Method: 采用系统工程分析方法，在基于真实数据集（Framingham、Cleveland）校准的高保真合成环境中进行严格压力测试，量化服务器端自适应优化在效用优先差分隐私下的操作权衡

Result: 集成服务器端动量作为时间降噪器，使架构达到稳定的F1分数0.84和AUC 0.96，统计上优于标准无状态基线。服务器端自适应是恢复临床效用的结构前提

Conclusion: 服务器端自适应优化是在现实隐私预算下恢复临床效用的必要条件，为安全的多机构协作提供了经过验证的工程蓝图
...
### [92] [Preventing Curriculum Collapse in Self-Evolving Reasoning Systems](https://arxiv.org/abs/2603.13309)
*Vaibhav Mishra*

Main category: cs.LG

TL;DR: Prism是一种问题中心的自进化方法，通过语义分区定义持久多样性信号，结合最近发展区门控机制，解决自进化推理框架中的多样性崩溃问题，在数学推理基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 自进化推理框架让LLM通过迭代生成和解决问题来提升推理能力，但现有系统在几轮迭代后会出现多样性崩溃，即使表面变化保持，问题空间探索不足。需要直接解决这种崩溃问题。

Method: Prism方法：1) 在嵌入诱导的语义分区上定义持久多样性信号；2) 使用该信号鼓励跨迭代平衡探索未充分表示区域；3) 结合最近发展区(ZPD)门控机制保持边缘可解难度；4) 生成语义多样且具有挑战性的问题。

Result: 在7个数学推理基准测试中，Prism在6个任务上达到最高准确率：AMC上比R-Zero提升3.98个绝对点，Minerva Math上提升3.68个点。生成了100k数学问题构建Prism-Math数据集，展示了语义多样性。

Conclusion: 跨迭代语义覆盖是构建更强大自进化推理器的高杠杆且未充分探索的维度。Prism通过直接解决多样性崩溃问题，实现了更有效的自进化学习，释放了代码、数据集和模型以促进进一步研究。
...
### [93] [Volumetric Radar Echo Motion Estimation Using Physics-Informed Deep Learning: A Case Study Over Slovakia](https://arxiv.org/abs/2603.13589)
*Peter Pavlík,Anna Bou Ezzeddine,Viera Rozinajová*

Main category: cs.LG

TL;DR: 研究提出基于物理信息的卷积神经网络，从三维雷达反射率数据中估计不同高度层的独立水平运动场，用于降水临近预报。实验表明，虽然模型能学习高度层运动场，但大多数降水事件中垂直层间的位移高度相关，三维方法并未带来系统性预报精度提升。


<details>
  <summary>Details</summary>
Motivation: 当前大多数基于外推的降水临近预报方法使用二维雷达合成数据估计降水系统水平运动，但实际中降水系统在不同高度可能呈现不同的运动特征。研究者希望探索基于三维雷达数据的逐层运动场估计是否能带来预报性能提升。

Method: 提出基于物理信息的卷积神经网络，直接从三维雷达反射率数据中估计多个高度层的独立水平运动场。模型在斯洛伐克雷达网络的体观测数据上进行端到端训练，并与使用垂直池化二维雷达合成数据的相同架构基线模型进行对比。

Result: 模型能成功学习高度层运动场，但绝大多数降水事件中垂直层间的位移高度相关。三维方法未带来系统性预报精度提升：虽然分类指标显示较长预报时效的降水检测有所提高，但这主要归因于非物理伪影，并伴随正偏差增长。垂直运动场相关性分析证实，在研究的斯洛伐克地区，具有显著垂直运动变化的事件很少见。

Conclusion: 对于斯洛伐克雷达数据集，三维运动场估计的额外复杂性并未带来预测技能的实质性提升，因此不具合理性。但该框架仍适用于降水系统水平运动垂直变异性更强的气候区域。
...
### [94] [Collapse or Preserve: Data-Dependent Temporal Aggregation for Spiking Neural Network Acceleration](https://arxiv.org/abs/2603.13810)
*Jiahao Qin*

Main category: cs.LG

TL;DR: 论文揭示了脉冲稀疏性在GPU上实现高效计算的"错觉"，提出了TAC和TAC-TP两种时域聚合策略，根据数据类型（速率编码vs事件数据）选择不同策略，在保持或提升精度的同时显著加速SNN推理。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为脉冲稀疏性能在GPU硬件上实现高效的SNN推理，但作者发现这是一个错觉——五种稀疏计算策略在Apple M3 Max上都无法超越密集卷积，因为SIMD架构无法有效利用脉冲的细粒度、非结构化稀疏性。

Method: 提出了两种方法：1) TAC（时域聚合卷积）：利用卷积线性性，将K个脉冲帧预聚合后进行一次卷积调用，将T次调用减少到T/K次；2) TAC-TP（时域保持）：在每组中共享卷积输出到K个独立的LIF步骤，为下游层保留完整的时间分辨率。

Result: 在速率编码数据上，TAC在MNIST上实现13.8倍加速且精度提升1.6%，在Fashion-MNIST上提升5.4%；在事件数据DVS128-Gesture上，TAC-TP达到95.1%精度（基线96.3%），卷积调用减少50%，而标准TAC精度降至91.3%。

Conclusion: 最优时域聚合策略是数据依赖的：对速率编码数据应压缩时间维度（降噪），对事件数据应保持时间维度（信息保留）。加速效果是硬件无关的，TAC在NVIDIA V100上实现11.0倍加速。所有算子已在mlx-snn库中开源。
...
### [95] [Robust Self-Training with Closed-loop Label Correction for Learning from Noisy Labels](https://arxiv.org/abs/2603.13894)
*Zhanhui Lin,Yanlin Liu,Sanping Zhou*

Main category: cs.LG

TL;DR: 提出基于解耦双层优化的自训练标签校正框架，通过分类器和神经校正函数协同进化，利用小规模干净数据集和噪声后验模拟实现高效标签噪声处理，在CIFAR和Clothing1M数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有标签噪声处理方法（如转移矩阵、噪声检测、元学习）存在噪声样本利用率低、计算成本高的问题，需要更高效实用的解决方案。

Method: 采用解耦双层优化自训练框架，分类器和神经校正函数协同进化，利用小规模干净数据集进行噪声后验模拟和中间特征传递，形成闭环反馈系统防止错误放大。

Result: 在CIFAR和Clothing1M等基准数据集上实现最先进性能，同时显著减少训练时间，证明了方法的实际应用价值。

Conclusion: 提出的解耦双层优化自训练标签校正框架能有效处理标签噪声，具有理论保证、高噪声样本利用率和低计算成本，适用于实际场景中的噪声标签学习任务。
...
### [96] [Distributed Acoustic Sensing for Urban Traffic Monitoring: Spatio-Temporal Attention in Recurrent Neural Networks](https://arxiv.org/abs/2603.13903)
*Izhan Fakhruzi,Manuel Titos,Carmen Benítez,Luz García*

Main category: cs.LG

TL;DR: 该研究提出了一种基于分布式声学传感（DAS）的交通监测方法，通过集成时空注意力机制的RNN模型，实现了对交通事件的准确识别和可解释性分析。


<details>
  <summary>Details</summary>
Motivation: 城市交通监测对于改善交通流动性、增强安全性和支持可持续城市发展至关重要。DAS技术能够将现有光纤基础设施转化为密集的振动传感器阵列，实现大规模交通观测。然而，如何对高分辨率时空结构的DAS数据进行建模以实现可靠的交通事件识别仍然具有挑战性。

Method: 在西班牙格拉纳达进行了真实世界的DAS交通监测实验，车辆穿过垂直于道路部署的光纤。采用循环神经网络（RNNs）建模事件内和事件间的时间依赖性。在RNN架构中系统集成了空间和时间注意力机制，分析它们对识别性能、参数效率和可解释性的影响。

Result: 结果显示，适当且互补的注意力模块放置改善了准确性和模型复杂性之间的平衡。注意力热图通过突出显示信息丰富的空间位置和时间段，为分类决策提供了物理上有意义的解释。此外，提出的SA-bi-TA配置展示了空间可迁移性，能够成功识别训练中未使用的传感位置的交通事件，性能下降适中。

Conclusion: 这些发现支持开发可扩展和可解释的DAS交通监测系统，能够在异构城市传感条件下运行。注意力机制不仅提高了性能，还增强了模型的可解释性，为实际应用提供了重要价值。
...
### [97] [Gated Graph Attention Networks for Predicting Duration of Large Scale Power Outages Induced by Natural Disasters](https://arxiv.org/abs/2603.14069)
*Chenghao Duan,Chuanyi Ji,Anwar Walid,Scott Ganz*

Main category: cs.LG

TL;DR: 提出BiGGAT模型，用于预测自然灾害导致的大规模停电持续时间，通过图注意力网络和门控循环单元结合处理空间依赖性和事件异质性。


<details>
  <summary>Details</summary>
Motivation: 气候变化导致自然灾害引发的大规模停电事件增加，这些停电持续时间长，造成重大经济损失和社会影响。准确估计停电持续时间对于提升能源基础设施在恶劣天气下的韧性至关重要。

Method: 开发了Bimodal Gated Graph Attention Network (BiGGAT)，一种基于图的神经网络模型，将图注意力网络(GAT)与门控循环单元(GRU)集成，以捕捉复杂的空间特征。该模型针对高维空间依赖性、中等数量的大规模停电事件、事件类型异质性以及区域内不同影响等挑战。

Result: 在美国东南部六次主要飓风的大规模停电数据上进行实验，采用归纳学习设置。实验结果表明，BiGGAT相比基准模型取得了更优的性能。

Conclusion: BiGGAT模型能够有效处理自然灾害导致的停电持续时间预测问题中的独特挑战，为增强能源基础设施在恶劣天气下的韧性提供了有效的机器学习解决方案。
...
### [98] [Soft Mean Expected Calibration Error (SMECE): A Calibration Metric for Probabilistic Labels](https://arxiv.org/abs/2603.14092)
*Michael Leznik*

Main category: cs.LG

TL;DR: 论文提出了Soft Mean Expected Calibration Error (smece)，作为ECE的泛化版本，专门处理标签本身是概率值而非二元事件的情况。


<details>
  <summary>Details</summary>
Motivation: 传统ECE只适用于二元标签，但在许多现代场景中（如放射科医生置信度、教师模型软输出、生成模型后验概率等），标签本身就是概率值。在这些情况下，ECE会丢弃标签的概​​率信息，导致结构性错配问题。

Method: 将ECE公式中的每个预测分箱内的硬标签经验频率替换为该分箱内样本的概率标签均值，这一行修改构成了smece的核心。

Result: smece是ECE的严格泛化：当标签为二元时，smece完全退化为ECE；当标签为概率值时，smece能正确利用标签的概率信息。

Conclusion: smece解决了ECE在处理概率标签时的结构性缺陷，为现代机器学习中广泛存在的概率标签校准问题提供了合适的度量标准。
...
### [99] [SPARQ: Spiking Early-Exit Neural Networks for Energy-Efficient Edge AI](https://arxiv.org/abs/2603.14380)
*Parth Patne,Mahdi Taheri,Ali Mahani,Maksim Jenihhin,Reza Mahani,Christian Herglotz*

Main category: cs.LG

TL;DR: SPARQ框架整合脉冲计算、量化感知训练和强化学习引导的早期退出，实现高效自适应推理，在边缘AI中显著提升能效和性能。


<details>
  <summary>Details</summary>
Motivation: 脉冲神经网络（SNNs）虽然具有事件驱动的能效优势，但在实际应用中受到深度架构计算开销和缺乏输入自适应控制的限制，需要更高效的边缘AI解决方案。

Method: 提出SPARQ统一框架，结合脉冲计算、量化感知训练和强化学习引导的早期退出机制，创建量化动态SNNs（QDSNNs），实现硬件友好的自适应推理。

Result: 在MLP、LeNet和AlexNet架构上的评估显示，QDSNNs相比传统SNNs和QSNNs，准确率提升最高达5.15%，系统能耗降低330倍以上，突触操作减少90%以上。

Conclusion: SPARQ验证了作为硬件友好、高能效的边缘实时AI解决方案的有效性，为脉冲神经网络的实际部署提供了可行路径。
...
### [100] [Multimodal Deep Learning for Early Prediction of Patient Deterioration in the ICU: Integrating Time-Series EHR Data with Clinical Notes](https://arxiv.org/abs/2603.14719)
*Binesh Sadanandan*

Main category: cs.LG

TL;DR: 提出一种结合结构化时间序列数据与非结构化临床笔记的多模态深度学习模型，用于预测ICU患者24小时内的临床恶化风险，相比仅使用结构化数据的基线模型有显著提升。


<details>
  <summary>Details</summary>
Motivation: ICU患者临床恶化的早期识别是重大挑战，延迟识别导致可预防的发病率和死亡率。现有方法主要依赖结构化数据，缺乏对临床笔记中丰富信息的利用。

Method: 使用MIMIC-IV数据库构建74,822次ICU住院的数据集，采用双向LSTM编码器处理生理数据的时间模式，使用ClinicalBERT嵌入处理临床笔记，通过跨模态注意力机制融合两种模态。

Result: 多模态模型在823,641个测试样本上达到AUROC 0.7857和AUPRC 0.1908，临床笔记使AUROC提升2.5个百分点，AUPRC提升39.2%。深度学习模型显著优于XGBoost（0.7486）和逻辑回归（0.7171）。

Conclusion: 该研究提供了该领域的全面综述和可复现的多模态预测框架，证明结合结构化数据与临床笔记能显著提升ICU临床恶化预测性能，为临床决策支持提供新方法。
...
### [101] [LaPro-DTA: Latent Dual-View Drug Representations and Salient Protein Feature Extraction for Generalizable Drug--Target Affinity Prediction](https://arxiv.org/abs/2603.14792)
*Zihan Dun,Liuyi Xu,An-Yang Lu,Shuang Li,Yining Qian*

Main category: cs.LG

TL;DR: LaPro-DTA是一个用于药物-靶点亲和力预测的框架，通过潜在双视图药物表示机制解决过拟合问题，使用模式感知top-k池化提取关键蛋白特征，在冷启动场景下显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有药物-靶点亲和力预测方法在现实的冷启动场景（未见过的药物/靶点/对）中性能显著下降，主要原因是过拟合训练实例以及从无关靶点序列中信息丢失。

Method: 1. 潜在双视图药物表示机制：结合实例级视图（捕捉细粒度子结构）和分布级视图（通过语义重映射提炼通用化学骨架）；2. 模式感知top-k池化策略：过滤背景噪声，分离高响应生物活性区域；3. 跨视图多头注意力机制：融合纯化特征以建模全面相互作用。

Result: 在基准数据集上的广泛实验表明，LaPro-DTA显著优于最先进的方法，在Davis数据集的挑战性未见药物设置中实现了8%的MSE减少，同时为结合机制提供了可解释的见解。

Conclusion: LaPro-DTA框架通过解决过拟合和信息丢失问题，实现了稳健且可泛化的药物-靶点亲和力预测，在冷启动场景下表现出色，为药物发现提供了有效的工具。
...
### [102] [Real-Time Driver Safety Scoring Through Inverse Crash Probability Modeling](https://arxiv.org/abs/2603.14841)
*Joyjit Roy,Samaresh Kumar Singh*

Main category: cs.LG

TL;DR: SafeDriver-IQ将二元碰撞分类器转化为0-100连续安全评分框架，结合国家碰撞统计和自动驾驶自然驾驶数据，提供实时可解释风险预防


<details>
  <summary>Details</summary>
Motivation: 现有碰撞预测模型多为二元结果，缺乏连续风险量化、可解释性，且未充分考虑行人、骑行者等弱势道路使用者，限制了实时驾驶员反馈的可行性

Method: 融合NHTSA碰撞记录与Waymo开放运动数据集场景，设计领域知识特征，基于交通安全文献构建校准层，将二元分类器转化为连续安全评分

Result: 框架在15项互补分析中可靠区分高低风险驾驶条件，87%碰撞涉及多个风险因素共存，非线性复合效应使风险增至基线4.5倍
...
### [103] [Spiking Layer-Adaptive Magnitude-based Pruning](https://arxiv.org/abs/2603.14946)
*Junqiao Wang,Zhehang Ye,Yuqi Ouyang*

Main category: cs.LG

TL;DR: SLAMP是一种理论指导的脉冲神经网络剪枝框架，通过控制跨层和时间步的最坏输出失真，实现高效连接和脉冲操作减少。


<details>
  <summary>Details</summary>
Motivation: 脉冲神经网络虽然能效高，但部署受到密集连接和高脉冲操作成本的限制。现有的基于幅度的剪枝策略直接应用于SNNs时，未能考虑时间累积、非均匀时间步贡献和膜稳定性，通常导致严重的性能下降。

Method: 提出Spiking Layer-Adaptive Magnitude-based Pruning (SLAMP)框架，将层自适应幅度剪枝推广到时域SNNs。通过将稀疏分配公式化为时间失真约束优化问题，产生时间感知的层重要性分数。采用高效的两阶段过程：时间分数估计、全局稀疏分配，以及带重训练的幅度剪枝以恢复稳定性。

Result: 在CIFAR10、CIFAR100和事件驱动的CIFAR10-DVS数据集上的实验表明，SLAMP实现了显著的连接和脉冲操作减少，同时保持了准确性，实现了高效可部署的SNN推理。

Conclusion: SLAMP通过理论指导的时间感知剪枝框架，解决了SNNs剪枝中的时间维度挑战，为高效可部署的脉冲神经网络推理提供了有效解决方案。
...
### [104] [CrossADR: enhancing adverse drug reactions prediction for combination pharmacotherapy with cross-layer feature integration and cross-level associative learning](https://arxiv.org/abs/2603.15047)
*Y. Cheung*

Main category: cs.LG

TL;DR: CrossADR是一个用于器官水平药物不良反应预测的分层框架，通过跨层特征整合和跨级关联学习，在包含1376种药物和946,000个独特组合的数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 联合药物治疗虽然具有显著治疗优势，但也带来严重的不良药物反应风险。当前基于图的方法难以有效整合多尺度生物信息，且依赖固定关联矩阵，限制了捕捉动态器官水平依赖性和跨数据集泛化能力。

Method: 提出CrossADR分层框架，采用门控残差流图神经网络融合多尺度分子特征，利用可学习的ADR嵌入空间动态捕捉15个器官系统的潜在生物相关性，实现跨层特征整合和跨级关联学习。

Result: 在新构建的CrossADR数据集（包含1376种药物和946,000个独特组合）上，CrossADR在80个不同实验场景中始终达到最先进的性能，并提供药物相关蛋白质相互作用和通路的高分辨率洞察。
...
### [105] [CATFormer: When Continual Learning Meets Spiking Transformers With Dynamic Thresholds](https://arxiv.org/abs/2603.15184)
*Vaishnavi Nagabhushana,Kartikay Agrawal,Ayon Borthakur*
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络在连续学习场景中面临灾难性遗忘问题，现有SNN在类增量学习中随着任务积累性能急剧下降，而人脑能够在不遗忘的情况下持续学习。需要开发能够像人脑一样持续学习的SNN架构。

Method: 提出CATFormer框架，核心是动态阈值漏积分发放(DTLIF)神经元模型，通过上下文自适应阈值调节神经元兴奋性来保留知识，配合门控动态头选择(G-DHS)机制实现任务无关推理。

Result: 在静态数据集(CIFAR-10/100/Tiny-ImageNet)和神经形态数据集(CIFAR10-DVS/SHD)上的广泛评估表明，CATFormer在各种任务划分下都优于现有的无排练类增量学习算法。

Conclusion: CATFormer通过调节神经元兴奋性而不仅仅是突触可塑性来防止遗忘，为能量高效的真正类增量学习提供了理想架构，解决了SNN中的灾难性遗忘问题。
...
### [106] [FuXiWeather2: Learning accurate atmospheric state estimation for operational global weather forecasting](https://arxiv.org/abs/2603.15358)
*Xiaoze Xu,Xiuyu Sun,Songling Zhu,Xiaohui Zhong,Yuanqing Huang,Zijian Zhu,Jun Liu,Hao Li*

Main category: cs.LG

TL;DR: FuXiWeather2是一个统一的端到端神经框架，结合数据同化和预报，通过直接对齐真实观测和再分析数据的训练目标来纠正再分析产品的系统偏差，在10天预报中超越现有NWP系统。


<details>
  <summary>Details</summary>
Motivation: 传统数值天气预报受限于数据同化和数值模拟的计算瓶颈，现有机器学习模型主要作为"再分析产品模拟器"，保留了系统偏差和操作延迟，需要开发能直接纠正再分析误差的端到端框架。

Method: 提出统一的端到端神经框架，采用递归展开训练方法处理训练与部署时的分布偏移，在混合数据集（原始和模拟观测）上训练，生成0.25度高分辨率全球分析场和10天预报。

Result: 分析场在大多数变量上超越NCEP-GFS，在低对流层和地表变量上优于ERA5和ECMWF-HRES系统；确定性预报在91%的评估指标上超过HRES系统；在台风路径预测中表现优异。

Conclusion: FuXiWeather2通过端到端神经框架有效纠正再分析产品偏差，在分析质量和预报技能上超越现有NWP系统，对极端天气事件的快速响应具有实用价值。
...
### [107] [Grokking as a Variance-Limited Phase Transition: Spectral Gating and the Epsilon-Stability Threshold](https://arxiv.org/abs/2603.15492)
*Pratyush Acharya,Habish Dhakal*

Main category: cs.LG

TL;DR: 论文分析了AdamW优化器在模块化算术任务中的grokking现象，揭示了"Spectral Gating"机制如何调节从记忆到泛化的转变，挑战了"平坦最小值"假说。


<details>
  <summary>Details</summary>
Motivation: 标准优化理论难以解释grokking现象（泛化在训练收敛后很久才发生）。几何研究虽然将其归因于缓慢漂移，但往往忽略了优化器噪声结构与损失函数曲率之间的相互作用。

Method: 分析AdamW在模块化算术任务上的动态，研究其作为方差门控随机系统的行为。通过消融实验识别三个复杂度机制：容量崩溃、方差受限机制和稳定性覆盖。

Result: 发现AdamW通过"Spectral Gating"机制调节泛化过程：泛化解位于尖锐的盆地中，初始时在低方差状态下不可访问。需要积累足够的梯度方差才能提升有效稳定性上限，进入该尖锐流形。各向同性噪声注入无法诱导grokking，泛化需要自适应优化器特有的各向异性整流。

Conclusion: grokking现象受AdamW的方差门控机制调控，泛化解位于尖锐盆地中，需要各向异性噪声而非各向同性噪声来诱导泛化。这挑战了"平坦最小值"假说在算法任务中的适用性。
...
### [108] [Bridging Local and Global Knowledge: Cascaded Mixture-of-Experts Learning for Near-Shortest Path Routing](https://arxiv.org/abs/2603.15541)
*Yung-Fu Chen,Anish Arora*

Main category: cs.LG

TL;DR: 提出Ca-MoE（级联专家混合）架构解决稀疏网络中路由问题，通过两级专家系统结合局部和全局特征，实现自适应推理和在线元学习，在稀疏网络中比基线提升29.1%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有基于局部特征的深度学习模型在密集欧几里得图中表现良好，但在稀疏网络中由于拓扑不规则性需要更广泛的结构感知，导致泛化能力不足。

Method: 采用模块化两级Ca-MoE架构：下层专家依赖局部特征，上层专家依赖全局特征；自适应推理机制仅在需要时触发上层专家；结合在线元学习策略，支持独立专家微调和稳定性更新机制防止灾难性遗忘。

Result: 在稀疏网络中比单专家基线提升29.1%准确率；在不同图密度下保持性能在理论上界1%-6%范围内；通过自适应推理实现计算效率，避免参数冗余。
...
### [109] [Multi-Axis Trust Modeling for Interpretable Account Hijacking Detection](https://arxiv.org/abs/2603.13246)
*Mohammad AL-Smadi*
...
### [110] [Deep Convolutional Architectures for EEG Classification: A Comparative Study with Temporal Augmentation and Confidence-Based Voting](https://arxiv.org/abs/2603.13261)
...
### [111] [The AI Fiction Paradox](https://arxiv.org/abs/2603.13545)
*Katherine Elkins*
...
### [112] [PA-Net: Precipitation-Adaptive Mixture-of-Experts for Long-Tail Rainfall Nowcasting](https://arxiv.org/abs/2603.13818)
...
### [113] [Punctuated Equilibria in Artificial Intelligence: The Institutional Scaling Law and the Speciation of Sovereign AI](https://arxiv.org/abs/2603.14664)
...
### [114] [Prompt Readiness Levels (PRL): a maturity scale and scoring framework for production grade prompt assets](https://arxiv.org/abs/2603.15044)
*Sebastien Guinard*
...
### [115] [Modeling Matches as Language: A Generative Transformer Approach for Counterfactual Player Valuation in Football](https://arxiv.org/abs/2603.15212)
...
### [116] [SCAN: Sparse Circuit Anchor Interpretable Neuron for Lifelong Knowledge Editing](https://arxiv.org/abs/2603.15226)
...
### [117] [SAGE: Multi-Agent Self-Evolution for LLM Reasoning](https://arxiv.org/abs/2603.15255)
...
### [118] [PMAx: An Agentic Framework for AI-Driven Process Mining](https://arxiv.org/abs/2603.15351)
...
### [119] [CRASH: Cognitive Reasoning Agent for Safety Hazards in Autonomous Driving](https://arxiv.org/abs/2603.15364)
...
### [120] [Unlocking the Value of Text: Event-Driven Reasoning and Multi-Level Alignment for Time Series Forecasting](https://arxiv.org/abs/2603.15452)
...
TL;DR: 提出受圣训启发的多轴信任建模框架，将圣训学中的五个信任维度转化为26个语义行为特征，结合轻量级时序特征，在账户劫持检测中显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 受古典圣训学中评估信息源可信度的启发，传统异常检测使用单一异常分数，而圣训学采用可解释的多维标准，这为解决现代账户信任评估问题提供了结构类比。

Method: 将五个信任轴（长期诚信、行为精确性、上下文连续性、累积声誉、异常证据）转化为26个语义行为特征，并引入轻量级时序特征捕捉连续活动窗口中的信任信号变化。

Result: 在CLUE-LDS数据集上，随机森林模型达到接近完美的检测性能，显著优于基于原始事件计数、统计基线和无监督异常检测的方法。在CERT内部威胁数据集上，时序特征将ROC-AUC从0.776提升到0.844，在4000用户配置中从0.627提升到0.715。

Conclusion: 圣训启发的多轴信任建模框架为账户信任评估提供了有效且可解释的方法，时序特征的加入在挑战性条件下显著提升了检测性能，验证了该框架的鲁棒性和实用性。
...
### [122] [Deep Convolutional Architectures for EEG Classification: A Comparative Study with Temporal Augmentation and Confidence-Based Voting](https://arxiv.org/abs/2603.13261)
*Aryan Patodiya,Hubert Cecotti*

Main category: cs.AI

TL;DR: 比较三种深度学习架构用于EEG事件相关电位分类：基于CSP的2D CNN、原始数据2D CNN和3D CNN，其中3D CNN结合时间偏移增强和置信度投票机制表现最佳。


<details>
  <summary>Details</summary>
Motivation: 脑电图分类在脑机接口中至关重要，但由于信号噪声比低、神经响应时间变异性和数据有限而具有挑战性。需要探索有效的深度学习架构来处理EEG信号中的事件相关电位分类问题。

Method: 设计并比较三种主要管道：1）使用CSP的2D CNN；2）直接在原始数据上训练的2D CNN；3）联合建模时空表示的3D CNN。为处理ERP延迟变化，引入时间偏移增强策略，并在推理时采用基于置信度的测试时间投票机制。

Result: 在分层五折交叉验证中，虽然CSP对2D架构有益，但提出的3D CNN在AUC和平衡准确率方面显著优于两种2D变体，证明了时间感知架构和增强策略的有效性。

Conclusion: 3D CNN架构结合时间偏移增强和置信度投票机制能够有效处理EEG信号中的时空特征，为稳健的EEG信号分类提供了有效解决方案。
...
### [123] [The AI Fiction Paradox](https://arxiv.org/abs/2603.13545)
*Katherine Elkins*

Main category: cs.AI

TL;DR: 论文提出"AI-小说悖论"：AI模型依赖海量小说数据训练却难以生成高质量小说，并识别了三个根本性挑战：叙事因果关系、信息价值重估、多尺度情感架构。


<details>
  <summary>Details</summary>
Motivation: AI发展面临"小说依赖问题"：模型需要大量现代小说数据训练，但生成能力却严重不足。这种悖论令人惊讶，因为在机器学习中，训练数据通常决定输出质量。作者旨在从理论上精确解释为什么小说抵抗AI生成。

Method: 通过理论分析识别AI生成小说的三个架构性挑战：1）叙事因果关系与transformer前向生成逻辑的根本冲突；2）信息价值重估挑战，小说违反信息重要性与统计显著性对齐的计算假设；3）基于七年情感弧线研究的发现，小说需要同时协调词、句、场景、弧线等多尺度情感架构。

Result: 三个挑战解释了为什么AI公司愿意冒数十亿美元诉讼风险获取现代小说数据，以及为什么这些小说仍然难以复制。分析揭示了当前transformer架构在生成叙事逻辑、注意力机制和情感协调方面的根本局限性。

Conclusion: 小说集中了独特强大的认知和情感模式来建模人类行为，AI系统掌握这些模式不仅代表创造性成就，更是大规模操纵人类的潜在工具。分析提出了当这些挑战被克服时的紧迫问题，暗示了AI掌握小说生成能力可能带来的社会风险。
...
### [124] [PA-Net: Precipitation-Adaptive Mixture-of-Experts for Long-Tail Rainfall Nowcasting](https://arxiv.org/abs/2603.13818)
*Xinyu Xiao,Sen Lei,Eryun Liu,Shiming Xiang,Hao Li,Cheng Yuan,Yuan Qi,Qizhao Jin*

Main category: cs.AI

TL;DR: PA-Net：基于降雨强度的自适应Transformer框架，通过动态激活专家数量处理极端降雨事件，解决降水临近预报中的计算成本和长尾分布问题。


<details>
  <summary>Details</summary>
Motivation: 降水临近预报面临两大瓶颈：1）多变量大气场百万级时空token建模的计算成本过高；2）降雨分布的极端长尾特性，其中对社会影响最大的暴雨事件仅占所有样本的不到0.1%。

Method: 提出降水自适应网络（PA-Net），其核心是降水自适应混合专家（PA-MoE），根据局部降雨强度动态调整每个token激活的专家数量；采用双轴压缩潜在注意力机制分解时空注意力；使用强度感知训练协议逐步增强极端降雨样本的学习信号。

Result: 在ERA5数据集上的实验表明，相对于最先进的基线方法取得了一致性改进，特别是在大雨和暴雨区域获得了显著提升。

Conclusion: PA-Net通过自适应计算分配有效解决了降水临近预报中的计算效率和长尾分布问题，为极端降雨事件的准确预测提供了有效解决方案。
...
### [125] [Punctuated Equilibria in Artificial Intelligence: The Institutional Scaling Law and the Speciation of Sovereign AI](https://arxiv.org/abs/2603.14664)
*Mark Baciak,Thomas A. Cellucci,Deanna M. Falkowski*

Main category: cs.AI

TL;DR: AI发展遵循间断平衡理论而非连续进步，模型规模与制度适应性呈非单调关系，小型领域适应模型在多数部署环境中能超越前沿通用模型。


<details>
  <summary>Details</summary>
Motivation: 挑战AI发展的主流叙事——认为进步是连续的且能力随模型规模单调增长。作者认为AI发展实际上遵循间断平衡理论，存在长期停滞和快速相变。

Method: 1. 应用进化生物学的间断平衡理论分析AI发展历史；2. 提出制度适应性流形数学框架，从能力、制度信任、可负担性和主权合规四个维度评估AI系统；3. 推导制度缩放定律，证明制度适应性在模型规模上是非单调的；4. 提供支持性实证证据。

Result: 1. 识别了自1943年以来的五个AI发展时代和当前生成式AI时代的四个纪元；2. 证明制度适应性在特定环境最优规模后随规模扩大而下降；3. 推导出协调的小型领域适应模型在多数制度部署环境中能数学上超越前沿通用模型的形式条件。

Conclusion: AI发展遵循间断平衡模式而非连续进步，制度适应性流形框架揭示了模型规模与部署成功之间的非单调关系，为领域适应模型在制度环境中的优势提供了理论依据，对AI发展战略具有重要启示。
...
TL;DR: 本文提出了Prompt Readiness Levels (PRL)和Prompt Readiness Score (PRS)框架，用于系统评估和治理提示工程资产，确保其满足运营目标、安全约束和合规要求。
...
Method: 提出PRL（九级成熟度量表，受TRL启发）和PRS（多维评分方法，具有门控阈值，防止薄弱环节故障模式）。

Result: PRL/PRS提供了一个原创的、结构化的方法论框架，用于治理提示资产的规范、测试、可追溯性、安全评估和部署准备。

Conclusion: 该框架使团队和行业能够通过可重复的资格决策来评估提示工程的价值，解决当前缺乏标准化评估方法的问题。
...
### [127] [Modeling Matches as Language: A Generative Transformer Approach for Counterfactual Player Valuation in Football](https://arxiv.org/abs/2603.15212)
*Miru Hong,Minho Lee,Geonhee Jo,Hyeokje Jo,Pascal Bauer,Sang-Ki Ko*
...
<details>
  <summary>Details</summary>
Motivation: 足球球员转会评估面临挑战，因为球员表现高度依赖战术体系、队友和比赛情境。现有方法主要依赖静态统计数据和主观专家判断，无法充分考虑这些情境因素，缺乏能够预测假设情景结果的反事实模拟机制。

Method: 提出ScoutGPT模型，采用基于NanoGPT的Transformer架构，通过下一标记预测训练学习比赛事件序列的动态特性。模型将足球比赛事件视为序列标记，在语言建模框架下工作，能够模拟假设阵容下的事件序列。利用蒙特卡洛采样进行反事实模拟，评估未观察到的情景。

Result: 在K联赛数据上的实验表明，ScoutGPT在预测性能上优于现有基线模型。模拟的球员转会导致进攻推进和进球概率的可测量变化，证明模型能够捕捉超越传统静态指标的球员特定影响。

Conclusion: ScoutGPT通过将足球比赛事件建模为序列标记，成功实现了反事实模拟能力，能够评估球员在假设阵容中的影响，为足球球员转会决策提供了超越传统静态指标的情境感知评估工具。
...
### [128] [SCAN: Sparse Circuit Anchor Interpretable Neuron for Lifelong Knowledge Editing](https://arxiv.org/abs/2603.15226)
*Yuhuan Liu,Haitian Zhong,Xinyuan Xia,Qiang Liu,Shu Wu,Liang Wang*

Main category: cs.AI

TL;DR: SCAN提出基于稀疏电路锚定神经元的稀疏编辑框架，通过稀疏转码器构建知识电路，解决LLM顺序知识编辑中的灾难性遗忘和模型崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在顺序知识编辑中经常遭受灾难性遗忘和模型崩溃，现有密集编辑范式将模型视为黑盒，依赖粗粒度参数干预，不可避免地破坏已保存知识。

Method: SCAN稀疏编辑框架基于稀疏电路锚定神经元，通过稀疏转码器构建知识电路，将编辑转化为机制感知的操作，实现精确的知识修改而不破坏其他知识。

Result: 在Gemma2、Qwen3和Llama3.1模型上的实验表明，SCAN在CounterFact、ZsRE和WikiFactDiff基准上表现优异，即使在3000次顺序编辑后仍能保持模型完整性，而其他方法随着编辑积累逐渐恶化最终导致模型崩溃。

Conclusion: SCAN通过稀疏电路编辑有效解决了LLM顺序知识编辑中的灾难性遗忘问题，实现了长期稳定的知识更新，为模型编辑提供了新的机制感知方法。
...
### [129] [SAGE: Multi-Agent Self-Evolution for LLM Reasoning](https://arxiv.org/abs/2603.15255)
*Yulin Peng,Xinxin Zhu,Chenxing Wei,Nianbo Zeng,Leilei Wang,Ying Tiffany He,F. Richard Yu*

Main category: cs.AI

TL;DR: SAGE是一个自演化的多智能体框架，通过四个角色（挑战者、规划者、求解者、批评者）从共享LLM主干协同演化，仅需少量种子数据就能实现稳定的长时程推理训练。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法依赖大量人工标注数据，而自对弈方法缺乏显式规划和质量控制，在长时程多步推理中稳定性不足。

Method: 提出SAGE框架：挑战者生成难度递增的任务，规划者将任务转化为结构化多步计划，求解者按计划生成答案，批评者评估和过滤生成的问题与计划以防止课程漂移。

Result: 在数学和代码生成基准测试中，SAGE在不同模型规模上均取得一致提升，将Qwen-2.5-7B模型在LiveCodeBench上提升8.9%，在OlympiadBench上提升10.7%。

Conclusion: SAGE通过闭环多智能体协同演化框架，仅需少量种子数据就能实现稳定高效的自训练，显著提升LLM在复杂推理任务上的性能。
...
### [130] [PMAx: An Agentic Framework for AI-Driven Process Mining](https://arxiv.org/abs/2603.15351)
*Anton Antonov,Humam Kourani,Alessandro Berti,Gyunam Park,Wil M. P. van der Aalst*

Main category: cs.AI

TL;DR: PMAx是一个基于多智能体架构的隐私保护过程挖掘框架，使用本地脚本执行精确计算，让非技术用户能用自然语言获取可靠的过程洞察。


<details>
  <summary>Details</summary>
Motivation: 传统过程挖掘需要专业知识，而直接使用大语言模型存在确定性推理困难和数据隐私问题，需要一种既能民主化过程挖掘又能保证准确性和隐私性的解决方案。

Method: 采用隐私保护的多智能体架构：工程师智能体分析事件日志元数据并自动生成本地脚本运行过程挖掘算法；分析师智能体解释结果并生成报告。将计算与解释分离，本地执行分析。

Result: PMAx确保数学准确性和数据隐私，同时使非技术用户能够将高层业务问题转化为可靠的过程洞察，解决了LLMs的幻觉问题和隐私担忧。

Conclusion: PMAx通过分离计算与解释、本地执行分析的多智能体框架，成功实现了过程挖掘的民主化，在保证准确性和隐私性的同时让非技术用户能够通过自然语言获取可靠洞察。
...
### [131] [CRASH: Cognitive Reasoning Agent for Safety Hazards in Autonomous Driving](https://arxiv.org/abs/2603.15364)
*Erick Silva,Rehana Yasmin,Ali Shoker*

Main category: cs.AI

TL;DR: 本文提出CRASH系统，利用LLM代理自动分析NHTSA自动驾驶事故报告，发现64%事故源于感知或规划故障，50%为追尾碰撞，验证准确率达86%。


<details>
  <summary>Details</summary>
Motivation: 随着自动驾驶系统架构日益复杂多样（端到端与模块化设计并存），事故调查缺乏标准化方法，难以系统化分析安全问题的根本原因。

Method: 构建包含2,168起事故（2021-2025年）的数据集，提出CRASH（Cognitive Reasoning Agent for Safety Hazards）系统，利用LLM代理结合结构化字段和非结构化叙述，自动生成摘要、归因主要责任、评估AV参与程度。

Result: 1) CRASH将64%事故归因于感知或规划故障；2) 约50%事故为追尾碰撞；3) 经5位专家验证，系统故障归因准确率达86%。

Conclusion: CRASH展示了作为可扩展、可解释的自动事故分析工具的潜力，能为自动驾驶安全研究和系统开发提供有价值的见解。
...
### [132] [Unlocking the Value of Text: Event-Driven Reasoning and Multi-Level Alignment for Time Series Forecasting](https://arxiv.org/abs/2603.15452)
*Siyuan Wang,Peng Chen,Yihang Wang,Wanghui Qiu,Chenjuan Guo,Bin Yang,Yang Shu*

Main category: cs.AI

TL;DR: VoT提出了一种利用文本信息进行时间序列预测的新方法，通过事件驱动推理和多层次对齐，结合LLMs的推理能力，显著提升了预测性能。
...
Method: 提出VoT方法，包含两个核心组件：1) 事件驱动推理：结合外生文本的丰富信息和LLMs的强大推理能力进行预测，采用历史上下文学习检索和应用历史示例作为上下文指导；2) 多层次对齐：在表示层面使用内生文本对齐将内生文本信息与时间序列整合，在预测层面设计自适应频率融合将事件驱动预测和数值预测的频率成分融合以实现优势互补。

Result: 在10个领域的真实世界数据集上的实验表明，相比现有方法有显著改进，验证了该方法在文本利用方面的有效性。

Conclusion: VoT通过充分利用文本信息，结合LLMs的推理能力和多层次对齐策略，有效提升了时间序列预测的性能，为多模态时间序列预测提供了新的解决方案。
...
### [133] [Design and evaluation of an agentic workflow for crisis-related synthetic tweet datasets](https://arxiv.org/abs/2603.13625)
...
### [135] [Creative Convergence or Imitation? Genre-Specific Homogeneity in LLM-Generated Chinese Literature](https://arxiv.org/abs/2603.14430)
...
### [136] [An Industrial-Scale Insurance LLM Achieving Verifiable Domain Mastery and Hallucination Control without Competence Trade-offs](https://arxiv.org/abs/2603.14463)
...
### [137] [OrgForge: A Multi-Agent Simulation Framework for Verifiable Synthetic Corporate Corpora](https://arxiv.org/abs/2603.14997)
*Jeffrey Flynt*
...
### [138] [When Does Sparsity Mitigate the Curse of Depth in LLMs](https://arxiv.org/abs/2603.15389)
...
### [139] [Design and evaluation of an agentic workflow for crisis-related synthetic tweet datasets](https://arxiv.org/abs/2603.13625)
*Roben Delos Reyes,Timothy Douglas,Asanobu Kitamoto*

Main category: cs.CL

TL;DR: 提出一种生成危机相关合成推文的智能工作流，以解决Twitter数据获取困难的问题，并应用于地震后损害评估任务。


<details>
  <summary>Details</summary>
Motivation: Twitter数据获取政策变化使得危机相关推文数据集难以收集，现有数据集局限于特定历史事件且标注成本高，限制了危机信息学AI系统的发展与评估。

Method: 开发智能工作流，迭代生成符合预设目标特征的合成推文，通过预定义合规性检查评估，并利用结构化反馈在后续迭代中优化。

Result: 工作流能生成捕获目标标签（位置和损害等级）的合成推文，生成的数据集可用于评估地理定位和损害等级预测等AI系统，效果良好。

Conclusion: 该工作流为真实推文数据收集提供了灵活可扩展的替代方案，能够跨不同危机事件、社会背景和危机信息学应用系统生成合成社交媒体数据。
...
### [140] [QuarkMedBench: A Real-World Scenario Driven Benchmark for Evaluating Large Language Models](https://arxiv.org/abs/2603.13691)
*Yao Wu,Kangping Yin,Liang Dong,Zhenxin Ma,Shuting Xu,Xuehai Wang,Yuxuan Jiang,Tingting Yu,Yunqing Hong,Jiayi Liu,Rianzhe Huang,Shuxin Zhao,Haiping Hu,Wen Shang,Jian Xu,Guanjun Jiang*

Main category: cs.CL

TL;DR: QuarkMedBench是一个面向真实世界医疗LLM评估的生态效度基准，包含20,821个单轮查询和3,853个多轮会话，通过自动评分框架生成220,617个细粒度评分标准，解决了传统选择题评估无法捕捉真实医疗查询复杂性的问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在标准化医学考试中表现出色，但高分往往无法转化为对真实世界医疗查询的高质量响应。现有评估主要依赖选择题，无法捕捉真实用户查询的非结构化、模糊性和长尾复杂性。

Method: 构建了包含临床护理、健康保健和专业咨询的大规模数据集，提出自动评分框架，整合多模型共识和基于证据的检索，动态生成细粒度评分标准，采用分层加权和安全约束结构化量化医疗准确性、关键点覆盖和风险拦截。

Result: 生成的评分标准与临床专家盲审达到91.8%的一致率，建立了高度可靠的医疗可靠性。基准评估显示，在应对真实世界临床细微差别时，最先进模型之间存在显著性能差异，突显了传统考试指标的局限性。

Conclusion: QuarkMedBench为衡量LLM在复杂健康问题上的表现建立了严格、可复现的标准，其框架支持动态知识更新以防止基准过时，为真实世界医疗LLM评估提供了更有效的工具。
...
### [141] [Creative Convergence or Imitation? Genre-Specific Homogeneity in LLM-Generated Chinese Literature](https://arxiv.org/abs/2603.14430)
*Yuanchi Ma,Kaize Shi,Hui He,Zhihua Zhang,Zhongxiang Lei,Ziliang Qiu,Renfen Hu,Jiamou Liu*

Main category: cs.CL

TL;DR: LLMs生成故事存在结构同质化问题，研究者提出结合普罗普叙事学的分析框架，通过定义34个适合网络文学叙事的叙事功能，发现LLMs无法正确理解叙事功能含义，导致僵化的叙事生成范式。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在叙事生成方面表现出色，但它们经常产生结构同质化的故事，遵循重复的情节安排和刻板结局。需要分析LLMs生成的叙事文本的底层叙事逻辑，以理解其同质化问题的根源。

Method: 提出结合普罗普叙事学和叙事功能的新理论分析框架，针对中文网络文学扩展普罗普叙事理论，定义了34个适合现代网络叙事结构的叙事功能。构建人工标注语料库来支持分析LLM生成文本的叙事结构。

Result: 实验发现，当前LLMs无法正确理解叙事功能的含义，而是遵循僵化的叙事生成范式，这是导致生成文本叙事逻辑单一和严重同质化的主要原因。

Conclusion: LLMs在叙事生成中存在结构性同质化问题，根源在于模型无法正确理解叙事功能，需要改进模型对叙事结构的理解能力。
...
### [142] [An Industrial-Scale Insurance LLM Achieving Verifiable Domain Mastery and Hallucination Control without Competence Trade-offs](https://arxiv.org/abs/2603.14463)
*Qian Zhu,Xinnan Guo,Jingjing Huo,Jun Li,Pan Liu,Wenyan Yang,Wanqing Xu,Xuan Lin*

Main category: cs.CL

TL;DR: INS-S1是一个保险领域专用的大语言模型家族，通过创新的端到端对齐范式训练，在保持通用智能的同时实现严格的领域专业化，达到0.6%的低幻觉率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在将大语言模型适配到保险等高风险垂直领域时面临挑战：需要严格遵守复杂法规和业务逻辑，但现有方法要么牺牲通用智能换取领域专业知识，要么过度依赖RAG而缺乏内在推理能力。

Method: 提出了两种方法创新：1）可验证数据合成系统，构建用于精算推理和合规性的分层数据集；2）渐进式SFT-RL课程框架，整合动态数据退火与验证推理（RLVR）和AI反馈（RLAIF）的协同组合。

Result: INS-S1在领域任务上达到最先进性能，显著优于DeepSeek-R1和Gemini-2.5-Pro，同时保持顶级通用能力，并创下0.6%的低幻觉率（HHEM）。

Conclusion: 研究表明，通过创新的训练范式，可以在不损害通用智能的情况下实现严格的领域专业化，为高风险垂直领域的LLM应用提供了可行方案。
...
### [143] [OrgForge: A Multi-Agent Simulation Framework for Verifiable Synthetic Corporate Corpora](https://arxiv.org/abs/2603.14997)
*Jeffrey Flynt*

Main category: cs.CL

TL;DR: OrgForge是一个开源多智能体模拟框架，用于生成具有严格时间一致性和事实一致性的组织文档数据，以解决现有RAG评估数据集的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有RAG评估数据集存在法律模糊性、人口统计偏差、缺乏结构化真实标签等问题。纯LLM生成的合成数据虽然解决了法律问题，但会导致模型在不同文档间产生自相矛盾的事实幻觉。

Method: 采用多智能体模拟框架，严格分离物理-认知边界：确定性Python引擎维护SimEvent真实总线，LLM仅生成表面文本并受验证提案约束。使用演员本地时钟确保跨文档时间戳因果正确性，并实现三个图动态子系统（中介中心性压力传播、时间边权重衰减、Dijkstra升级路由）来独立于LLM控制组织行为。

Result: OrgForge能够生成可配置N天模拟，产生交错的Slack线程、JIRA工单、Confluence页面、Git拉取请求和电子邮件，所有这些都可追溯到共享的不可变事件日志。还包含因果链跟踪系统、混合互惠排名融合重复故障检测器以及带概率丢弃模拟的入站/出站电子邮件引擎。

Conclusion: OrgForge提供了一个开源解决方案，能够生成具有严格时间一致性和事实一致性的组织文档数据，解决了现有RAG评估数据集的局限性，为评估检索增强生成系统提供了更可靠的测试基础。
...
### [144] [When Does Sparsity Mitigate the Curse of Depth in LLMs](https://arxiv.org/abs/2603.15389)
*Dilxat Muhtar,Xinyuan Song,Sebastian Pokutta,Max Zimmer,Nico Pelleriti,Thomas Hofmann,Shiwei Liu*

Main category: cs.CL

TL;DR: 稀疏性（包括隐式和显式）通过调节方差传播，缓解大语言模型中的深度诅咒问题，提高深层利用率，最终提出实用训练方法使下游任务准确率提升4.6%。


<details>
  <summary>Details</summary>
Motivation: 近期研究发现大语言模型存在深度诅咒问题，深层对学习和表示的贡献较小，这种利用不足与Pre-Layer Normalization中方差累积增长有关，导致深层块接近恒等行为。本文旨在探索稀疏性如何调节方差传播从而改善深度利用。

Method: 研究覆盖两种稀疏性来源：(1) 隐式稀疏性：来自训练和数据条件，包括权重衰减诱导的权重稀疏性和长上下文输入诱导的注意力稀疏性；(2) 显式稀疏性：通过架构设计强制执行，包括分组查询注意力中的键/值共享稀疏性和混合专家模型中的专家激活稀疏性。通过受控深度扩展实验和针对性层有效性干预来验证。

Result: 在所有设置中观察到一致关系：稀疏性通过减少输出方差和促进功能分化来提高层利用率。最终将发现提炼为训练深度有效大语言模型的实用经验法则，在下游任务上实现了4.6%的准确率提升。

Conclusion: 稀疏性作为标准设计选择中自然产生的机制，是大语言模型中有效深度扩展的关键但先前被忽视的因素。研究揭示了稀疏性通过调节方差传播缓解深度诅咒，提高模型深度利用效率。
...
### [145] [GraspADMM: Improving Dexterous Grasp Synthesis via ADMM Optimization](https://arxiv.org/abs/2603.13832)
...
### [146] [See, Learn, Assist: Safe and Self-Paced Robotic Rehabilitation via Video-Based Learning from Demonstration](https://arxiv.org/abs/2603.14160)
...
### [147] [eNavi: Event-based Imitation Policies for Low-Light Indoor Mobile Robot Navigation](https://arxiv.org/abs/2603.14397)
...
### [148] [Latent Dynamics-Aware OOD Monitoring for Trajectory Prediction with Provable Guarantees](https://arxiv.org/abs/2603.14603)
*Tongfei Guo,Lili Su*
...
### [149] [LiDAR-EVS: Enhance Extrapolated View Synthesis for 3D Gaussian Splatting with Pseudo-LiDAR Supervision](https://arxiv.org/abs/2603.14763)
...
### [150] [BodyGuards: Escorting by Multiple Robots in Unknown Environment under Limited Communication](https://arxiv.org/abs/2603.15108)
...
### [151] [From Passive Observer to Active Critic: Reinforcement Learning Elicits Process Reasoning for Robotic Manipulation](https://arxiv.org/abs/2603.15600)
...
### [152] [GraspADMM: Improving Dexterous Grasp Synthesis via ADMM Optimization](https://arxiv.org/abs/2603.13832)
*Liangwang Ruan,Jiayi Chen,He Wang,Baoquan Chen*

Main category: cs.RO

TL;DR: 提出GraspADMM框架，通过ADMM分解优化，在保持抓取多样性的同时，显著提升运动学可行性和动态稳定性，实现比现有方法更高的抓取成功率。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：Dexonomy框架依赖固定接触点限制了手部可达性，无法优化动态稳定性；而纯梯度优化器虽能最大化动态稳定性，但使用简化接触近似会导致物理穿透。需要一种能同时保证多样性、运动学可行性和动态稳定性的抓取合成方法。

Method: 提出GraspADMM框架，采用交替方向乘子法(ADMM)将细化阶段分解：1) 更新目标物体接触点以直接最大化动态抓取指标；2) 调整手部姿势以物理到达这些目标点，同时严格尊重碰撞边界。这种分解允许在保持采样多样性的同时优化动态稳定性。

Result: 在广泛实验中，GraspADMM显著优于现有基线方法：在类型无关合成中实现近15%的绝对抓取成功率提升，在类型感知合成中实现约100%的相对改进。即使在极端低摩擦条件下也能保持鲁棒的物理合理抓取生成。

Conclusion: GraspADMM通过ADMM分解有效桥接了现有方法的差距，在保持多样性的同时显著提升了运动学可行性和动态稳定性，为机器人灵巧抓取合成提供了更优的解决方案。
...
### [153] [See, Learn, Assist: Safe and Self-Paced Robotic Rehabilitation via Video-Based Learning from Demonstration](https://arxiv.org/abs/2603.14160)
*Ali Alabbas,Camillo Murgia,Joanne Regan,Philip Long*

Main category: cs.RO

TL;DR: 提出远程机器人辅助康复框架，通过RGB-D视频编码治疗师演示，使用6-DoF轨迹和混合控制架构实现安全、自适应的康复训练。


<details>
  <summary>Details</summary>
Motivation: 解决远程康复治疗中治疗师无法现场指导的问题，需要适应不同患者解剖结构差异，同时确保训练过程的安全性和适应性。

Method: 1) 使用RGB-D视频编码治疗师演示为6-DoF身体中心轨迹；2) 采用解耦混合控制架构构建空间合规虚拟隧道；3) 基于努力的时域扩展机制；4) 动态链接执行阶段与患者切向力贡献；5) 使用GMR模型实时学习患者肢体特征检测异常力。

Result: 平均轨迹复制误差3.7cm，运动范围误差5.5度；动态交互试验证实控制器成功实施基于努力的进展，同时在人类干扰下保持严格空间路径遵循。

Conclusion: 该框架能够实现精确、安全、自适应的远程机器人辅助康复，支持多种康复模式，并能有效防止患者受伤。
...
### [154] [eNavi: Event-based Imitation Policies for Low-Light Indoor Mobile Robot Navigation](https://arxiv.org/abs/2603.14397)
*Prithvi Jai Ramesh,Kaustav Chanda,Krishna Vinod,Joseph Raj Vishal,Yezhou Yang,Bharatesh Chakravarthi*

Main category: cs.RO

TL;DR: 提出首个真实世界室内人跟随数据集eNavi，包含同步的事件流、RGB帧和专家控制动作，并开发了基于MobileNet编码器和Transformer融合模块的RGB-Event导航策略，在低光条件下表现优于RGB-only模型。


<details>
  <summary>Details</summary>
Motivation: 事件相机具有高动态范围和微秒级时间分辨率，适合室内机器人导航，但现有研究缺乏利用事件流异步特性的端到端控制策略。需要真实世界数据集来推动事件相机在导航任务中的应用。

Method: 1) 收集TurtleBot 2机器人的真实世界室内人跟随数据集，包含同步的原始事件流、RGB帧和专家控制动作；2) 构建多模态数据预处理流程，时间对齐事件和RGB观测，从里程计重建地面真实动作；3) 提出基于MobileNet双编码器和Transformer融合模块的RGB-Event导航策略，通过行为克隆训练。

Result: 在12种训练变体（从单路径模仿到多路径模仿）的系统评估中，融合事件数据的策略，特别是RGB-Event融合模型，在未见过的低光条件下表现出更好的鲁棒性和更低的动作预测误差，而RGB-only模型在此条件下失效。

Conclusion: 事件相机数据显著提升了室内导航策略的鲁棒性，特别是在低光条件下。提出的eNavi数据集、同步流程和训练模型为事件相机在机器人导航中的应用提供了重要资源。
...
### [155] [Latent Dynamics-Aware OOD Monitoring for Trajectory Prediction with Provable Guarantees](https://arxiv.org/abs/2603.14603)
*Tongfei Guo,Lili Su*

Main category: cs.RO

TL;DR: 提出基于隐马尔可夫模型和累积最大均值差异的快速变点检测方法，用于轨迹预测中的分布外事件监测，在三个真实驾驶数据集上验证了延迟降低和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在安全关键的网络物理系统中，轨迹预测的可靠性在分布外场景下会下降，而现有方法难以检测不断变化的交通条件，且安全关键应用需要形式化的检测延迟和误报率保证。

Method: 将分布外监测建模为快速变点检测问题，利用隐马尔可夫模型建模分布内预测误差演化，扩展累积最大均值差异方法实现无需后变化分布知识的检测。

Result: 在三个真实驾驶数据集上的实验表明，该方法减少了检测延迟，对重尾误差和未知后变化条件具有鲁棒性。

Conclusion: 提出的基于隐马尔可夫模型的快速变点检测框架为轨迹预测中的分布外监测提供了理论保证，在真实驾驶场景中表现出优越性能。
...
### [156] [LiDAR-EVS: Enhance Extrapolated View Synthesis for 3D Gaussian Splatting with Pseudo-LiDAR Supervision](https://arxiv.org/abs/2603.14763)
*Yiming Huang,Xin Kang,Sipeng Zhang,Hongliang Ren,Weihua Zhang,Junjie Lai*

Main category: cs.RO

TL;DR: LiDAR-EVS：一个轻量级框架，用于自动驾驶中鲁棒的外推视角LiDAR模拟，通过伪外推视角点云监督和空间约束dropout正则化解决3DGS在训练轨迹外泛化差的问题。
...
Method: 提出LiDAR-EVS框架，包含两个核心组件：1) 伪外推视角点云监督，通过多帧LiDAR融合、视角变换、遮挡剔除和强度调整；2) 空间约束dropout正则化，增强对真实驾驶中轨迹变化的鲁棒性。

Result: 在三个数据集上的实验表明，LiDAR-EVS在外推视角LiDAR合成方面达到了最先进的性能，且框架轻量、即插即用，可扩展到不同LiDAR传感器和神经渲染基线。

Conclusion: LiDAR-EVS是自动驾驶中数据驱动模拟、闭环评估和合成数据生成的有前景工具，解决了3DGS在外推视角LiDAR模拟中的泛化挑战。
...
### [157] [BodyGuards: Escorting by Multiple Robots in Unknown Environment under Limited Communication](https://arxiv.org/abs/2603.15108)
*Zhuoli Tian,Yanze Bao,Meng Guo*
...
<details>
  <summary>Details</summary>
Motivation: 多机器人系统在高风险任务中部署增多，但在通信受限的未知对抗环境中保护人类操作员仍具挑战。现有协同探索方法追求完全覆盖，而本文关注任务导向探索以最小化操作员导航时间并确保安全。
...
Conclusion: BodyGuards框架通过任务导向探索和协同护送策略，在通信受限的对抗环境中有效保护人类操作员并优化任务完成时间，为高风险任务中的多机器人系统提供实用解决方案。
...
### [158] [From Passive Observer to Active Critic: Reinforcement Learning Elicits Process Reasoning for Robotic Manipulation](https://arxiv.org/abs/2603.15600)
*Yibin Liu,Yaxing Lyu,Daqi Gao,Zhixuan Liang,Weiliang Tang,Shilong Mu,Xiaokang Yang,Yao Mu*

Main category: cs.RO

TL;DR: PRIMO R1是一个7B参数框架，将视频MLLM从被动观察者转变为主动批评者，通过强化学习激励显式的思维链生成进行进度估计，在机器人操作监控任务上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前视频MLLM主要基于监督微调训练，只能作为被动"观察者"识别事件，而无法评估当前状态相对于最终任务目标的进展，这限制了长时程机器人操作的准确过程监督。

Method: 1) 使用基于结果的强化学习激励显式思维链生成进行进度估计；2) 构建结构化时序输入，通过初始状态和当前状态图像明确锚定视频序列；3) 提出PRIMO数据集和基准进行评估。

Result: 1) 7B模型在专业推理基线上平均绝对误差减少50%；2) 在72B规模通用MLLM上实现显著相对精度提升；3) 在RoboFail基准上达到67.0%准确率，超越OpenAI o1 6.0%；4) 在困难故障检测任务上展示强零样本泛化能力。

Conclusion: PRIMO R1成功将视频MLLM从被动观察者转变为主动批评者，通过强化学习驱动的思维链生成实现了准确的过程监督，在机器人操作监控任务上建立了新的SOTA性能，并展示了良好的泛化能力。
