...
TL;DR: DSA-SRGS：首个用于动态稀疏视图DSA重建的超分辨率高斯溅射框架，通过多保真度纹理学习和辐射亚像素致密化技术，显著提升血管细节重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯溅射和动态神经表示的3D血管重建方法受限于输入投影分辨率，简单上采样会导致严重模糊和伪影，缺乏超分辨率能力限制了在精准诊疗中的应用。

Method: 提出多保真度纹理学习模块，集成微调的DSA专用超分辨率模型的高质量先验；采用置信感知策略自适应加权低分辨率投影与高分辨率伪标签的监督信号；开发辐射亚像素致密化策略，利用高分辨率亚像素采样的梯度积累优化4D辐射高斯核。

Result: 在两个临床DSA数据集上的大量实验表明，DSA-SRGS在定量指标和定性视觉保真度上均显著优于最先进方法。

Conclusion: DSA-SRGS成功解决了动态稀疏视图DSA重建中的超分辨率问题，能够恢复细粒度血管细节和复杂分支结构，为精准诊断和治疗提供了更好的4D血管模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有隐私保护方法（物理脱敏、加密、模糊化）要么损害语义理解能力，要么无法保证数学上可证明的不可逆性。Privacy Camera 1.0虽然从源头消除视觉数据，但仅提供文本判断，在争议中存在证据盲区。

Method: 采用AI Flow范式和边云协同架构，在边缘部署视觉脱敏器，通过非线性映射和随机噪声注入将原始图像转换为抽象特征向量（基于信息瓶颈原理），确保身份敏感信息被剥离且原始图像数学上不可重建。抽象表示传输到云端，通过"动态轮廓"视觉语言进行行为识别和语义重建。

Result: 实现了隐私保护与感知能力的平衡：原始图像在数学上不可重建，身份敏感信息被剥离，同时云端能够进行行为识别和生成说明性视觉参考而不暴露原始图像。

Conclusion: 该框架解决了隐私-安全悖论，在高度敏感环境中实现了既保护隐私又保持感知能力的目标，通过边云协同和AI Flow范式为智能感知系统提供了新的隐私保护解决方案。
...
TL;DR: 该论文提出两种网络适配器来解决医学图像分析中的空间不平衡问题：LAW用于提升扩散模型生成质量，ORDER用于高效分割，在息肉和肾肿瘤数据集上均取得显著效果。


<details>
  <summary>Details</summary>
Motivation: 医学图像分析面临空间不平衡问题：病灶区域小而背景区域大。扩散模型在生成时容易偏离指定的病灶布局，而高效分割器在空间不确定区域表现不佳。需要自适应空间加权来合理分配计算资源。

Method: 提出两种网络适配器：1) LAW（可学习自适应加权器）：从特征和掩码预测逐像素损失调制，通过归一化、截断和正则化稳定训练；2) ORDER（高效分辨率最优区域检测）：在解码器后期应用选择性双向跳跃注意力进行高效分割。
...
Conclusion: 提出的LAW和ORDER适配器有效解决了医学图像分析中的空间不平衡问题，既提升了扩散模型的生成质量，又实现了高效准确的分割，为医学图像分析提供了实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有后训练量化方法（如SmoothQuant）在单模态LLMs上表现良好，但在多模态大语言模型（MLLMs）中存在两个关键问题：平滑错位和跨模态计算不变性，导致量化性能下降。
...
Conclusion: MASQuant有效解决了MLLMs量化中的关键挑战，为多模态模型的部署提供了高效的量化解决方案。
...
TL;DR: GloSplat是一个在3D高斯泼溅训练中执行联合姿态-外观优化的框架，通过保留显式SfM特征轨迹作为一等实体，结合重投影损失和光度监督，实现更精确的姿态估计和场景重建。


<details>
  <summary>Details</summary>
Motivation: 传统方法将特征提取、匹配、运动恢复结构和新视角合成作为独立问题处理，各自优化目标不同。现有联合优化方法（如BARF、NeRF--、3RGS）仅依赖光度梯度进行姿态细化，缺乏几何锚点，容易导致早期姿态漂移。

Method: GloSplat将显式SfM特征轨迹作为训练过程中的一等实体：轨迹3D点作为独立于高斯基元的可优化参数，通过重投影损失与光度监督并行工作。提出两种变体：GloSplat-F（无COLMAP，使用检索式对选择）和GloSplat-A（穷举匹配，追求最高质量）。两者都采用全局SfM初始化，然后在3DGS训练期间进行联合光度-几何优化。

Result: 实验表明，GloSplat-F在无COLMAP方法中达到最先进水平，而GloSplat-A超越了所有基于COLMAP的基线方法。

Conclusion: 通过将显式特征轨迹整合到3D高斯泼溅训练中，GloSplat实现了更鲁棒的联合姿态-外观优化，克服了纯光度方法的局限性，在效率和精度上都取得了显著提升。
...
TL;DR: 通过单目3D姿态序列分类8种投球类型，仅使用身体运动学数据，不使用球飞行数据，在119,561个专业投球上达到80.4%准确率，揭示了身体各部位对预测的贡献比例。


<details>
  <summary>Details</summary>
Motivation: 研究投手身体姿态能多大程度上揭示即将投出的球类型，探索仅通过身体运动学数据（不使用球飞行数据）进行投球分类的可能性，为棒球分析提供新的视角。

Method: 采用基于扩散的3D姿态估计作为骨干网络，结合自动投球事件检测、经过地面实况验证的生物力学特征提取，以及基于229个运动学特征的梯度提升分类器。

Result: 在119,561个专业投球（迄今最大基准）上达到80.4%的准确率。重要性分析显示：上半身贡献64.9%预测信号，下半身35.1%；手腕位置是最具信息量的关节组（14.8%），躯干侧倾是最具信息量的生物力学特征。

Conclusion: 仅通过姿态数据，握法定义的变体（四缝线vs.二缝线快速球）无法区分，表明运动学信息的上限约为80%，这界定了运动学信息与球飞行信息的边界。
...
TL;DR: 提出AdaIAT方法，通过自适应调整对生成文本的关注来减少大视觉语言模型的幻觉问题，同时避免重复描述


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型中的幻觉问题严重阻碍了其发展和应用，现有方法直接增加对图像token的关注虽然能减少幻觉但会导致重复描述

Method: 提出Attention to Generated Text (IAT)方法，利用生成文本中的视觉信息和上下文知识来缓解幻觉；进一步提出Adaptive IAT (AdaIAT)，采用层间阈值控制干预时机，并根据每个注意力头的特性进行细粒度放大

Result: AdaIAT显著减少幻觉率（在LLaVA-1.5上分别降低C_S和C_I指标35.8%和37.1%），同时保持语言性能和预测能力，实现良好平衡

Conclusion: 通过分析注意力模式发现真实对象token对生成文本的关注更高，利用这一特性提出的AdaIAT方法能有效缓解幻觉问题，同时避免重复描述，为大视觉语言模型的幻觉缓解提供了有效解决方案
...
TL;DR: BiEvLight：一种用于低光图像增强的双层优化框架，通过梯度引导的事件去噪先验和任务感知的协同优化，解决事件相机中的双重退化问题。
...
Method: 提出分层任务感知框架BiEvLight：1）利用图像与事件的强梯度相关性构建梯度引导的事件去噪先验；2）将事件去噪重新构建为受增强任务约束的双层优化问题，通过跨任务交互学习针对增强目标的事件表示。

Result: 在真实噪声数据集SDE上的实验表明，该方法显著优于现有SOTA方法，PSNR平均提升1.30dB，PSNR*提升2.03dB，SSIM提升0.047。

Conclusion: 精确的事件去噪是释放事件融合潜力的前提，BiEvLight通过协同优化增强和去噪任务，有效解决了双重退化问题，显著提升了低光图像增强质量。
...
TL;DR: TAPFormer：基于Transformer的帧-事件异步融合框架，用于鲁棒高频任意点跟踪，通过瞬态异步融合和跨模态局部加权融合解决模态失配问题


<details>
  <summary>Details</summary>
Motivation: 现有RGB帧与事件流融合方法通常依赖同步或非自适应融合，导致时间错位和模态失效时的性能严重下降，需要更鲁棒的点跟踪解决方案

Method: 提出TAPFormer框架：1）瞬态异步融合（TAF）机制，通过连续事件更新建模离散帧间的时间演化；2）跨模态局部加权融合（CLWF）模块，根据模态可靠性自适应调整空间注意力

Result: 在真实世界帧-事件TAP数据集上，平均像素误差在阈值内提升28.2%；在标准点跟踪基准测试中持续达到最佳性能

Conclusion: TAPFormer通过异步时间一致的帧-事件融合，实现了鲁棒且高频的任意点跟踪，解决了现有方法的时间错位和模态失效问题
...
<details>
  <summary>Details</summary>
Motivation: 尽管生成式图像修复在感知真实性方面取得了显著进展，但研究者希望了解其实际能力相比传统方法究竟提升了多少，以及当前模型存在哪些关键性能差距和失败模式。

Method: 提出了一个基于多维度评估的大规模研究框架，评估细节、锐度、语义正确性和整体质量；分析了包括扩散模型、GAN模型、PSNR导向模型和通用生成模型在内的多种架构；并利用基准训练了新的图像质量评估模型。

Result: 研究发现生成式图像修复领域出现了关键范式转变：核心挑战从过去的细节不足（欠生成）转变为细节质量和语义控制（防止过生成）；揭示了不同架构模型之间的关键性能差距；训练的新IQA模型能更好地与人类感知判断对齐。

Conclusion: 这项工作系统性地研究了现代生成式图像修复模型，提供了重新理解其真实状态的关键见解，并为未来发展指明了方向：需要从单纯追求细节丰富度转向控制细节质量和语义准确性。
...
TL;DR: 提出一个自适应VLA框架，根据任务复杂度动态路由执行路径（Act/Think/Abstain），使用视觉嵌入检测任务复杂度，实现高效资源分配和不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型通过推理技术提升泛化能力，但会增加计算复杂度和推理延迟，且资源分配不高效，缺乏对分布外任务的失败预防机制。

Method: 将VLA的视觉-语言主干转换为主动检测工具，通过将潜在嵌入投影到参数化和非参数化估计器集合中，根据感知状态复杂度动态路由执行：已知任务立即执行（Act），模糊场景推理（Think），显著异常时停止执行（Abstain）。

Result: 在LIBERO和LIBERO-PRO基准测试及真实机器人上，仅使用视觉嵌入的配置仅需5%训练数据即可达到80% F1分数，成为可靠高效的任务复杂度检测器。

Conclusion: 提出的自适应框架能有效平衡VLA模型的效率和安全性，通过视觉嵌入检测任务复杂度实现智能资源分配，为实际部署提供实用解决方案。
...
TL;DR: 提出STD-4D扩散模型，通过正交时空分布转移机制从3D和视频扩散模型中迁移先验知识，结合时空感知HexPlane，实现高质量4D内容生成
...
Method: 提出STD-4D扩散模型，通过解耦的时空潜在变量合成4D感知视频；设计正交时空分布转移机制，精心建模并注入时空特征分布；开发时空感知HexPlane，整合转移的时空特征以改进4D变形和4D高斯特征建模

Result: 实验表明，该方法显著优于现有方法，实现了更好的时空一致性和更高质量的4D合成

Conclusion: 通过从现有3D和视频扩散模型迁移丰富的空间和时间先验，提出的框架有效解决了4D数据集缺乏的问题，推动了高质量4D内容生成的发展
...
TL;DR: BLINK是一个基于轨迹的循环状态空间模型，用于建模自然杀伤细胞与肿瘤细胞的相互作用动力学，通过学习部分观测的交互序列来预测细胞毒性结果。


<details>
  <summary>Details</summary>
Motivation: 自然杀伤细胞的细胞毒性是细胞相互作用动力学的重要例子，通常使用时序多通道荧光显微镜研究。虽然可以在单帧中标注肿瘤细胞死亡事件，但细胞毒性结果随时间从细胞相互作用中产生，仅靠逐帧分类无法可靠推断。

Method: BLINK是一个基于轨迹的循环状态空间模型，作为NK-肿瘤相互作用的世界模型。它从部分观测的NK-肿瘤相互作用序列中学习潜在相互作用动力学，并预测累积成细胞毒性结果的凋亡增量。

Result: 在长期延时NK-肿瘤记录上的实验显示，BLINK改进了细胞毒性结果检测，并能够预测未来结果，同时提供了可解释的潜在表示，将NK轨迹组织成一致的行为模式和时间结构化的相互作用阶段。
...
TL;DR: UniPAR是一个统一的Transformer框架，通过统一数据调度和动态分类头，能够同时处理RGB图像、视频序列和事件流等多种模态的PAR任务，在多数据集联合训练中表现出优越的跨域泛化能力。
...
Conclusion: UniPAR成功打破了传统"一个模型对应一个数据集"的限制，为处理多模态、多域的行人属性识别任务提供了统一的解决方案，具有优越的泛化能力和实际应用价值。
...
Method: 提出SAIL方法：1）通过跨模态对齐构建语义感知掩码，使用相似性感知训练目标引导掩码关注与事件字幕高度相似的视频区域；2）引入基于LLM的增强策略生成合成字幕，提供额外的对齐信号；3）通过掩码间机制整合合成字幕，为精确时间定位提供辅助指导。
...
TL;DR: 提出一个可解释的集成学习框架，结合多种机器学习算法和深度神经网络，用于阿尔茨海默病的早期准确分类，并通过SHAP等技术提供模型可解释性。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病早期检测困难，需要准确且可解释的诊断方法。现有方法在可解释性和临床适用性方面存在不足，需要开发既高性能又透明的预测框架。

Method: 使用结构化临床、生活方式和代谢特征，采用严格预处理、高级特征工程、SMOTE-Tomek混合类别平衡技术，集成五种算法（随机森林、XGBoost、LightGBM、CatBoost、Extra Trees）和深度人工神经网络，通过分层验证防止数据泄露。

Result: 集成方法优于深度学习，XGBoost、随机森林和软投票集成表现最佳，在准确率、敏感性和F1分数方面表现优异。可解释性分析显示MMSE、功能评估年龄和多个工程交互特征是最重要的预测因子。

Conclusion: 该框架为阿尔茨海默病预测提供了可靠且透明的解决方案，具有强大的临床决策支持应用潜力，特别是在需要可解释性的医疗诊断场景中。
...
TL;DR: 本文揭示了预训练模型对不可学习示例(UEs)的免疫性，并提出BAIT方法通过双层优化绑定扰动与错误标签，有效克服预训练先验的影响


<details>
  <summary>Details</summary>
Motivation: 不可学习示例(UEs)作为一种数据保护策略，通过添加不可察觉的扰动使模型学习虚假相关性而非底层语义。然而研究发现，当从预训练模型开始学习时，即使数据受到精心设计的扰动保护，预训练先验仍能提供丰富的语义表示，使模型能够绕过UEs引入的捷径并捕获真实特征，从而抵消不可学习性。

Method: 提出BAIT（绑定人工扰动到错误目标）方法，采用双层优化公式：内层将扰动样本与真实标签关联以模拟标准数据-标签对齐；外层主动破坏这种对齐，通过强制执行误标签-扰动绑定，将样本映射到指定的错误目标。这种机制有效覆盖了先验的语义指导，迫使模型依赖注入的扰动，从而防止获取真实语义。

Result: 在标准基准测试和多个预训练骨干网络上进行的广泛实验表明，BAIT能有效减轻预训练先验的影响，并保持数据的不可学习性。

Conclusion: BAIT方法通过创新的双层优化框架，成功解决了预训练模型对不可学习示例的免疫问题，为数据保护提供了更可靠的解决方案。
...
Result: 在三个大型独立数据集（SHHS、CFS、MrOS；总n=9,815）上评估，KindSleep在估计AHI评分方面表现出色（R2=0.917，ICC=0.957），在OSA严重程度分类方面始终优于现有方法，在不同人群中加权F1分数达到0.827至0.941。
...
TL;DR: 研究评估了三种基于LSTM、GRU和Transformer的序列模型，结合蒙特卡洛dropout或证据输出层进行不确定性量化，用于1型糖尿病血糖预测和不良血糖事件识别。Transformer模型配合证据输出头表现最佳，提供更准确的预测和更好的校准不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 将原则性不确定性量化整合到实时机器学习血糖预测系统中，提高预测准确性并识别不良血糖事件，为1型糖尿病管理提供更可靠的决策支持。

Method: 使用基于LSTM、GRU和Transformer的三种序列模型家族，通过蒙特卡洛dropout或Deep Evidential Regression兼容的证据输出层实现不确定性量化。在HUPA-UCM糖尿病数据集上进行验证，并使用糖尿病技术学会误差网格评估临床风险。

Result: 配备证据输出头的Transformer模型提供了最有效的不确定性感知框架，实现了持续更高的预测准确性和更好的校准不确定性估计，其不确定性大小与预测误差显著相关。

Conclusion: 将原则性不确定性量化整合到实时机器学习血糖预测系统中具有重要价值，能够提高预测可靠性并为临床决策提供更好的支持。
...
TL;DR: 本文提出了IAENet，一个基于Transformer的多标签学习框架，用于术中不良事件的早期预警，通过改进的TAFiLM模块融合静态和动态数据，并使用LCRLoss解决类别不平衡问题，在多个时间窗口上显著提升了预测性能。


<details>
  <summary>Details</summary>
Motivation: 术中不良事件的早期预警对降低手术风险和改善患者安全至关重要。现有深度学习方法存在三个主要问题：忽略不良事件之间的依赖关系、未能充分利用异构临床数据、以及医疗数据集中固有的类别不平衡问题。

Method: 1. 构建了首个多标签不良事件数据集(MuAE)，涵盖六个关键事件；2. 提出了基于Transformer的多标签学习框架(IAENet)，包含改进的TAFiLM模块用于静态协变量和动态变量的鲁棒融合及复杂时间依赖建模；3. 引入了带有共现正则化的标签约束重加权损失(LCRLoss)，有效缓解事件内不平衡并增强频繁共现事件间的结构化一致性。

Result: 在5、10和15分钟的早期预警任务中，IAENet始终优于强基线方法，平均F1分数分别提升了+5.05%、+2.82%和+7.57%。这些结果突显了IAENet在支持临床实践中智能术中决策的潜力。

Conclusion: IAENet通过有效融合异构临床数据、建模事件间依赖关系以及解决类别不平衡问题，显著提升了术中不良事件的预测性能，为临床智能决策支持提供了有前景的解决方案。
...
TL;DR: 该论文提出了"模型医学"这一新研究领域，将AI模型视为类似生物有机体的实体，建立了包括基础模型科学、临床模型科学、模型公共卫生和模型架构医学在内的完整学科体系，并开发了多种诊断工具和框架。


<details>
  <summary>Details</summary>
Motivation: 当前AI可解释性研究主要停留在"解剖观察"层面，而复杂AI系统需要更系统化的临床实践。作者希望建立类似医学的完整学科体系，为AI模型的诊断、治疗和预防提供系统化方法。

Method: 提出了15个子学科的学科分类法；开发了四壳模型行为遗传学框架；创建了神经MRI诊断工具；设计了五层诊断框架；建立了临床模型科学体系包括模型气质指数、模型症状学和标准化病例报告系统。

Result: 建立了完整的模型医学学科体系，开发了多种实用工具和框架，包括基于720个代理和24,923个决策验证的四壳模型，以及经过四个临床案例验证的神经MRI诊断工具。

Conclusion: 模型医学为AI系统提供了类似医学的系统化临床实践框架，填补了当前可解释性研究与实际应用之间的空白，为AI模型的诊断、治疗和预防奠定了理论基础和实践工具。
...
TL;DR: EchoGuard是一个基于知识图谱的智能体框架，通过结构化记忆和模式检测帮助用户识别操纵性沟通（如煤气灯效应、情感勒索等），采用日志-分析-反思循环引导用户自我发现。


<details>
  <summary>Details</summary>
Motivation: 现有AI系统缺乏结构化、长期记忆来追踪操纵性沟通的微妙、上下文相关策略，受限于有限的上下文窗口和灾难性遗忘问题。操纵性沟通（如煤气灯效应、情感勒索）难以被个体识别，需要更有效的检测和支持系统。

Method: 引入EchoGuard框架，使用知识图谱作为智能体的核心情景和语义记忆。采用结构化日志-分析-反思循环：1) 用户记录互动，系统构建个人情景知识图谱；2) 执行复杂图查询检测六种心理学基础的操纵模式；3) 基于检测到的模式子图，LLM生成针对性苏格拉底式提示，引导用户自我发现。

Result: 论文提出了理论基础、框架设计、全面评估策略和验证愿景，展示了智能体架构与知识图谱的协同作用如何帮助个体识别操纵性沟通，同时保持个人自主性和安全性。

Conclusion: EchoGuard框架通过知识图谱与智能体架构的结合，为解决识别操纵性沟通的挑战提供了创新方案，能够增强个体能力，同时尊重个人自主权和安全需求。
...
<details>
  <summary>Details</summary>
Motivation: 传统机器学习开发面临新部署中数据稀缺、分布漂移下的适应性差以及手动迭代收益递减等问题，需要自动化解决方案来提升时间序列预测的准确性和适应性。

Method: 提出了SEA-TS框架，包含三个核心创新：1) 基于标准化优势分数的Metric-Advantage MCTS进行判别性搜索；2) 代码审查与运行提示细化，通过自动审查和提示更新防止类似错误；3) 全局可引导推理，实现跨轨迹知识转移。采用MAP-Elites存档保持架构多样性。
...
Conclusion: SEA-TS框架能够自主生成超越人工设计的算法创新，包括物理信息单调衰减头、站点特定日周期配置和可学习小时偏差校正等新颖架构模式，证明了自动化机器学习工程能够产生真正新颖的算法思想。
...
Result: 在多个跨域基准测试中，AoD-IP在保持强授权域性能的同时，提供可靠的未授权检测，支持用户控制授权以适应动态环境部署。
...
TL;DR: 研究发现CLIP文本编码器的某些中间层在SF-CDFSL任务中被移除后性能反而提升，作者发现这些层的信息实际上是有益的，但视觉差距阻碍了其充分利用。论文提出了一种重新利用这些"丢失层"信息的方法。


<details>
  <summary>Details</summary>
Motivation: 当前SF-CDFSL研究中发现CLIP的文本编码器更适合跨域任务，但作者观察到移除文本编码器的某些中间层能提升性能。作者希望深入理解这一现象，并解决这些层中有用信息未被充分利用的问题。

Method: 提出一种在层级别和编码器级别重新利用"丢失层"信息的方法，指导视觉分支在域偏移下的重新学习，解决文本编码器中信息未充分利用的问题。

Result: 在多种设置、骨干网络（CLIP、SigLip、PE-Core）和任务（4个CDFSL数据集和10个Meta-dataset数据集）上的广泛实验证明了方法的有效性。

Conclusion: 论文揭示了CLIP文本编码器中"丢失层"现象的本质，并提出了一种重新利用这些层中有用信息的方法，有效解决了SF-CDFSL中信息未充分利用的问题。
...
TL;DR: OPENDEV是一个开源命令行AI编码代理，采用复合AI系统架构，通过双代理设计、惰性工具发现、自适应上下文压缩等技术，为终端原生AI辅助提供安全可扩展的基础。


<details>
  <summary>Details</summary>
Motivation: AI编码辅助正从复杂的IDE插件转向终端原生代理，CLI代理在开发者管理源代码、执行构建和部署环境的地方直接运行，为长期开发任务提供前所未有的自主性。

Method: 采用复合AI系统架构，包括：1) 工作负载专用模型路由；2) 分离规划与执行的双代理架构；3) 惰性工具发现；4) 逐步减少旧观察的自适应上下文压缩；5) 跨会话积累项目知识的自动化记忆系统；6) 通过事件驱动系统提醒对抗指令衰减。

Result: OPENDEV通过强制执行显式推理阶段和优先考虑上下文效率，为终端优先的AI辅助提供了安全、可扩展的基础，为稳健的自主软件工程提供了蓝图。

Conclusion: OPENDEV代表了AI编码辅助向终端原生代理的范式转变，通过创新的系统架构解决了自主协助中的安全控制和上下文管理挑战，为未来AI辅助软件开发奠定了基础。
...
TL;DR: 提出平均偏误有界性(A-BB)框架，为LLM评判系统提供形式化保证，减少可测量偏误带来的危害，在Arena-Hard-Auto数据集上验证有效性。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统向自主反馈循环发展，需要可靠的自动评判机制。在缺乏确定性基准的场景中，LLM作为评判者成为实用选择，但现有系统无法在未知或对抗性偏误下提供强保证。

Method: 提出平均偏误有界性(A-BB)算法框架，形式化保证LLM评判中任何可测量偏误带来的危害/影响都能被减少。在Arena-Hard-Auto数据集上使用四个LLM评判者进行评估。

Result: 在格式化和模式偏误设置下，实现了(tau=0.5, delta=0.01)的偏误有界保证，同时保持61-99%与原始排名的相关性，大多数评判者-偏误组合超过80%相关性。

Conclusion: A-BB框架为LLM评判系统提供了形式化的偏误控制保证，在保持评判质量的同时显著降低偏误影响，为自主AI系统的可靠反馈机制提供了解决方案。
...
TL;DR: 提出基于对比似然奖励（CLR）的"内外混合"奖励框架，解决RAG中忠实性评估和自奖励机制缺失问题，通过优化有/无支持证据条件下的对数似然差距来增强模型对相关证据的提取和上下文信心。


<details>
  <summary>Details</summary>
Motivation: 现有RAG导向的强化学习方法依赖外部奖励，但无法有效评估文档忠实性，在开放域设置中可能误判相似答案；缺乏RAG自奖励机制；自判断缺乏客观反馈会导致幻觉累积和模型崩溃。

Method: 提出"内外混合"奖励框架，核心是对比似然奖励（CLR），直接优化在有支持证据和无支持证据条件下响应的对数似然差距，鼓励模型提取相关证据并增强在特定上下文中的信心。

Result: 实验表明，该方法（单独使用或与外部正确性奖励结合）在单跳、多跳、垂直领域和忠实性基准测试中均取得强劲性能。

Conclusion: 提出的CLR框架有效解决了RAG中忠实性评估和自奖励机制问题，通过优化证据条件概率差距来增强模型推理能力，训练代码和模型即将发布。
...
<details>
  <summary>Details</summary>
Motivation: 当前领域特定语言模型通常先在大规模通用数据上预训练，然后在有限目标数据上微调。传统做法在微调时混合通用数据以防止灾难性遗忘，但作者发现回放通用数据实际上能提升目标任务性能

Method: 在控制实验环境中（4M目标token，4B总token，150M参数模型），对比不同训练策略：1）传统微调；2）带通用数据回放的微调；3）带通用数据回放的中期训练。分析数据调度策略，研究预训练中目标数据引入时机对回放效果的影响

Result: 通用数据回放使目标数据效率提升：微调时1.87倍，中期训练时2.06倍。回放效果在预训练中目标数据较少时更显著。在实际8B参数模型中，回放使智能网页导航成功率提升4.5%，巴斯克语问答准确率提升2%

Conclusion: 与传统认知相反，在目标领域微调时回放通用数据不仅能防止灾难性遗忘，还能显著提升目标任务的性能和数据效率，这一发现在不同规模和实际应用场景中都得到了验证
...
TL;DR: 开发了一个统一的CT到模拟管道，用于机器人辅助人工耳蜗植入的接触感知插入规划和验证，通过低维可微分模型和患者特异性解剖重建来减少创伤风险。


<details>
  <summary>Details</summary>
Motivation: 机器人辅助人工耳蜗植入需要精确预测和调节接触力，以最小化内耳创伤并防止锁定和屈曲等故障模式。需要将先进的医学成像与机器人技术结合，实现自主、精确的干预。
...
Result: 模拟和实验验证了变形和力趋势，展示了减少锁定/屈曲风险和改进插入深度。CT成像增强了机器人辅助内耳手术中的建模、规划和安全性能力。

Conclusion: 该研究展示了基于CT的成像如何增强机器人辅助内耳手术中的建模、规划和安全性能力，为精确、安全的耳蜗植入提供了统一的规划验证管道。
...
TL;DR: Omni-Manip：一种基于LiDAR的端到端3D视觉运动策略，利用全景点云实现大工作空间中的稳健操作，无需频繁重新定位机器人。


<details>
  <summary>Details</summary>
Motivation: 人形机器人在非结构化环境中的灵巧操作面临感知限制，传统RGB-D方案视野狭窄且存在自遮挡问题，需要频繁移动底座，引入运动不确定性和安全风险。现有扩展感知的方法（如主动视觉系统和第三方视角相机）存在机械复杂性、校准依赖性和延迟问题。

Method: 提出Omni-Manip，一种端到端LiDAR驱动的3D视觉运动策略。方法处理全景点云，通过时间感知注意力池化机制高效编码稀疏3D数据并捕捉时间依赖性。开发全身遥操作系统用于高效收集全身协调数据。

Result: 在仿真和真实环境中的广泛实验表明，Omni-Manip在大工作空间和杂乱场景中实现了稳健性能，优于依赖自我中心深度相机的基线方法。

Conclusion: Omni-Manip通过360°全景感知解决了传统视觉运动策略的视野限制问题，使机器人能够在无需频繁重新定位的情况下在大工作空间中执行稳健操作，为人形机器人在受限环境中的部署提供了有效解决方案。
