...
<details>
  <summary>Details</summary>
Motivation: 结直肠癌是全球主要癌症死因之一，息肉是重要前兆。现有深度学习方法虽然有效但缺乏可解释性，限制了临床应用。

Method: 集成U-Net架构和梯度加权类激活映射（Grad-CAM）的可解释深度学习框架，在1000张标注内镜图像数据集上训练评估。

Result: 测试集平均IoU达到0.9257，训练和验证集Dice系数均高于0.96，Grad-CAM可视化确认预测基于临床相关区域。

Conclusion: PolypSeg-GradCAM通过结合高精度分割和可解释性，向可靠、可信的AI辅助结肠镜检查迈出了重要一步。
...
Method: URNet包含局部-全局精炼模块，能够有效捕捉细粒度局部细节和长距离全局上下文；引入基于KL散度的不确定性建模方法来增强预测可靠性。

Result: 在DSEC数据集上的大量实验表明，URNet在定性和定量评估中都一致优于最先进的方法。

Conclusion: URNet通过不确定性感知的精炼网络架构，显著提升了事件相机立体深度估计的性能和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 从单目视频中重建动态人体和静态场景存在困难，特别是在快速运动下RGB帧会出现运动模糊。事件相机具有微秒级时间分辨率的优势，更适合动态人体重建。
...
Conclusion: 该方法无需外部人体掩码，简化了高斯集合管理，通过事件引导有效提升了快速运动区域的局部保真度，为动态人体与静态场景的联合重建提供了有效解决方案。
...
Method: 利用XMem模型的内存增强架构，通过集成记忆机制来实时跟踪肿瘤运动，在有限标注数据下实现高效分割

Result: 由于实验记录丢失无法提供精确定量结果，但初步开发印象显示框架具有合理的分割性能并满足临床实时要求

Conclusion: 该工作为改善MRI引导放疗中的肿瘤追踪精度做出了贡献，尽管缺乏详细实验结果，但展示了XMem框架在实时肿瘤分割方面的潜力
...
<details>
  <summary>Details</summary>
Motivation: 当前基于单潜在向量的图像反演方法难以平衡结构保真度和语义准确性，导致重建图像出现语义漂移（如细节模糊或属性错误）。
...
Conclusion: PDLS框架无需逐图像优化即可有效防止语义漂移，同时保持细节完整性，为扩散模型图像反演提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统方法中由于不准确的相机姿态导致的累积误差问题，以及利用事件相机对动态变化的高敏感性来改进去模糊效果。

Method: 1) 使用预训练的DUSt3R密集立体模块直接从模糊图像获取准确初始点云，避免相机姿态计算误差；2) 引入事件流，通过解码潜在清晰图像为场景重建优化提供精细监督信号。

Result: 在多种场景下的实验表明，DeblurSplat不仅能生成高保真度的新视角图像，而且在渲染效率上显著优于现有的去模糊3D-GS方法。

Conclusion: 该方法成功实现了无需SfM的高效去模糊3D重建，结合事件相机的优势，在质量和效率方面都达到了先进水平。
...
<details>
  <summary>Details</summary>
Motivation: 解决边缘设备上SNN推理的延迟和能耗问题，以及边缘-云协同推理系统中的高延迟和特征传输成本问题。

Method: 引入学习型脉冲驱动压缩模块减少数据传输，采用动态提前退出机制基于输出置信度自适应终止推理。

Result: 在CIFAR10、Caltech等数据集上，数据传输减少高达2048倍，边缘能耗降低90%以上，端到端延迟减少3倍，精度损失小于2%。
...
<details>
  <summary>Details</summary>
Motivation: 数字人脸生成和操纵技术的快速发展带来了严重的社会风险，现有检测模型难以跟上不断演变的伪造技术。需要让模型能够快速适应新领域，同时避免遗忘已学习的伪造类型。

Method: 使用发育型混合专家架构，以LoRA模型作为专家。分为Real-LoRA学习真实人脸知识，多个Fake-LoRAs捕获不同伪造类型的增量信息。通过正交梯度和正交损失防止梯度干扰和灾难性遗忘。

Result: 在数据集和操纵类型增量协议下的实验结果表明该方法的有效性。

Conclusion: 将人脸伪造检测构建为持续学习问题，提出的发育型MoE架构能够有效应对不断演变的伪造技术，实现快速适应和知识保留。
...
<details>
  <summary>Details</summary>
Motivation: 研究结构偏差在不同需求机制下对预测牛鞭效应的帮助或阻碍，为混合建模提供具体指导。

Method: 使用单级测试平台，比较完全学习的神经ODE和物理信息通用微分方程在三种需求机制（AR(1)、i.i.d.高斯、重尾对数正态）下的表现，通过不同比例的训练数据评估多步预测性能。

Result: 在结构化机制下，UDE泛化能力更好：AR(1)需求下库存RMSE从4.92降至0.26，高斯需求下从5.96降至0.95；但在重尾对数正态冲击下，NODE的灵活性更优。
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在持续学习中面临的灾难性遗忘问题，使模型能够在动态数据分布下自我进化，适应工业场景中多样且不断变化的任务需求。
...
Conclusion: MoE-CL适用于需要持续适应和稳定迁移的大规模工业部署，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法仅将轨迹视为采样位置序列，在欧几里得空间中分析，忽略了底层移动网络（如道路或交通网络）的约束和连通性信息，导致异常检测效果有限。

Method: GETAD框架使用图注意力网络学习道路感知嵌入，结合基于图的位置编码，采用Transformer解码器建模序列移动，并使用结合自回归预测和监督链接预测的多目标损失函数。

Result: 在真实世界和合成数据集上的实验表明，GETAD相比现有方法实现了持续改进，特别是在检测道路约束环境中的细微异常方面表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 现有神经网络扩展方法中新添加的神经元往往无法适应已训练网络而变得不活跃，无法有效贡献容量增长

Method: SWE方法通过将新神经元与现有神经元耦合实现平滑集成，SVoD方法基于梯度分配神经元到不同层

Result: 在四个数据集上的广泛测试表明，该方法能有效抑制神经元不活跃现象，性能优于其他扩展方法和基线

Conclusion: 所提出的方法能够有效解决神经网络扩展中的神经元失效问题，实现更好的性能表现
...
<details>
  <summary>Details</summary>
Motivation: 随着隐私问题日益严重，公开发布的贝叶斯网络模型需要保护训练数据中的敏感信息。现有噪声添加方法虽然提供隐私保护，但严重损害模型效用。

Method: 将贝叶斯网络转换为置信网络，通过添加不确定性来掩盖学习到的BN，同时保持有意义的推理能力。识别并隐藏关键学习信息以防止攻击者恢复原始BN。

Result: 数值实验表明，通过调整CN超参数可以调节隐私增益，CN能够在保护隐私的同时保持模型的有效推理能力。
...
Conclusion: 这种两阶段方法实现了高效的概率推理，同时避免了全数据集条件独立性测试的不可行成本。
...
Method: 1）时空概念编码器（STCE）整合地理位置、POI类别语义和周期性时间节奏；2）认知轨迹记忆（CTM）自适应过滤历史访问，强调近期和行为显著事件；3）生活方式概念库（LCB）提供结构化人类偏好线索；4）任务导向生成头将学习表示转换为下游任务预测。

Result: 在Gowalla、WeePlace、Brightkite和FourSquare四个数据集上，对下一个位置预测、轨迹-用户识别和时间估计三个任务进行评估，结果显示相比强基线有持续且显著的性能提升。

Conclusion: 生成式建模为构建更鲁棒、可解释和可泛化的人类移动智能系统提供了有前景的基础。GSTM-HMU在从复杂移动数据中提取语义规律方面表现出有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有土壤水分时间序列分析方法要么依赖基于阈值的规则，要么需要大量数据的机器学习模型，存在适应性和可解释性不足的问题。

Method: SPADE利用ChatGPT-4.1的推理能力，将时间序列数据转换为文本表示，通过领域知识提示模板识别灌溉事件、估计净灌溉增益、检测和分类异常。

Result: 在真实土壤水分传感器数据上的实验表明，SPADE在异常检测方面优于现有方法，召回率和F1分数更高，灌溉事件检测精度和召回率也很高。

Conclusion: 该研究展示了LLMs作为精准农业可扩展、适应性工具的潜力，能够整合定性知识和数据驱动推理，为土壤水分监测和灌溉调度提供可操作的见解。
...
### [45] [HSGM: Hierarchical Segment-Graph Memory for Scalable Long-Text Semantics](https://arxiv.org/abs/2509.18168)
*Dong Liu,Yanxuan Yu*
...
Method: HSGM框架将输入分解为M个段落，为每个段落构建局部语义图，提取摘要节点形成全局图内存，支持增量更新和分层查询处理。
...
### [46] [Similarity Field Theory: A Mathematical Framework for Intelligence](https://arxiv.org/abs/2509.18218)
*Kei-Sing Ng*
...
Method: 定义了相似性场S: U×U→[0,1]，满足自反性但允许不对称性和非传递性；系统演化序列Zp=(Xp,S(p))；概念K诱导纤维Fα(K)；生成算子G。通过这个框架形式化智能的定义。

Result: 证明了两个定理：(i)不对称性阻止相互包含；(ii)稳定性需要锚坐标或最终限制在f的水平集内。这些结果确保相似性场的演化既受约束又可解释。

Conclusion: 相似性场理论为理解大语言模型提供了框架，并可将它们用作社会认知的实验探针。该理论为智能系统的表征和构建提供了基础。
...
### [47] [Towards General Computer Control with Hierarchical Agents and Multi-Level Action Spaces](https://arxiv.org/abs/2509.18230)
*Zihan Dong,Xinyu Fan,Zixiang Tang,Yunqing Li*

Main category: cs.AI

TL;DR: ComputerAgent是一个轻量级分层强化学习框架，通过两层级选项过程（管理器和子策略）控制桌面应用，解决了现有MLLM方法的高延迟、样本效率低和无法在设备上部署的问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型（MLLM）在桌面应用控制中存在推理延迟高、长视野稀疏奖励任务样本效率差、无法在设备上部署等问题，需要更实用的解决方案。

Method: 采用分层强化学习框架，包含三模态状态编码器（截图、任务ID、数值状态）、元动作与早停机制、紧凑视觉骨干网络和小型策略网络（1500万参数）。

Result: 在135个真实桌面任务测试中，简单任务（<8步）成功率92.1%，困难任务（≥8步）成功率58.8%，模型大小减少四个数量级，推理时间减半，性能匹配或超过200B参数MLLM基线。
...
### [48] [Gödel Test: Can Large Language Models Solve Easy Conjectures?](https://arxiv.org/abs/2509.18383)
*Moran Feldman,Amin Karbasi*

Main category: cs.AI

TL;DR: 该论文提出了Gödel测试，评估大型语言模型能否为简单未解数学猜想生成正确证明，并在组合优化领域的五个猜想上测试了GPT-5的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管前沿AI模型在数学竞赛中表现优异，但能否解决更高级数学领域的新简单猜想仍不明确，因此需要新的评估方法。

Method: 设计了Gödel测试，选取五个组合优化猜想，提供相关源论文但隐藏作者自己的猜想，详细评估GPT-5的推理过程。

Result: GPT-5在三个较简单问题上产生接近正确的解，在问题2中甚至推翻了作者的猜想并提供了有效解，但在需要跨论文综合的问题4上失败，在更复杂的问题5中算法正确但分析失败。

Conclusion: 结果表明GPT-5在常规推理上有显著进步，偶尔展现原创性，但在跨论文综合方面仍有局限，可能是前沿模型最终通过Gödel测试的早期步骤。
...
### [49] [LLMZ+: Contextual Prompt Whitelist Principles for Agentic LLMs](https://arxiv.org/abs/2509.18557)
*Tom Pawelek,Raj Patel,Charlotte Crowell,Noorbakhsh Amiri,Sudip Mittal,Shahram Rahimi,Andy Perkins*

Main category: cs.AI

TL;DR: LLMZ+是一种针对智能AI代理的安全防御方法，通过提示白名单机制替代传统的恶意意图检测，确保只有上下文适当的安全消息能与AI代理交互。
...
Method: 提出LLMZ+方法，采用提示白名单机制，只允许符合预定义用例和操作边界的上下文适当消息与智能LLM代理交互。
...
### [50] [Solving Math Word Problems Using Estimation Verification and Equation Generation](https://arxiv.org/abs/2509.18565)
*Mitchell Piehl,Dillon Wilson,Ananya Kalita,Jugal Kalita*

Main category: cs.AI

TL;DR: 该论文提出了一种新方法，通过将数学应用题分解为方程，使用外部符号方程求解器求解，并通过LLM进行答案估计和验证来提高解题准确率。
...
Method: 首先将问题分解为方程，使用外部求解器得到答案；然后让LLM估计答案进行验证；如果验证失败，采用迭代修正过程确保找到正确答案。

Result: 该方法在数值和代数数学应用题数据集上取得了新的最先进结果，平均比之前最佳结果提高了近2%，并在三角函数应用题上获得满意结果。
...
### [51] [Autonomous Data Agents: A New Opportunity for Smart Data](https://arxiv.org/abs/2509.18710)
*Yanjie Fu,Dongjie Wang,Wangyang Ying,Xiangliang Zhang,Huan Liu,Jian Pei*
...
Method: DataAgents通过动态规划工作流、调用强大工具和适应多样化数据任务，能够处理数据收集、集成、预处理、选择、转换、重新加权、增强、重新编程、修复和检索等操作。
...
### [52] [Remaining Time Prediction in Outbound Warehouse Processes: A Case Study (Short Paper)](https://arxiv.org/abs/2509.18986)
*Erik Penther,Michael Grohs,Jana-Rebecca Rehse*
...
<details>
  <summary>Details</summary>
Motivation: 预测性过程监控旨在预测正在进行的过程执行的未来，其中剩余时间预测是常见目标。本研究旨在在实际物流公司的出库仓库流程中比较不同剩余时间预测方法的性能。

Method: 在物流公司提供的包含169,523条轨迹的原始事件日志上，比较了四种剩余时间预测方法，包括深度学习模型和浅层方法（如传统提升技术）。

Result: 深度学习模型达到最高准确率，但浅层方法如传统提升技术能达到竞争性准确率，且需要显著更少的计算资源。
...
### [54] [Speculate Deep and Accurate: Lossless and Training-Free Acceleration for Offloaded LLMs via Substitute Speculative Decoding](https://arxiv.org/abs/2509.18344)
...
### [55] [Interactive Real-Time Speaker Diarization Correction with Human Feedback](https://arxiv.org/abs/2509.18377)
...
### [56] [Financial Risk Relation Identification through Dual-view Adaptation](https://arxiv.org/abs/2509.18775)
...
### [57] [Systematic Comparative Analysis of Large Pretrained Language Models on Contextualized Medication Event Extraction](https://arxiv.org/abs/2509.19224)
...
### [58] [Event Causality Identification with Synthetic Control](https://arxiv.org/abs/2509.18156)
*Haoyu Wang,Fengze Liu,Jiayao Zhang,Dan Roth,Kyle Richardson*

Main category: cs.CL

TL;DR: 本文提出了一种基于Rubin因果模型的事件因果关系识别方法，通过将第一个事件视为处理，第二个事件视为结果，利用合成控制方法生成虚拟双胞胎来估计因果效应。
...
Method: 采用Rubin因果模型框架，将时间顺序排列的事件分别视为处理和结果。使用合成控制方法从相关历史数据中生成虚拟双胞胎，该双胞胎在处理前与主角有相同的生活经历但经历了不同的处理干预。利用文本嵌入合成和反演技术实现这一过程。

Result: 该方法在因果关系基准测试COPES-hard上表现出色，识别效果优于包括GPT-4在内的先前方法，能够更稳健地识别因果关系。

Conclusion: 基于Rubin因果模型和合成控制方法的事件因果关系识别框架提供了一种更可靠的因果关系识别途径，有效解决了传统方法中虚假因果关系识别的问题。
...
TL;DR: SubSpec是一种无需训练的即插即用方法，通过生成低比特量化替代层来构建高度对齐的草稿模型，加速参数卸载，在内存受限的GPU上实现无损推理加速。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在内存受限消费级GPU上部署的挑战，现有方法要么牺牲质量（压缩），要么推理速度慢（参数卸载），而推测解码方法需要额外训练且加速效果有限。

Method: 构建高度对齐的草稿模型：1）从卸载的目标LLM部分生成低比特量化替代层；2）共享剩余的GPU驻留层和KV缓存；3）使用草稿模型生成多个候选token，由目标LLM并行验证。

Result: 在8GB VRAM限制下，Qwen2.5 7B在MT-Bench上实现9.1倍加速；在24GB VRAM限制下，Qwen2.5 32B在流行生成基准测试中平均实现12.5倍加速。

Conclusion: SubSpec提供了一种无需训练、无损的解决方案，通过高度对齐的草稿模型实现了显著的推理加速，解决了参数卸载中的性能瓶颈问题。
...
### [60] [Interactive Real-Time Speaker Diarization Correction with Human Feedback](https://arxiv.org/abs/2509.18377)
*Xinlu He,Yiwen Guan,Badrivishal Paurana,Zilin Dai,Jacob Whitehill*

Main category: cs.CL

TL;DR: 提出一个LLM辅助的说话人日志校正系统，通过实时用户反馈来修正说话人归属错误，结合流式ASR和日志技术，使用LLM生成摘要并接受语音反馈。
...
Method: 采用流式ASR和说话人日志，使用LLM生成摘要，接受用户语音反馈并实时整合。开发了分割合并技术检测多说话人段，以及基于用户校正的在线说话人注册。
...
### [61] [Financial Risk Relation Identification through Dual-view Adaptation](https://arxiv.org/abs/2509.18775)
*Wei-Ning Chiu,Yu-Hsiang Wang,Andy Hsiao,Yu-Shiang Huang,Chuan-Ju Wang*

Main category: cs.CL

TL;DR: 提出一种基于10-K文件的无监督方法，用于提取企业间风险关系，通过自然语言处理技术捕捉隐含风险连接，优于现有基线方法。
...
Method: 利用10-K文件作为数据源，通过无监督微调基于时间序列和词汇模式，开发领域特定的金融编码器来捕捉隐含风险关系。

Result: 大量实验表明该方法在多个评估设置下优于强基线模型。

Conclusion: 该方法能够系统化提取企业间风险关系，为投资组合管理和投资策略提供透明、可解释的分析工具。
...
### [62] [Systematic Comparative Analysis of Large Pretrained Language Models on Contextualized Medication Event Extraction](https://arxiv.org/abs/2509.19224)
*Tariq Abdul-Quddoos,Xishuang Dong,Lijun Qian*

Main category: cs.CL

TL;DR: 本文比较了多种预训练注意力模型在电子健康记录信息提取任务中的表现，发现临床数据预训练的模型在药物和药物事件检测方面更有效，而通用领域预训练的Bert Base在药物相关事件上下文分类方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: 基于注意力的模型已成为临床笔记自然语言处理的主要方法，但需要比较不同预训练模型在电子健康记录信息提取任务中的性能差异。
...
Result: 临床数据预训练的模型在药物和药物事件检测方面表现更好，而Bert Base在药物相关事件上下文分类方面F1分数最高。

Conclusion: 不同预训练模型在不同医疗NLP任务中各有优势，需要根据具体任务需求选择合适的模型。
...
### [63] [Number Adaptive Formation Flight Planning via Affine Deformable Guidance in Narrow Environments](https://arxiv.org/abs/2509.18636)
...
### [64] [DexSkin: High-Coverage Conformable Robotic Skin for Learning Contact-Rich Manipulation](https://arxiv.org/abs/2509.18830)
...
### [65] [Proactive-reactive detection and mitigation of intermittent faults in robot swarms](https://arxiv.org/abs/2509.19246)
...
### [66] [Number Adaptive Formation Flight Planning via Affine Deformable Guidance in Narrow Environments](https://arxiv.org/abs/2509.18636)
*Yuan Zhou,Jialiang Hou,Guangtong Xu,Fei Gao*

Main category: cs.RO

TL;DR: 本文提出了一种基于可变形虚拟结构（DVS）的无人机编队规划方法，通过连续时空变换解决狭窄环境中无人机数量变化时的编队维护问题。


<details>
  <summary>Details</summary>
Motivation: 在狭窄环境中，无人机数量的变化会阻碍编队规划收敛到期望配置，需要一种能够适应数量变化和环境约束的编队规划方法。

Method: 使用Lloyd算法进行均匀分割和匈牙利算法进行分配（PAAS）来保持编队形状完整性，通过基于基元的路径搜索和非线性轨迹优化规划DVS的时空轨迹，最后进行分布式轨迹规划。

Result: 在模拟环境中，该方法支持多达15%的无人机加入或离开编队，并能快速恢复期望编队形状，相比现有方法具有更快的编队恢复能力和环境适应性。

Conclusion: 真实世界实验验证了该编队规划方法的有效性和鲁棒性，能够实现自适应过渡并确保在狭窄环境中的适应性。
...
### [67] [DexSkin: High-Coverage Conformable Robotic Skin for Learning Contact-Rich Manipulation](https://arxiv.org/abs/2509.18830)
*Suzannah Wistreich,Baiyu Shi,Stephen Tian,Samuel Clarke,Michael Nath,Chengyi Xu,Zhenan Bao,Jiajun Wu*

Main category: cs.RO

TL;DR: DexSkin是一种柔软、可适应的电容式电子皮肤，能够在机器人手指表面提供高灵敏度、局部化和可校准的触觉感知，用于学习接触丰富的操作任务。


<details>
  <summary>Details</summary>
Motivation: 人类皮肤能够在大面积曲面区域上定位有意和无意的接触事件，而复制这种触觉感知能力对于灵巧的机器人操作系统仍是一个长期挑战。

Method: 开发了DexSkin电子皮肤，将其安装在平行夹爪手指上，覆盖几乎整个手指表面，并在学习演示框架中评估其在重定向物体和缠绕弹性带等任务中的表现。

Result: DexSkin能够有效学习需要整个手指表面感知覆盖的挑战性操作任务，并且可以通过校准实现传感器实例间的模型迁移，适用于在线强化学习。

Conclusion: DexSkin展示了其在学习真实世界、接触丰富的操作任务中的适用性和实用性。
...
### [68] [Proactive-reactive detection and mitigation of intermittent faults in robot swarms](https://arxiv.org/abs/2509.19246)
*Sinan Oğuz,Emanuele Garone,Marco Dorigo,Mary Katherine Heinrich*

Main category: cs.RO

TL;DR: 本文提出了一种针对机器人群体中间歇性故障的主动-反应式检测与缓解策略，利用自组织备份层和多路网络中的分布式共识来解决传统方法难以检测的间歇性故障问题。


<details>
  <summary>Details</summary>
Motivation: 间歇性故障在机器人群体中频繁出现但难以检测，现有研究主要关注永久性故障。自组织神经系统（SoNS）方法首次实现了机器人群体自组织持久网络结构，为检测间歇性故障提供了新机会。

Method: 采用主动-反应式策略：主动方面，机器人在故障发生前自组织动态备份路径；反应方面，使用一次性似然比检验比较多路网络中不同路径的信息，实现早期故障检测。检测到故障后，通信以自组织方式临时重路由。

Result: 在形成控制场景中验证，间歇性故障被有效阻止破坏期望形成的收敛，实现了高故障检测精度和低误报率。

Conclusion: 该方法成功解决了机器人群体中间歇性故障的检测和缓解问题，为提升机器人群体可靠性提供了有效解决方案。
